...
~> **Note:** You must specify at most one of `ident_by_icmp6_type`, `ident_by_icmp_type`, `ident_by_ip_protocol` or `port`.
...
~> **Note:** You must specify exactly one of `equal_to`, `greater_than`, `less_than` or `pattern_match`.
