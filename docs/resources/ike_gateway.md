...
~> **Note:** You must specify exactly one of `certificate` or `pre_shared_key`.
...
~> **Note:** You must specify exactly one of `dynamic`, `fqdn` or `ip`.
...
```bash
terraform import scm_ike_gateway.example folder:::id
```

or

```bash
terraform import scm_ike_gateway.example :snippet::id
```

or

```bash
terraform import scm_ike_gateway.example ::device:id
```
