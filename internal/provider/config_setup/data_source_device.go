...
func (d *DeviceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DeviceDataSource.Schema ---")
...
func (d *DeviceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DevicesDataSource.Configure ---")
...
func (d *DeviceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if !data.Id.IsNull() {
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading Devices", fmt.Sprintf("Could not read Devices with ID %s: %s", objectId, err.Error()))
		...
		packedObject, diags := packDevicesFromSdk(ctx, *scmObject)
		...
		tflog.Debug(ctx, "Reading Devices data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DevicesAPI.ListDevices(ctx)

		listResponse, httpRes, err := listReq.Execute()
		if err != nil {
			resp.Diagnostics.AddError("Error Listing Devicess", fmt.Sprintf("Could not list Devicess: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *config_setup.Devices
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("Devices Not Found", fmt.Sprintf("No Devices found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDevicesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Devices data source.")
		return
	}
