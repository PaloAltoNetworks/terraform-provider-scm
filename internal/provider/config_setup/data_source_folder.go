...
// DATA SOURCE for SCM Folder (Package: config_setup)
var (
...
func (d *FolderDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: FolderDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading Folders data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.FoldersAPI.GetFolderByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading Folders data source by Name", map[string]interface{}{"name": objectName})
		...
		var foundObject *config_setup.Folders
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("Folders Not Found", fmt.Sprintf("No Folders found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packFoldersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Folders data source.")
		return
	}
