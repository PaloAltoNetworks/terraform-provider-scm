...
// DATA SOURCE for SCM TrustedTenantOverview (Package: config_setup)
var (
	_ datasource.DataSource              = &TrustedTenantOverviewDataSource{}
	_ datasource.DataSourceWithConfigure = &TrustedTenantOverviewDataSource{}
)

func NewTrustedTenantOverviewDataSource() datasource.DataSource {
	return &TrustedTenantOverviewDataSource{}
}

// TrustedTenantOverviewDataSource defines the data source implementation.
type TrustedTenantOverviewDataSource struct {
	client *config_setup.APIClient
}

func (d *TrustedTenantOverviewDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: TrustedTenantOverviewDataSource.Metadata ---")
	resp.TypeName = "scm_trusted_tenant_overview"
}

func (d *TrustedTenantOverviewDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: TrustedTenantOverviewDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.TrustedTenantOverviewDataSourceSchema
}

func (d *TrustedTenantOverviewDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: TrustedTenantOverviewDataSource.Configure ---")
...
func (d *TrustedTenantOverviewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: TrustedTenantOverviewDataSource.Read ---")
	...
	tflog.Debug(ctx, "Reading Singleton TrustedTenantOverview")

	// 1. Perform the API call (no ID argument)
	readReq := d.client.TrustedTenantsOverviewAPI.GetTrustedTenantsOverview(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Reading TrustedTenantOverview", err.Error())
		return
	}

	// 4. Dynamic Response Handling (Reflection + JSON)
	var scmObject *config_setup.TrustedTenantOverview
	...
	if val.Kind() == reflect.Struct {
		...
		if dataField.IsValid() && dataField.Kind() == reflect.Slice {
			if dataField.Len() > 0 {
				...
				var targetStruct config_setup.TrustedTenantOverview
	...
	packedObject, diags := packTrustedTenantOverviewFromSdk(ctx, *scmObject)
	...
	// Force synthetic ID
	data.Tfid = types.StringValue("singleton_trusted_tenant_overview")
