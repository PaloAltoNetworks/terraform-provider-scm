...
// RESOURCE for SCM Folder (Package: config_setup)
var (
...
func (r *FolderResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.FoldersResourceSchema
}
...
func (r *FolderResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Folder")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Folders{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackFoldersToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating folders on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.FoldersAPI.CreateFolder(ctx).Folders(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating folders", err.Error())
	...
	packedObject, diags := packFoldersFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created folders", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *FolderResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Folders - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Folder")
	var data, savestate models.Folders
	...
	tflog.Debug(ctx, "Reading folders from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.FoldersAPI.GetFolderByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no folders on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
	...
	packedObject, diags := packFoldersFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Folders{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *FolderResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Folders which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Folder")
	var plan, state models.Folders
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Folders{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating folders on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.FoldersAPI.UpdateFolderByID(ctx, objectId).Folders(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no folders on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating folders", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packFoldersFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated folders", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *FolderResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting folders", map[string]interface{}{"id": objectId})
	deleteReq := r.client.FoldersAPI.DeleteFolderByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting folders", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
