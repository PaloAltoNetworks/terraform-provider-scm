...
// RESOURCE for SCM Snippet (Package: config_setup)
var (
...
func (r *SnippetResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SnippetsResourceSchema
}
...
func (r *SnippetResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Snippet")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Snippets{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSnippetsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating snippets on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SnippetsAPI.CreateSnippet(ctx).Snippets(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating snippets", err.Error())
	...
	packedObject, diags := packSnippetsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created snippets", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SnippetResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Snippets - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Snippet")
	var data, savestate models.Snippets
	...
	tflog.Debug(ctx, "Reading snippets from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SnippetsAPI.GetSnippetByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no snippets on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading snippets", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Snippets{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SnippetResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Snippets which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Snippet")
	var plan, state models.Snippets
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Snippets{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating snippets on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SnippetsAPI.UpdateSnippetByID(ctx, objectId).Snippets(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no snippets on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating snippets", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSnippetsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated snippets", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *SnippetResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting snippets", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SnippetsAPI.DeleteSnippetByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting snippets", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
