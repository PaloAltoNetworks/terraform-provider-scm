...
// --- Unpacker for Trusts ---
func unpackTrustsToSdk(ctx context.Context, obj types.Object) (*config_setup.Trusts, diag.Diagnostics) {
	tflog.Debug(ctx, "Entering unpack helper for models.Trusts", map[string]interface{}{"tf_object": obj})
	diags := diag.Diagnostics{}
	var model models.Trusts
	diags.Append(obj.As(ctx, &model, basetypes.ObjectAsOptions{})...)
	if diags.HasError() {
		tflog.Error(ctx, "Error converting Terraform object to Go model", map[string]interface{}{"diags": diags})
		return nil, diags
	...
	if !model.TrustId.IsNull() && !model.TrustId.IsUnknown() {
		val := int32(model.TrustId.ValueInt64())
		sdk.TrustId.Set(&val)
		tflog.Debug(ctx, "Unpacked NullableInt32", map[string]interface{}{"field": "TrustId"})
	}

	// Handling Primitives
	if !model.Tsg.IsNull() && !model.Tsg.IsUnknown() {
		sdk.Tsg = model.Tsg.ValueStringPointer()
		tflog.Debug(ctx, "Unpacked primitive pointer", map[string]interface{}{"field": "Tsg", "value": *sdk.Tsg})
	}

	diags.Append(d...)

	tflog.Debug(ctx, "Exiting unpack helper for models.Trusts", map[string]interface{}{"has_errors": diags.HasError()})
	return &sdk, diags

}

// --- Packer for Trusts ---
func packTrustsFromSdk(ctx context.Context, sdk config_setup.Trusts) (types.Object, diag.Diagnostics) {
	tflog.Debug(ctx, "Entering pack helper for models.Trusts", map[string]interface{}{"sdk_struct": sdk})
	diags := diag.Diagnostics{}
	var model models.Trusts
	var d diag.Diagnostics
	// Handling Primitives
	...
	// Handling Primitives
	// Standard primitive packing
	// Handling Nullable SDK wrapper type (NullableInt32)
	if sdk.TrustId.IsSet() {
		model.TrustId = basetypes.NewInt64Value(int64(*sdk.TrustId.Get()))
		tflog.Debug(ctx, "Packed NullableInt32", map[string]interface{}{"field": "TrustId"})
	} else {
		model.TrustId = basetypes.NewInt64Null()
	}
	// Handling Primitives
	// Standard primitive packing
	if sdk.Tsg != nil {
		model.Tsg = basetypes.NewStringValue(*sdk.Tsg)
		tflog.Debug(ctx, "Packed primitive pointer", map[string]interface{}{"field": "Tsg", "value": *sdk.Tsg})
	} else {
		model.Tsg = basetypes.NewStringNull()
	}
	diags.Append(d...)

	obj, d := types.ObjectValueFrom(ctx, models.Trusts{}.AttrTypes(), &model)
	tflog.Debug(ctx, "Final object to be returned from pack helper", map[string]interface{}{"object": obj})
	diags.Append(d...)
	tflog.Debug(ctx, "Exiting pack helper for models.Trusts", map[string]interface{}{"has_errors": diags.HasError()})
	return obj, diags

}

// --- List Unpacker for Trusts ---
func unpackTrustsListToSdk(ctx context.Context, list types.List) ([]config_setup.Trusts, diag.Diagnostics) {
	tflog.Debug(ctx, "Entering list unpack helper for models.Trusts")
	diags := diag.Diagnostics{}
	var data []models.Trusts
	diags.Append(list.ElementsAs(ctx, &data, false)...)
	if diags.HasError() {
		tflog.Error(ctx, "Error converting list elements to Go models", map[string]interface{}{"diags": diags})
		return nil, diags
	...
	ans := make([]config_setup.Trusts, 0, len(data))
	for i, item := range data {
		tflog.Debug(ctx, "Unpacking item from list", map[string]interface{}{"index": i})
		obj, _ := types.ObjectValueFrom(ctx, models.Trusts{}.AttrTypes(), &item)
		unpacked, d := unpackTrustsToSdk(ctx, obj)
		diags.Append(d...)
		if unpacked != nil {
			ans = append(ans, *unpacked)
		}
	}
	tflog.Debug(ctx, "Exiting list unpack helper for models.Trusts", map[string]interface{}{"has_errors": diags.HasError()})
	return ans, diags
}

// --- List Packer for Trusts ---
func packTrustsListFromSdk(ctx context.Context, sdks []config_setup.Trusts) (types.List, diag.Diagnostics) {
	tflog.Debug(ctx, "Entering list pack helper for models.Trusts")
	diags := diag.Diagnostics{}
	var data []models.Trusts
	...
	for i, sdk := range sdks {
		tflog.Debug(ctx, "Packing item to list", map[string]interface{}{"index": i})
		var model models.Trusts
		obj, d := packTrustsFromSdk(ctx, sdk)
		diags.Append(d...)
		if diags.HasError() {
			return basetypes.NewListNull(models.Trusts{}.AttrType()), diags
		}
		diags.Append(obj.As(ctx, &model, basetypes.ObjectAsOptions{})...)
		data = append(data, model)
	}
	tflog.Debug(ctx, "Exiting list pack helper for models.Trusts", map[string]interface{}{"has_errors": diags.HasError()})
	return basetypes.NewListValueFrom(ctx, models.Trusts{}.AttrType(), data)
}
