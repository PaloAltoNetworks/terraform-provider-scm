...
// RESOURCE for SCM Variable (Package: config_setup)
var (
...
func (r *VariableResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.VariablesResourceSchema
}
...
func (r *VariableResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Variable")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Variables{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackVariablesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating variables on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.VariablesAPI.CreateVariable(ctx).Variables(*unpackedScmObject)
	...
	if !data.Snippet.IsNull() {
		createReq = createReq.Snippet(data.Snippet.ValueString())
	}
	if !data.Device.IsNull() {
		createReq = createReq.Device(data.Device.ValueString())
	}

	// 5. Execute the API call.
	createdObject, _, err := createReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error creating variables", err.Error())
	...
	packedObject, diags := packVariablesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created variables", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *VariableResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Variables - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Variable")
	var data, savestate models.Variables
	...
	tflog.Debug(ctx, "Reading variables from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.VariablesAPI.GetVariableByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no variables on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Variables{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *VariableResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Variables which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Variable")
	var plan, state models.Variables
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Variables{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating variables on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.VariablesAPI.UpdateVariableByID(ctx, objectId).Variables(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no variables on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating variables", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packVariablesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated variables", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *VariableResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting variables", map[string]interface{}{"id": objectId})
	deleteReq := r.client.VariablesAPI.DeleteVariableByID(ctx, objectId)
