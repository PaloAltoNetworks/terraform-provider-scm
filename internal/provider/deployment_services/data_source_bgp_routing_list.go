...
// BgpRoutingListDataSource defines the data source implementation.
type BgpRoutingListDataSource struct {
	client *deployment_services.APIClient
}

func (d *BgpRoutingListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_bgp_routing_list"
}

func (d *BgpRoutingListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.BgpRoutingListDataSourceSchema
}

func (d *BgpRoutingListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *BgpRoutingListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.BgpRoutingListModel
	...
	listReq := d.client.BGPRoutingAPI.GetBGPRouting(ctx)
	...
	packedList, diags := packBgpRoutingListFromSdk(ctx, listResponse.GetData())
