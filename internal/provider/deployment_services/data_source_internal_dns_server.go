...
// DATA SOURCE for SCM InternalDnsServer (Package: deployment_services)
var (
...
// InternalDnsServerDataSource defines the data source implementation.
...
func (d *InternalDnsServerDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: InternalDnsServerDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.InternalDnsServersDataSourceSchema
}

func (d *InternalDnsServerDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: InternalDnsServersDataSource.Configure ---")
...
func (d *InternalDnsServerDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: InternalDnsServerDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading InternalDnsServers data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.InternalDNSServersAPI.GetInternalDNSServersByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading InternalDnsServers data source by Name", map[string]interface{}{"name": objectName})
		...
		var foundObject *deployment_services.InternalDnsServers
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("InternalDnsServers Not Found", fmt.Sprintf("No InternalDnsServers found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packInternalDnsServersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the InternalDnsServers data source.")
		return
	}
