...
func NewLocationListDataSource() datasource.DataSource {
	return &LocationListDataSource{}
}

// LocationListDataSource defines the data source implementation.
type LocationListDataSource struct {
	client *deployment_services.APIClient
}

func (d *LocationListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "scm_location_list"
}

func (d *LocationListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.LocationsListDataSourceSchema
}

func (d *LocationListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *LocationListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.LocationsListModel
	...
	listReq := d.client.NetworkLocationsAPI.ListLocations(ctx)

	// Execute the request.
	listResponse, _, err := listReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error Listing Locationss", fmt.Sprintf("Could not list Locationss: %s", err.Error()))
	...
	packedList, diags := packLocationsListFromSdk(ctx, listResponse)
