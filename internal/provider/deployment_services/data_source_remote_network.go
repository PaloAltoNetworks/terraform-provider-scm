...
// RemoteNetworkDataSource defines the data source implementation.
...
func (d *RemoteNetworkDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: RemoteNetworkDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.RemoteNetworksDataSourceSchema
}

func (d *RemoteNetworkDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: RemoteNetworksDataSource.Configure ---")
...
func (d *RemoteNetworkDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: RemoteNetworkDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading RemoteNetworks data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.RemoteNetworksAPI.GetRemoteNetworksByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading RemoteNetworks data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.RemoteNetworksAPI.ListRemoteNetworks(ctx)
		...
		var foundObject *deployment_services.RemoteNetworks
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("RemoteNetworks Not Found", fmt.Sprintf("No RemoteNetworks found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packRemoteNetworksFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the RemoteNetworks data source.")
		return
	}
