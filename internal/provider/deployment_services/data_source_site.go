...
// DATA SOURCE for SCM Site (Package: deployment_services)
var (
...
func (d *SiteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SiteDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		objectId := data.Id.ValueString()
		tflog.Debug(ctx, "Reading Sites data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SitesAPI.GetSitesByID(ctx, objectId)
		...
		var foundObject *deployment_services.Sites
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("Sites Not Found", fmt.Sprintf("No Sites found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSitesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Sites data source.")
		return
	}
