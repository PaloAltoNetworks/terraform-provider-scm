...
// RESOURCE for SCM BgpRouting (Package: deployment_services)
var (
...
func (r *BgpRoutingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for BgpRouting")
...
func (r *BgpRoutingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, "Starting Read function for BgpRouting")
	...
	tflog.Debug(ctx, "Reading bgp_routing from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.BGPRoutingAPI.(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_routing on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
...
func (r *BgpRoutingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Debug(ctx, "Starting Update function for BgpRouting")
	...
	tflog.Debug(ctx, "Updating bgp_routing on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.BGPRoutingAPI.UpdateBGPRouting(ctx, objectId).BgpRouting(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_routing on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
...
func (r *BgpRoutingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting bgp_routing", map[string]interface{}{"id": objectId})
	deleteReq := r.client.BGPRoutingAPI.(ctx, objectId)
