...
// RESOURCE for SCM SharedInfrastructureSetting (Package: deployment_services)
var (
...
func (r *SharedInfrastructureSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SharedInfrastructureSetting")
...
func (r *SharedInfrastructureSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	tflog.Debug(ctx, "Starting Read function for SharedInfrastructureSetting")
	...
	tflog.Debug(ctx, "Reading shared_infrastructure_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SharedInfrastructureSettingsAPI.(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no shared_infrastructure_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
...
func (r *SharedInfrastructureSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	tflog.Debug(ctx, "Starting Update function for SharedInfrastructureSetting")
	...
	tflog.Debug(ctx, "Updating shared_infrastructure_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SharedInfrastructureSettingsAPI.UpdateSharedInfrastructureSettings(ctx, objectId).SharedInfrastructureSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no shared_infrastructure_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
...
func (r *SharedInfrastructureSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting shared_infrastructure_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SharedInfrastructureSettingsAPI.(ctx, objectId)
