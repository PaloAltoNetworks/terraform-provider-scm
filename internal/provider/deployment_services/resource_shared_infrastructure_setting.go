...
func (r *SharedInfrastructureSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
    ...
    unpackedScmObject, diags := unpackEditSharedInfrastructureSettingsToSdk(ctx, planObject)
    resp.Diagnostics.Append(diags...)
    if resp.Diagnostics.HasError() { return }
    ...
    var err error
       ...
       // Call Update (PUT)
       updateReq := r.client.SharedInfrastructureSettingsAPI.UpdateSharedInfrastructureSettings(ctx).EditSharedInfrastructureSettings(*unpackedScmObject)
