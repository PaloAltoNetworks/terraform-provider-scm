...
var (
	_ datasource.DataSource              = &AuthenticationSettingListDataSource{}
	_ datasource.DataSourceWithConfigure = &AuthenticationSettingListDataSource{}
)

func NewAuthenticationSettingListDataSource() datasource.DataSource {
	return &AuthenticationSettingListDataSource{}
}

// AuthenticationSettingListDataSource defines the data source implementation.
type AuthenticationSettingListDataSource struct {
	client *device_settings.APIClient
}

func (d *AuthenticationSettingListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_authentication_setting_list"
}

func (d *AuthenticationSettingListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.AuthenticationSettingsListDataSourceSchema
}

func (d *AuthenticationSettingListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *AuthenticationSettingListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.AuthenticationSettingsListModel
	...
	listReq := d.client.AuthenticationSettingsAPI.ListAuthenticationSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing AuthenticationSettingss", fmt.Sprintf("Could not list AuthenticationSettingss: %s", err.Error()))
	...
	packedList, diags := packAuthenticationSettingsListFromSdk(ctx, listResponse.GetData())
