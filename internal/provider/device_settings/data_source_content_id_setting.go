...
// DATA SOURCE for SCM ContentIdSetting (Package: device_settings)
var (
	_ datasource.DataSource              = &ContentIdSettingDataSource{}
	_ datasource.DataSourceWithConfigure = &ContentIdSettingDataSource{}
)

func NewContentIdSettingDataSource() datasource.DataSource {
	return &ContentIdSettingDataSource{}
}

// ContentIdSettingDataSource defines the data source implementation.
type ContentIdSettingDataSource struct {
...
func (d *ContentIdSettingDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ContentIdSettingDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ContentIdSettingsDataSourceSchema
}

func (d *ContentIdSettingDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ContentIdSettingsDataSource.Configure ---")
...
func (d *ContentIdSettingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ContentIdSettingDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ContentIdSettings data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ContentIDSettingsAPI.GetContentIDSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ContentIdSettings", fmt.Sprintf("Could not read ContentIdSettings with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ContentIdSettings data source.")
		return
	}
