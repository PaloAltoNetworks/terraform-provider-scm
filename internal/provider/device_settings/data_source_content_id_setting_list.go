...
var (
	_ datasource.DataSource              = &ContentIdSettingListDataSource{}
	_ datasource.DataSourceWithConfigure = &ContentIdSettingListDataSource{}
)

func NewContentIdSettingListDataSource() datasource.DataSource {
	return &ContentIdSettingListDataSource{}
}

// ContentIdSettingListDataSource defines the data source implementation.
type ContentIdSettingListDataSource struct {
...
func (d *ContentIdSettingListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.ContentIdSettingsListDataSourceSchema
}

func (d *ContentIdSettingListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ContentIdSettingListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.ContentIdSettingsListModel
	...
	listReq := d.client.ContentIDSettingsAPI.ListContentIDSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing ContentIdSettingss", fmt.Sprintf("Could not list ContentIdSettingss: %s", err.Error()))
	...
	packedList, diags := packContentIdSettingsListFromSdk(ctx, listResponse.GetData())
