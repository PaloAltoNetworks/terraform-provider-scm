...
// DATA SOURCE for SCM DeviceRedistributionCollector (Package: device_settings)
var (
	_ datasource.DataSource              = &DeviceRedistributionCollectorDataSource{}
	_ datasource.DataSourceWithConfigure = &DeviceRedistributionCollectorDataSource{}
)

func NewDeviceRedistributionCollectorDataSource() datasource.DataSource {
	return &DeviceRedistributionCollectorDataSource{}
}

// DeviceRedistributionCollectorDataSource defines the data source implementation.
type DeviceRedistributionCollectorDataSource struct {
...
func (d *DeviceRedistributionCollectorDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DeviceRedistributionCollectorDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DeviceRedistributionCollectorDataSourceSchema
}

func (d *DeviceRedistributionCollectorDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DeviceRedistributionCollectorDataSource.Configure ---")
...
func (d *DeviceRedistributionCollectorDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DeviceRedistributionCollectorDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.DeviceRedistributionCollectorSettingsAPI.GetDeviceRedistributionCollectorSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading DeviceRedistributionCollector", fmt.Sprintf("Could not read DeviceRedistributionCollector with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DeviceRedistributionCollector data source.")
		return
	}
