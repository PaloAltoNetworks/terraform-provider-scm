...
// DATA SOURCE for SCM GeneralSetting (Package: device_settings)
var (
	_ datasource.DataSource              = &GeneralSettingDataSource{}
	_ datasource.DataSourceWithConfigure = &GeneralSettingDataSource{}
)

func NewGeneralSettingDataSource() datasource.DataSource {
	return &GeneralSettingDataSource{}
}

// GeneralSettingDataSource defines the data source implementation.
type GeneralSettingDataSource struct {
...
func (d *GeneralSettingDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: GeneralSettingDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.GeneralSettingsDataSourceSchema
}

func (d *GeneralSettingDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: GeneralSettingsDataSource.Configure ---")
...
func (d *GeneralSettingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: GeneralSettingDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading GeneralSettings data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.GeneralSettingsAPI.GetGeneralSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading GeneralSettings", fmt.Sprintf("Could not read GeneralSettings with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the GeneralSettings data source.")
		return
	}
