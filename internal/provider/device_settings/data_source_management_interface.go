...
// DATA SOURCE for SCM ManagementInterface (Package: device_settings)
var (
	_ datasource.DataSource              = &ManagementInterfaceDataSource{}
	_ datasource.DataSourceWithConfigure = &ManagementInterfaceDataSource{}
)

func NewManagementInterfaceDataSource() datasource.DataSource {
	return &ManagementInterfaceDataSource{}
}

// ManagementInterfaceDataSource defines the data source implementation.
...
func (d *ManagementInterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ManagementInterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ManagementInterfaceDataSourceSchema
}
...
func (d *ManagementInterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ManagementInterfaceDataSource.Configure ---")
...
func (d *ManagementInterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ManagementInterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ManagementInterface data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ManagementInterfaceSettingsAPI.GetManagementInterfaceSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ManagementInterface", fmt.Sprintf("Could not read ManagementInterface with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ManagementInterface data source.")
		return
	}
