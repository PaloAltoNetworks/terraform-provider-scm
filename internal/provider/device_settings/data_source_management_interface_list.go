...
var (
	_ datasource.DataSource              = &ManagementInterfaceListDataSource{}
	_ datasource.DataSourceWithConfigure = &ManagementInterfaceListDataSource{}
)

func NewManagementInterfaceListDataSource() datasource.DataSource {
	return &ManagementInterfaceListDataSource{}
}

// ManagementInterfaceListDataSource defines the data source implementation.
type ManagementInterfaceListDataSource struct {
	client *device_settings.APIClient
}

func (d *ManagementInterfaceListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_management_interface_list"
}

func (d *ManagementInterfaceListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.ManagementInterfaceListDataSourceSchema
}

func (d *ManagementInterfaceListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ManagementInterfaceListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.ManagementInterfaceListModel
	...
	listReq := d.client.ManagementInterfaceSettingsAPI.ListManagementInterfaceSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing ManagementInterfaces", fmt.Sprintf("Could not list ManagementInterfaces: %s", err.Error()))
	...
	packedList, diags := packManagementInterfaceListFromSdk(ctx, listResponse.GetData())
