...
// DATA SOURCE for SCM MotdBannerSetting (Package: device_settings)
var (
	_ datasource.DataSource              = &MotdBannerSettingDataSource{}
	_ datasource.DataSourceWithConfigure = &MotdBannerSettingDataSource{}
)

func NewMotdBannerSettingDataSource() datasource.DataSource {
	return &MotdBannerSettingDataSource{}
}

// MotdBannerSettingDataSource defines the data source implementation.
type MotdBannerSettingDataSource struct {
...
func (d *MotdBannerSettingDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: MotdBannerSettingDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.MotdBannerSettingsDataSourceSchema
}

func (d *MotdBannerSettingDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: MotdBannerSettingsDataSource.Configure ---")
...
func (d *MotdBannerSettingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: MotdBannerSettingDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.LoginBannerSettingsAPI.GetLoginBannerSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading MotdBannerSettings", fmt.Sprintf("Could not read MotdBannerSettings with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the MotdBannerSettings data source.")
		return
	}
