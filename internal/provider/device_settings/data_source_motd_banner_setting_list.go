...
var (
	_ datasource.DataSource              = &MotdBannerSettingListDataSource{}
	_ datasource.DataSourceWithConfigure = &MotdBannerSettingListDataSource{}
)

func NewMotdBannerSettingListDataSource() datasource.DataSource {
	return &MotdBannerSettingListDataSource{}
}

// MotdBannerSettingListDataSource defines the data source implementation.
type MotdBannerSettingListDataSource struct {
	client *device_settings.APIClient
}

func (d *MotdBannerSettingListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_motd_banner_setting_list"
}

func (d *MotdBannerSettingListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.MotdBannerSettingsListDataSourceSchema
}

func (d *MotdBannerSettingListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *MotdBannerSettingListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.MotdBannerSettingsListModel
	...
	listReq := d.client.LoginBannerSettingsAPI.ListLoginBannerSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing MotdBannerSettingss", fmt.Sprintf("Could not list MotdBannerSettingss: %s", err.Error()))
	...
	packedList, diags := packMotdBannerSettingsListFromSdk(ctx, listResponse.GetData())
