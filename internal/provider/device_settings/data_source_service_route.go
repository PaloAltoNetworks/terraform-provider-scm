...
// DATA SOURCE for SCM ServiceRoute (Package: device_settings)
var (
	_ datasource.DataSource              = &ServiceRouteDataSource{}
	_ datasource.DataSourceWithConfigure = &ServiceRouteDataSource{}
)

func NewServiceRouteDataSource() datasource.DataSource {
	return &ServiceRouteDataSource{}
}

// ServiceRouteDataSource defines the data source implementation.
type ServiceRouteDataSource struct {
...
func (d *ServiceRouteDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceRouteDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ServiceRouteDataSourceSchema
}

func (d *ServiceRouteDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceRouteDataSource.Configure ---")
...
func (d *ServiceRouteDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceRouteDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.ServiceRouteSettingsAPI.GetServiceRouteSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ServiceRoute", fmt.Sprintf("Could not read ServiceRoute with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ServiceRoute data source.")
		return
	}
