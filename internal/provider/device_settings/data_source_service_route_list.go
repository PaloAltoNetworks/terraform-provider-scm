...
var (
	_ datasource.DataSource              = &ServiceRouteListDataSource{}
	_ datasource.DataSourceWithConfigure = &ServiceRouteListDataSource{}
)

func NewServiceRouteListDataSource() datasource.DataSource {
	return &ServiceRouteListDataSource{}
}

// ServiceRouteListDataSource defines the data source implementation.
type ServiceRouteListDataSource struct {
	client *device_settings.APIClient
}

func (d *ServiceRouteListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_service_route_list"
}

func (d *ServiceRouteListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.ServiceRouteListDataSourceSchema
}

func (d *ServiceRouteListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ServiceRouteListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.ServiceRouteListModel
	...
	listReq := d.client.ServiceRouteSettingsAPI.ListServiceRouteSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing ServiceRoutes", fmt.Sprintf("Could not list ServiceRoutes: %s", err.Error()))
	...
	packedList, diags := packServiceRouteListFromSdk(ctx, listResponse.GetData())
