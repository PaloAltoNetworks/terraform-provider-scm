...
// DATA SOURCE for SCM SessionSetting (Package: device_settings)
var (
	_ datasource.DataSource              = &SessionSettingDataSource{}
	_ datasource.DataSourceWithConfigure = &SessionSettingDataSource{}
)

func NewSessionSettingDataSource() datasource.DataSource {
	return &SessionSettingDataSource{}
}

// SessionSettingDataSource defines the data source implementation.
type SessionSettingDataSource struct {
...
func (d *SessionSettingDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SessionSettingDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SessionSettingsDataSourceSchema
}

func (d *SessionSettingDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SessionSettingsDataSource.Configure ---")
...
func (d *SessionSettingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SessionSettingDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.SessionSettingsAPI.GetSessionSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SessionSettings", fmt.Sprintf("Could not read SessionSettings with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SessionSettings data source.")
		return
	}
