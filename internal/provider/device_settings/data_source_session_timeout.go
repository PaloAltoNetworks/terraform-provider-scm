...
// DATA SOURCE for SCM SessionTimeout (Package: device_settings)
var (
	_ datasource.DataSource              = &SessionTimeoutDataSource{}
	_ datasource.DataSourceWithConfigure = &SessionTimeoutDataSource{}
)

func NewSessionTimeoutDataSource() datasource.DataSource {
	return &SessionTimeoutDataSource{}
}

// SessionTimeoutDataSource defines the data source implementation.
type SessionTimeoutDataSource struct {
...
func (d *SessionTimeoutDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SessionTimeoutDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SessionTimeoutsDataSourceSchema
}

func (d *SessionTimeoutDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SessionTimeoutsDataSource.Configure ---")
...
func (d *SessionTimeoutDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SessionTimeoutDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading SessionTimeouts data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SessionTimeoutsSettingsAPI.GetSessionTimeoutsSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SessionTimeouts", fmt.Sprintf("Could not read SessionTimeouts with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SessionTimeouts data source.")
		return
	}
