...
var (
	_ datasource.DataSource              = &SessionTimeoutListDataSource{}
	_ datasource.DataSourceWithConfigure = &SessionTimeoutListDataSource{}
)

func NewSessionTimeoutListDataSource() datasource.DataSource {
	return &SessionTimeoutListDataSource{}
}

// SessionTimeoutListDataSource defines the data source implementation.
type SessionTimeoutListDataSource struct {
...
func (d *SessionTimeoutListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.SessionTimeoutsListDataSourceSchema
}

func (d *SessionTimeoutListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SessionTimeoutListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.SessionTimeoutsListModel
	...
	listReq := d.client.SessionTimeoutsSettingsAPI.ListSessionTimeoutsSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing SessionTimeoutss", fmt.Sprintf("Could not list SessionTimeoutss: %s", err.Error()))
	...
	packedList, diags := packSessionTimeoutsListFromSdk(ctx, listResponse.GetData())
