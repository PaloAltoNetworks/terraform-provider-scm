...
// DATA SOURCE for SCM TcpSetting (Package: device_settings)
var (
	_ datasource.DataSource              = &TcpSettingDataSource{}
	_ datasource.DataSourceWithConfigure = &TcpSettingDataSource{}
)

func NewTcpSettingDataSource() datasource.DataSource {
	return &TcpSettingDataSource{}
}

// TcpSettingDataSource defines the data source implementation.
type TcpSettingDataSource struct {
...
func (d *TcpSettingDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: TcpSettingDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.TcpSettingsDataSourceSchema
}

func (d *TcpSettingDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: TcpSettingsDataSource.Configure ---")
...
func (d *TcpSettingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: TcpSettingDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading TcpSettings data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.TCPSettingsAPI.GetTCPSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading TcpSettings", fmt.Sprintf("Could not read TcpSettings with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the TcpSettings data source.")
		return
	}
