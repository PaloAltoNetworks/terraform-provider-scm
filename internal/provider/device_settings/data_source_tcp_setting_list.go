...
var (
	_ datasource.DataSource              = &TcpSettingListDataSource{}
	_ datasource.DataSourceWithConfigure = &TcpSettingListDataSource{}
)

func NewTcpSettingListDataSource() datasource.DataSource {
	return &TcpSettingListDataSource{}
}

// TcpSettingListDataSource defines the data source implementation.
type TcpSettingListDataSource struct {
...
func (d *TcpSettingListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.TcpSettingsListDataSourceSchema
}

func (d *TcpSettingListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *TcpSettingListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.TcpSettingsListModel
	...
	listReq := d.client.TCPSettingsAPI.ListTCPSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing TcpSettingss", fmt.Sprintf("Could not list TcpSettingss: %s", err.Error()))
	...
	packedList, diags := packTcpSettingsListFromSdk(ctx, listResponse.GetData())
