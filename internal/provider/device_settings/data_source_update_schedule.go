...
// DATA SOURCE for SCM UpdateSchedule (Package: device_settings)
var (
	_ datasource.DataSource              = &UpdateScheduleDataSource{}
	_ datasource.DataSourceWithConfigure = &UpdateScheduleDataSource{}
)

func NewUpdateScheduleDataSource() datasource.DataSource {
	return &UpdateScheduleDataSource{}
}

// UpdateScheduleDataSource defines the data source implementation.
type UpdateScheduleDataSource struct {
...
func (d *UpdateScheduleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: UpdateScheduleDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.UpdateScheduleDataSourceSchema
}

func (d *UpdateScheduleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: UpdateScheduleDataSource.Configure ---")
...
func (d *UpdateScheduleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: UpdateScheduleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading UpdateSchedule data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.UpdateScheduleSettingsAPI.GetUpdateScheduleSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading UpdateSchedule", fmt.Sprintf("Could not read UpdateSchedule with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the UpdateSchedule data source.")
		return
	}
