...
// DATA SOURCE for SCM VpnSetting (Package: device_settings)
var (
	_ datasource.DataSource              = &VpnSettingDataSource{}
	_ datasource.DataSourceWithConfigure = &VpnSettingDataSource{}
)

func NewVpnSettingDataSource() datasource.DataSource {
	return &VpnSettingDataSource{}
}

// VpnSettingDataSource defines the data source implementation.
type VpnSettingDataSource struct {
	client *device_settings.APIClient
}

func (d *VpnSettingDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: VpnSettingDataSource.Metadata ---")
	resp.TypeName = req.ProviderTypeName + "_vpn_setting"
}

func (d *VpnSettingDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: VpnSettingDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.VpnSettingsDataSourceSchema
}

func (d *VpnSettingDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: VpnSettingsDataSource.Configure ---")
...
func (d *VpnSettingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: VpnSettingDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading VpnSettings data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.VPNSettingsAPI.GetVPNSettingsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading VpnSettings", fmt.Sprintf("Could not read VpnSettings with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the VpnSettings data source.")
		return
	}
