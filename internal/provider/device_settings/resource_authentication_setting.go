...
// RESOURCE for SCM AuthenticationSetting (Package: device_settings)
var (
	_ resource.Resource                = &AuthenticationSettingResource{}
	_ resource.ResourceWithConfigure   = &AuthenticationSettingResource{}
	_ resource.ResourceWithImportState = &AuthenticationSettingResource{}
)

func NewAuthenticationSettingResource() resource.Resource {
	return &AuthenticationSettingResource{}
}

// AuthenticationSettingResource defines the resource implementation.
type AuthenticationSettingResource struct {
	client *device_settings.APIClient
}

func (r *AuthenticationSettingResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_authentication_setting"
}

func (r *AuthenticationSettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AuthenticationSettingsResourceSchema
}

func (r *AuthenticationSettingResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *AuthenticationSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for AuthenticationSetting")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AuthenticationSettings{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackAuthenticationSettingsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating authentication_settings on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.AuthenticationSettingsAPI.CreateAuthenticationSettings(ctx).AuthenticationSettings(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating authentication_settings", err.Error())
	...
	packedObject, diags := packAuthenticationSettingsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created authentication_settings", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *AuthenticationSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.AuthenticationSettings - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for AuthenticationSetting")
	var data, savestate models.AuthenticationSettings
	...
	tflog.Debug(ctx, "Reading authentication_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.AuthenticationSettingsAPI.GetAuthenticationSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no authentication_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading authentication_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAuthenticationSettingsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *AuthenticationSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.AuthenticationSettings which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for AuthenticationSetting")
	var plan, state models.AuthenticationSettings
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AuthenticationSettings{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating authentication_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.AuthenticationSettingsAPI.UpdateAuthenticationSettingsByID(ctx, objectId).AuthenticationSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no authentication_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating authentication_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAuthenticationSettingsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated authentication_settings", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *AuthenticationSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.AuthenticationSettings
	...
	tflog.Debug(ctx, "Deleting authentication_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AuthenticationSettingsAPI.DeleteAuthenticationSettingsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting authentication_settings", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.AuthenticationSettingsAPI.GetAuthenticationSettingsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *AuthenticationSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
