...
// RESOURCE for SCM ContentIdSetting (Package: device_settings)
var (
	_ resource.Resource                = &ContentIdSettingResource{}
	_ resource.ResourceWithConfigure   = &ContentIdSettingResource{}
	_ resource.ResourceWithImportState = &ContentIdSettingResource{}
)

func NewContentIdSettingResource() resource.Resource {
	return &ContentIdSettingResource{}
}

// ContentIdSettingResource defines the resource implementation.
type ContentIdSettingResource struct {
	client *device_settings.APIClient
}

func (r *ContentIdSettingResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_content_id_setting"
}

func (r *ContentIdSettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ContentIdSettingsResourceSchema
}

func (r *ContentIdSettingResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *ContentIdSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ContentIdSetting")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ContentIdSettings{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackContentIdSettingsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating content_id_settings on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ContentIDSettingsAPI.CreateContentIDSettings(ctx).ContentIdSettings(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating content_id_settings", err.Error())
	...
	packedObject, diags := packContentIdSettingsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created content_id_settings", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ContentIdSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ContentIdSettings - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ContentIdSetting")
	var data, savestate models.ContentIdSettings
	...
	tflog.Debug(ctx, "Reading content_id_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ContentIDSettingsAPI.GetContentIDSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no content_id_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading content_id_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packContentIdSettingsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.ContentIdSettings{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ContentIdSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.ContentIdSettings which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ContentIdSetting")
	var plan, state models.ContentIdSettings
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ContentIdSettings{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating content_id_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ContentIDSettingsAPI.UpdateContentIDSettingsByID(ctx, objectId).ContentIdSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no content_id_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating content_id_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packContentIdSettingsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated content_id_settings", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *ContentIdSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.ContentIdSettings
	...
	tflog.Debug(ctx, "Deleting content_id_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ContentIDSettingsAPI.DeleteContentIDSettingsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting content_id_settings", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *ContentIdSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
