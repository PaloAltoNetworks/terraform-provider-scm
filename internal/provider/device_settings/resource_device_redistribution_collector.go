...
// RESOURCE for SCM DeviceRedistributionCollector (Package: device_settings)
var (
	_ resource.Resource                = &DeviceRedistributionCollectorResource{}
	_ resource.ResourceWithConfigure   = &DeviceRedistributionCollectorResource{}
	_ resource.ResourceWithImportState = &DeviceRedistributionCollectorResource{}
)

func NewDeviceRedistributionCollectorResource() resource.Resource {
	return &DeviceRedistributionCollectorResource{}
}

// DeviceRedistributionCollectorResource defines the resource implementation.
type DeviceRedistributionCollectorResource struct {
	client *device_settings.APIClient
}

func (r *DeviceRedistributionCollectorResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_device_redistribution_collector"
}

func (r *DeviceRedistributionCollectorResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DeviceRedistributionCollectorResourceSchema
}

func (r *DeviceRedistributionCollectorResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DeviceRedistributionCollectorResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DeviceRedistributionCollector")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DeviceRedistributionCollector{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDeviceRedistributionCollectorToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating device_redistribution_collector on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DeviceRedistributionCollectorSettingsAPI.CreateDeviceRedistributionCollectorSettings(ctx).DeviceRedistributionCollector(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating device_redistribution_collector", err.Error())
	...
	packedObject, diags := packDeviceRedistributionCollectorFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created device_redistribution_collector", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DeviceRedistributionCollectorResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DeviceRedistributionCollector - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DeviceRedistributionCollector")
	var data, savestate models.DeviceRedistributionCollector
	...
	tflog.Debug(ctx, "Reading device_redistribution_collector from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DeviceRedistributionCollectorSettingsAPI.GetDeviceRedistributionCollectorSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no device_redistribution_collector on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading device_redistribution_collector", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDeviceRedistributionCollectorFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DeviceRedistributionCollectorResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.DeviceRedistributionCollector which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DeviceRedistributionCollector")
	var plan, state models.DeviceRedistributionCollector
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DeviceRedistributionCollector{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating device_redistribution_collector on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DeviceRedistributionCollectorSettingsAPI.UpdateDeviceRedistributionCollectorSettingsByID(ctx, objectId).DeviceRedistributionCollector(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no device_redistribution_collector on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating device_redistribution_collector", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDeviceRedistributionCollectorFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated device_redistribution_collector", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *DeviceRedistributionCollectorResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.DeviceRedistributionCollector
	...
	tflog.Debug(ctx, "Deleting device_redistribution_collector", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DeviceRedistributionCollectorSettingsAPI.DeleteDeviceRedistributionCollectorSettingsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting device_redistribution_collector", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.DeviceRedistributionCollectorSettingsAPI.GetDeviceRedistributionCollectorSettingsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *DeviceRedistributionCollectorResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if len(tokens) > 0 && tokens[0] != "" {
		...
		if folderValue == "Global" {
			resp.Diagnostics.AddError(
				"Invalid Folder Value in Import ID",
				"The folder value 'Global' is not allowed in import IDs. Please use 'All' instead.\nExample: terraform import scm_device_redistribution_collector.example \"All\":::\"<id>\"",
			)
			return
		}
