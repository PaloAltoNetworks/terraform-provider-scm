...
// RESOURCE for SCM GeneralSetting (Package: device_settings)
var (
	_ resource.Resource                = &GeneralSettingResource{}
	_ resource.ResourceWithConfigure   = &GeneralSettingResource{}
	_ resource.ResourceWithImportState = &GeneralSettingResource{}
)

func NewGeneralSettingResource() resource.Resource {
	return &GeneralSettingResource{}
}

// GeneralSettingResource defines the resource implementation.
type GeneralSettingResource struct {
	client *device_settings.APIClient
}

func (r *GeneralSettingResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_general_setting"
}

func (r *GeneralSettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.GeneralSettingsResourceSchema
}

func (r *GeneralSettingResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *GeneralSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for GeneralSetting")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.GeneralSettings{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackGeneralSettingsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating general_settings", err.Error())
	...
	packedObject, diags := packGeneralSettingsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created general_settings", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *GeneralSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.GeneralSettings - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for GeneralSetting")
	var data, savestate models.GeneralSettings
	...
	tflog.Debug(ctx, "Reading general_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.GeneralSettingsAPI.GetGeneralSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no general_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading general_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packGeneralSettingsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.GeneralSettings{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *GeneralSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.GeneralSettings which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for GeneralSetting")
	var plan, state models.GeneralSettings
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.GeneralSettings{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating general_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.GeneralSettingsAPI.UpdateGeneralSettingsByID(ctx, objectId).GeneralSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no general_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating general_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packGeneralSettingsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated general_settings", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *GeneralSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.GeneralSettings
	...
	tflog.Debug(ctx, "Deleting general_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.GeneralSettingsAPI.DeleteGeneralSettingsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting general_settings", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *GeneralSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
