...
// RESOURCE for SCM MotdBannerSetting (Package: device_settings)
var (
	_ resource.Resource                = &MotdBannerSettingResource{}
	_ resource.ResourceWithConfigure   = &MotdBannerSettingResource{}
	_ resource.ResourceWithImportState = &MotdBannerSettingResource{}
)

func NewMotdBannerSettingResource() resource.Resource {
	return &MotdBannerSettingResource{}
}

// MotdBannerSettingResource defines the resource implementation.
type MotdBannerSettingResource struct {
	client *device_settings.APIClient
}

func (r *MotdBannerSettingResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_motd_banner_setting"
}

func (r *MotdBannerSettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.MotdBannerSettingsResourceSchema
}

func (r *MotdBannerSettingResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *MotdBannerSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for MotdBannerSetting")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.MotdBannerSettings{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackMotdBannerSettingsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating motd_banner_settings on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.LoginBannerSettingsAPI.CreateLoginBannerSettings(ctx).MotdBannerSettings(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating motd_banner_settings", err.Error())
	...
	packedObject, diags := packMotdBannerSettingsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created motd_banner_settings", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *MotdBannerSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.MotdBannerSettings - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for MotdBannerSetting")
	var data, savestate models.MotdBannerSettings
	...
	tflog.Debug(ctx, "Reading motd_banner_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.LoginBannerSettingsAPI.GetLoginBannerSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no motd_banner_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading motd_banner_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packMotdBannerSettingsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *MotdBannerSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.MotdBannerSettings which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for MotdBannerSetting")
	var plan, state models.MotdBannerSettings
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.MotdBannerSettings{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating motd_banner_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.LoginBannerSettingsAPI.UpdateLoginBannerSettingsByID(ctx, objectId).MotdBannerSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no motd_banner_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating motd_banner_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packMotdBannerSettingsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated motd_banner_settings", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *MotdBannerSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.MotdBannerSettings
	...
	tflog.Debug(ctx, "Deleting motd_banner_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.LoginBannerSettingsAPI.DeleteLoginBannerSettingsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting motd_banner_settings", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.LoginBannerSettingsAPI.GetLoginBannerSettingsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *MotdBannerSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
