...
// RESOURCE for SCM ServiceRoute (Package: device_settings)
var (
	_ resource.Resource                = &ServiceRouteResource{}
	_ resource.ResourceWithConfigure   = &ServiceRouteResource{}
	_ resource.ResourceWithImportState = &ServiceRouteResource{}
)

func NewServiceRouteResource() resource.Resource {
	return &ServiceRouteResource{}
}

// ServiceRouteResource defines the resource implementation.
type ServiceRouteResource struct {
	client *device_settings.APIClient
}

func (r *ServiceRouteResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_service_route"
}

func (r *ServiceRouteResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ServiceRouteResourceSchema
}

func (r *ServiceRouteResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *ServiceRouteResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ServiceRoute")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ServiceRoute{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackServiceRouteToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating service_route on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ServiceRouteSettingsAPI.CreateServiceRouteSettings(ctx).ServiceRoute(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating service_route", err.Error())
	...
	packedObject, diags := packServiceRouteFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created service_route", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ServiceRouteResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ServiceRoute - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ServiceRoute")
	var data, savestate models.ServiceRoute
	...
	tflog.Debug(ctx, "Reading service_route from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ServiceRouteSettingsAPI.GetServiceRouteSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no service_route on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading service_route", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packServiceRouteFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ServiceRouteResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.ServiceRoute which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ServiceRoute")
	var plan, state models.ServiceRoute
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ServiceRoute{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating service_route on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ServiceRouteSettingsAPI.UpdateServiceRouteSettingsByID(ctx, objectId).ServiceRoute(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no service_route on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating service_route", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packServiceRouteFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated service_route", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *ServiceRouteResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.ServiceRoute
	...
	tflog.Debug(ctx, "Deleting service_route", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ServiceRouteSettingsAPI.DeleteServiceRouteSettingsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting service_route", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.ServiceRouteSettingsAPI.GetServiceRouteSettingsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *ServiceRouteResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
