...
// RESOURCE for SCM SessionSetting (Package: device_settings)
var (
	_ resource.Resource                = &SessionSettingResource{}
	_ resource.ResourceWithConfigure   = &SessionSettingResource{}
	_ resource.ResourceWithImportState = &SessionSettingResource{}
)

func NewSessionSettingResource() resource.Resource {
	return &SessionSettingResource{}
}

// SessionSettingResource defines the resource implementation.
type SessionSettingResource struct {
	client *device_settings.APIClient
}

func (r *SessionSettingResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_session_setting"
}

func (r *SessionSettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SessionSettingsResourceSchema
}

func (r *SessionSettingResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *SessionSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SessionSetting")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SessionSettings{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSessionSettingsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating session_settings on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SessionSettingsAPI.CreateSessionSettings(ctx).SessionSettings(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating session_settings", err.Error())
	...
	packedObject, diags := packSessionSettingsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created session_settings", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SessionSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.SessionSettings - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for SessionSetting")
	var data, savestate models.SessionSettings
	...
	tflog.Debug(ctx, "Reading session_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SessionSettingsAPI.GetSessionSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no session_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading session_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSessionSettingsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.SessionSettings{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SessionSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.SessionSettings which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for SessionSetting")
	var plan, state models.SessionSettings
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SessionSettings{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating session_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SessionSettingsAPI.UpdateSessionSettingsByID(ctx, objectId).SessionSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no session_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating session_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSessionSettingsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated session_settings", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *SessionSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.SessionSettings
	...
	tflog.Debug(ctx, "Deleting session_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SessionSettingsAPI.DeleteSessionSettingsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting session_settings", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *SessionSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
