...
// RESOURCE for SCM SessionTimeout (Package: device_settings)
var (
	_ resource.Resource                = &SessionTimeoutResource{}
	_ resource.ResourceWithConfigure   = &SessionTimeoutResource{}
	_ resource.ResourceWithImportState = &SessionTimeoutResource{}
)

func NewSessionTimeoutResource() resource.Resource {
	return &SessionTimeoutResource{}
}

// SessionTimeoutResource defines the resource implementation.
type SessionTimeoutResource struct {
	client *device_settings.APIClient
}

func (r *SessionTimeoutResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_session_timeout"
}

func (r *SessionTimeoutResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SessionTimeoutsResourceSchema
}

func (r *SessionTimeoutResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *SessionTimeoutResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SessionTimeout")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SessionTimeouts{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSessionTimeoutsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating session_timeouts on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SessionTimeoutsSettingsAPI.CreateSessionTimeoutsSettings(ctx).SessionTimeouts(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating session_timeouts", err.Error())
	...
	packedObject, diags := packSessionTimeoutsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created session_timeouts", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SessionTimeoutResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.SessionTimeouts - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for SessionTimeout")
	var data, savestate models.SessionTimeouts
	...
	tflog.Debug(ctx, "Reading session_timeouts from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SessionTimeoutsSettingsAPI.GetSessionTimeoutsSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no session_timeouts on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading session_timeouts", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSessionTimeoutsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SessionTimeoutResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.SessionTimeouts which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for SessionTimeout")
	var plan, state models.SessionTimeouts
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SessionTimeouts{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating session_timeouts on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SessionTimeoutsSettingsAPI.UpdateSessionTimeoutsSettingsByID(ctx, objectId).SessionTimeouts(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no session_timeouts on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating session_timeouts", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSessionTimeoutsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated session_timeouts", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *SessionTimeoutResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.SessionTimeouts
	...
	tflog.Debug(ctx, "Deleting session_timeouts", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SessionTimeoutsSettingsAPI.DeleteSessionTimeoutsSettingsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting session_timeouts", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.SessionTimeoutsSettingsAPI.GetSessionTimeoutsSettingsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *SessionTimeoutResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
