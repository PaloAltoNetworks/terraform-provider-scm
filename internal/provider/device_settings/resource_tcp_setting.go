...
// RESOURCE for SCM TcpSetting (Package: device_settings)
var (
	_ resource.Resource                = &TcpSettingResource{}
	_ resource.ResourceWithConfigure   = &TcpSettingResource{}
	_ resource.ResourceWithImportState = &TcpSettingResource{}
)

func NewTcpSettingResource() resource.Resource {
	return &TcpSettingResource{}
}

// TcpSettingResource defines the resource implementation.
type TcpSettingResource struct {
	client *device_settings.APIClient
}

func (r *TcpSettingResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_tcp_setting"
}

func (r *TcpSettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.TcpSettingsResourceSchema
}

func (r *TcpSettingResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *TcpSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for TcpSetting")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TcpSettings{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackTcpSettingsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating tcp_settings on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.TCPSettingsAPI.CreateTCPSettings(ctx).TcpSettings(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating tcp_settings", err.Error())
	...
	packedObject, diags := packTcpSettingsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created tcp_settings", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *TcpSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.TcpSettings - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for TcpSetting")
	var data, savestate models.TcpSettings
	...
	tflog.Debug(ctx, "Reading tcp_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.TCPSettingsAPI.GetTCPSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tcp_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading tcp_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTcpSettingsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.TcpSettings{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *TcpSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.TcpSettings which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for TcpSetting")
	var plan, state models.TcpSettings
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TcpSettings{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating tcp_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.TCPSettingsAPI.UpdateTCPSettingsByID(ctx, objectId).TcpSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tcp_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating tcp_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTcpSettingsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated tcp_settings", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *TcpSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.TcpSettings
	...
	tflog.Debug(ctx, "Deleting tcp_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.TCPSettingsAPI.DeleteTCPSettingsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting tcp_settings", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *TcpSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
