...
// RESOURCE for SCM UpdateSchedule (Package: device_settings)
var (
...
// UpdateScheduleResource defines the resource implementation.
type UpdateScheduleResource struct {
	client *device_settings.APIClient
}

func (r *UpdateScheduleResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_update_schedule"
}

func (r *UpdateScheduleResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.UpdateScheduleResourceSchema
}

func (r *UpdateScheduleResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *UpdateScheduleResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for UpdateSchedule")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UpdateSchedule{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackUpdateScheduleToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating update_schedule on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.UpdateScheduleSettingsAPI.CreateUpdateScheduleSettings(ctx).UpdateSchedule(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating update_schedule", err.Error())
	...
	packedObject, diags := packUpdateScheduleFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created update_schedule", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *UpdateScheduleResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.UpdateSchedule - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for UpdateSchedule")
	var data, savestate models.UpdateSchedule
	...
	tflog.Debug(ctx, "Reading update_schedule from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.UpdateScheduleSettingsAPI.GetUpdateScheduleSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no update_schedule on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading update_schedule", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packUpdateScheduleFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *UpdateScheduleResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.UpdateSchedule which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for UpdateSchedule")
	var plan, state models.UpdateSchedule
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UpdateSchedule{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating update_schedule on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.UpdateScheduleSettingsAPI.UpdateUpdateScheduleSettingsByID(ctx, objectId).UpdateSchedule(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no update_schedule on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating update_schedule", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packUpdateScheduleFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated update_schedule", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *UpdateScheduleResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.UpdateSchedule
	...
	tflog.Debug(ctx, "Deleting update_schedule", map[string]interface{}{"id": objectId})
	deleteReq := r.client.UpdateScheduleSettingsAPI.DeleteUpdateScheduleSettingsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting update_schedule", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.UpdateScheduleSettingsAPI.GetUpdateScheduleSettingsByID(ctx, objectId).Execute()
			if getErr != nil {
