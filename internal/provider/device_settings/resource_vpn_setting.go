...
// RESOURCE for SCM VpnSetting (Package: device_settings)
var (
	_ resource.Resource                = &VpnSettingResource{}
	_ resource.ResourceWithConfigure   = &VpnSettingResource{}
	_ resource.ResourceWithImportState = &VpnSettingResource{}
)

func NewVpnSettingResource() resource.Resource {
	return &VpnSettingResource{}
}

// VpnSettingResource defines the resource implementation.
type VpnSettingResource struct {
	client *device_settings.APIClient
}

func (r *VpnSettingResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_vpn_setting"
}

func (r *VpnSettingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.VpnSettingsResourceSchema
}

func (r *VpnSettingResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *VpnSettingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for VpnSetting")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.VpnSettings{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackVpnSettingsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating vpn_settings on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.VPNSettingsAPI.CreateVPNSettings(ctx).VpnSettings(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating vpn_settings", err.Error())
	...
	packedObject, diags := packVpnSettingsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created vpn_settings", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *VpnSettingResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.VpnSettings - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for VpnSetting")
	var data, savestate models.VpnSettings
	...
	tflog.Debug(ctx, "Reading vpn_settings from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.VPNSettingsAPI.GetVPNSettingsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no vpn_settings on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading vpn_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packVpnSettingsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.VpnSettings{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *VpnSettingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.VpnSettings which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for VpnSetting")
	var plan, state models.VpnSettings
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.VpnSettings{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating vpn_settings on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.VPNSettingsAPI.UpdateVPNSettingsByID(ctx, objectId).VpnSettings(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no vpn_settings on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating vpn_settings", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packVpnSettingsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated vpn_settings", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *VpnSettingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.VpnSettings
	...
	tflog.Debug(ctx, "Deleting vpn_settings", map[string]interface{}{"id": objectId})
	deleteReq := r.client.VPNSettingsAPI.DeleteVPNSettingsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting vpn_settings", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *VpnSettingResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
