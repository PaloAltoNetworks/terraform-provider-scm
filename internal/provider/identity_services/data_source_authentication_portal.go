...
// AuthenticationPortalDataSource defines the data source implementation.
...
func (d *AuthenticationPortalDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationPortalDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.AuthenticationPortalsDataSourceSchema
}

func (d *AuthenticationPortalDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationPortalsDataSource.Configure ---")
...
func (d *AuthenticationPortalDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationPortalDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.AuthenticationPortalsAPI.GetAuthenticationPortalsByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading AuthenticationPortals data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.AuthenticationPortalsAPI.ListAuthenticationPortals(ctx)
		...
		var foundObject *identity_services.AuthenticationPortals
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("AuthenticationPortals Not Found", fmt.Sprintf("No AuthenticationPortals found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packAuthenticationPortalsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the AuthenticationPortals data source.")
		return
	}
