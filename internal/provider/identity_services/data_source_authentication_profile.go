...
// DATA SOURCE for SCM AuthenticationProfile (Package: identity_services)
var (
...
// AuthenticationProfileDataSource defines the data source implementation.
...
func (d *AuthenticationProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.AuthenticationProfilesDataSourceSchema
}

func (d *AuthenticationProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationProfilesDataSource.Configure ---")
...
func (d *AuthenticationProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading AuthenticationProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.AuthenticationProfilesAPI.GetAuthenticationProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading AuthenticationProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.AuthenticationProfilesAPI.ListAuthenticationProfiles(ctx)
		...
		var foundObject *identity_services.AuthenticationProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("AuthenticationProfiles Not Found", fmt.Sprintf("No AuthenticationProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packAuthenticationProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the AuthenticationProfiles data source.")
		return
	}
