...
// AuthenticationRuleDataSource defines the data source implementation.
type AuthenticationRuleDataSource struct {
	client *identity_services.APIClient
}

func (d *AuthenticationRuleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationRuleDataSource.Metadata ---")
	resp.TypeName = "scm_authentication_rule"
}

func (d *AuthenticationRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationRuleDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.AuthenticationRulesDataSourceSchema
}

func (d *AuthenticationRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationRulesDataSource.Configure ---")
...
func (d *AuthenticationRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationRuleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading AuthenticationRules data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.AuthenticationRulesAPI.GetAuthenticationRulesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading AuthenticationRules data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.AuthenticationRulesAPI.ListAuthenticationRules(ctx)
		...
		var foundObject *identity_services.AuthenticationRules
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("AuthenticationRules Not Found", fmt.Sprintf("No AuthenticationRules found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packAuthenticationRulesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the AuthenticationRules data source.")
		return
	}
