...
// DATA SOURCE for SCM AuthenticationSequence (Package: identity_services)
var (
	_ datasource.DataSource              = &AuthenticationSequenceDataSource{}
	_ datasource.DataSourceWithConfigure = &AuthenticationSequenceDataSource{}
)

func NewAuthenticationSequenceDataSource() datasource.DataSource {
	return &AuthenticationSequenceDataSource{}
}

// AuthenticationSequenceDataSource defines the data source implementation.
type AuthenticationSequenceDataSource struct {
...
func (d *AuthenticationSequenceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationSequenceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.AuthenticationSequencesDataSourceSchema
}

func (d *AuthenticationSequenceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationSequencesDataSource.Configure ---")
...
func (d *AuthenticationSequenceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: AuthenticationSequenceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.AuthenticationSequencesAPI.GetAuthenticationSequencesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading AuthenticationSequences", fmt.Sprintf("Could not read AuthenticationSequences with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading AuthenticationSequences data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.AuthenticationSequencesAPI.ListAuthenticationSequences(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing AuthenticationSequencess", fmt.Sprintf("Could not list AuthenticationSequencess: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *identity_services.AuthenticationSequences
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("AuthenticationSequences Not Found", fmt.Sprintf("No AuthenticationSequences found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packAuthenticationSequencesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the AuthenticationSequences data source.")
		return
	}
