...
// DATA SOURCE for SCM CertificateProfile (Package: identity_services)
var (
...
// CertificateProfileDataSource defines the data source implementation.
...
func (d *CertificateProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: CertificateProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.CertificateProfilesDataSourceSchema
}

func (d *CertificateProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: CertificateProfilesDataSource.Configure ---")
...
func (d *CertificateProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: CertificateProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading CertificateProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.CertificateProfilesAPI.GetCertificateProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading CertificateProfiles", fmt.Sprintf("Could not read CertificateProfiles with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading CertificateProfiles data source by Name", map[string]interface{}{"name": objectName})
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing CertificateProfiless", fmt.Sprintf("Could not list CertificateProfiless: %s", err.Error()))
			return
		}
		if httpRes.StatusCode != 200 {
			resp.Diagnostics.AddError("Unexpected HTTP status code", fmt.Sprintf("Expected 200, got %d", httpRes.StatusCode))
		...
		var foundObject *identity_services.CertificateProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("CertificateProfiles Not Found", fmt.Sprintf("No CertificateProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packCertificateProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the CertificateProfiles data source.")
		return
	}
