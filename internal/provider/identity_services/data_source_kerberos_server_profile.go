...
// DATA SOURCE for SCM KerberosServerProfile (Package: identity_services)
var (
...
// KerberosServerProfileDataSource defines the data source implementation.
...
func (d *KerberosServerProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: KerberosServerProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.KerberosServerProfilesDataSourceSchema
}

func (d *KerberosServerProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: KerberosServerProfilesDataSource.Configure ---")
...
func (d *KerberosServerProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: KerberosServerProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading KerberosServerProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.KerberosServerProfilesAPI.GetKerberosServerProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading KerberosServerProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.KerberosServerProfilesAPI.ListKerberosServerProfiles(ctx)
		...
		var foundObject *identity_services.KerberosServerProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("KerberosServerProfiles Not Found", fmt.Sprintf("No KerberosServerProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packKerberosServerProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the KerberosServerProfiles data source.")
		return
	}
