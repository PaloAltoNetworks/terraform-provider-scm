...
// LocalUserDataSource defines the data source implementation.
...
func (d *LocalUserDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: LocalUserDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.LocalUsersDataSourceSchema
}
...
func (d *LocalUserDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: LocalUsersDataSource.Configure ---")
...
func (d *LocalUserDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LocalUserDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.LocalUsersAPI.GetLocalUsersByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading LocalUsers data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.LocalUsersAPI.ListLocalUsers(ctx)
		...
		var foundObject *identity_services.LocalUsers
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("LocalUsers Not Found", fmt.Sprintf("No LocalUsers found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLocalUsersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the LocalUsers data source.")
		return
	}
