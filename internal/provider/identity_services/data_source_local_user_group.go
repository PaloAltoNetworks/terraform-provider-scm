...
// DATA SOURCE for SCM LocalUserGroup (Package: identity_services)
var (
	_ datasource.DataSource              = &LocalUserGroupDataSource{}
	_ datasource.DataSourceWithConfigure = &LocalUserGroupDataSource{}
)

func NewLocalUserGroupDataSource() datasource.DataSource {
	return &LocalUserGroupDataSource{}
}

// LocalUserGroupDataSource defines the data source implementation.
type LocalUserGroupDataSource struct {
...
func (d *LocalUserGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: LocalUserGroupDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.LocalUserGroupsDataSourceSchema
}

func (d *LocalUserGroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: LocalUserGroupsDataSource.Configure ---")
...
func (d *LocalUserGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LocalUserGroupDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading LocalUserGroups data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.LocalUserGroupsAPI.GetLocalUserGroupsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading LocalUserGroups", fmt.Sprintf("Could not read LocalUserGroups with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading LocalUserGroups data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.LocalUserGroupsAPI.ListLocalUserGroups(ctx)
		...
		var foundObject *identity_services.LocalUserGroups
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("LocalUserGroups Not Found", fmt.Sprintf("No LocalUserGroups found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLocalUserGroupsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the LocalUserGroups data source.")
		return
	}
