...
// DATA SOURCE for SCM MfaServer (Package: identity_services)
var (
...
// MfaServerDataSource defines the data source implementation.
...
func (d *MfaServerDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: MfaServerDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.MfaServersDataSourceSchema
}

func (d *MfaServerDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: MfaServersDataSource.Configure ---")
...
func (d *MfaServerDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: MfaServerDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.MFAServersAPI.GetMFAServersByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading MfaServers data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.MFAServersAPI.ListMFAServers(ctx)
		...
		var foundObject *identity_services.MfaServers
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("MfaServers Not Found", fmt.Sprintf("No MfaServers found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packMfaServersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the MfaServers data source.")
		return
	}
