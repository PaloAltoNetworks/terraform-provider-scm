...
// DATA SOURCE for SCM OcspResponder (Package: identity_services)
var (
...
// OcspResponderDataSource defines the data source implementation.
...
func (d *OcspResponderDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: OcspResponderDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.OcspRespondersDataSourceSchema
}

func (d *OcspResponderDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: OcspRespondersDataSource.Configure ---")
...
func (d *OcspResponderDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: OcspResponderDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.OCSPRespondersAPI.GetOCSPRespondersByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading OcspResponders data source by Name", map[string]interface{}{"name": objectName})
		...
		var foundObject *identity_services.OcspResponders
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("OcspResponders Not Found", fmt.Sprintf("No OcspResponders found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packOcspRespondersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the OcspResponders data source.")
		return
	}
