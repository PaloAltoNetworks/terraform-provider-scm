...
// DATA SOURCE for SCM RadiusServerProfile (Package: identity_services)
var (
...
// RadiusServerProfileDataSource defines the data source implementation.
...
func (d *RadiusServerProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: RadiusServerProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.RadiusServerProfilesDataSourceSchema
}

func (d *RadiusServerProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: RadiusServerProfilesDataSource.Configure ---")
...
func (d *RadiusServerProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: RadiusServerProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading RadiusServerProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.RADIUSServerProfilesAPI.GetRADIUSServerProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading RadiusServerProfiles", fmt.Sprintf("Could not read RadiusServerProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading RadiusServerProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.RADIUSServerProfilesAPI.ListRADIUSServerProfiles(ctx)
		...
		var foundObject *identity_services.RadiusServerProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("RadiusServerProfiles Not Found", fmt.Sprintf("No RadiusServerProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packRadiusServerProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the RadiusServerProfiles data source.")
		return
	}
