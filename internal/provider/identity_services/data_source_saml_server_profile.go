...
// DATA SOURCE for SCM SamlServerProfile (Package: identity_services)
var (
...
// SamlServerProfileDataSource defines the data source implementation.
...
func (d *SamlServerProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SamlServerProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SamlServerProfilesDataSourceSchema
}

func (d *SamlServerProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SamlServerProfilesDataSource.Configure ---")
...
func (d *SamlServerProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SamlServerProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading SamlServerProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SAMLServerProfilesAPI.GetSAMLServerProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SamlServerProfiles", fmt.Sprintf("Could not read SamlServerProfiles with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading SamlServerProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SAMLServerProfilesAPI.ListSAMLServerProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing SamlServerProfiless", fmt.Sprintf("Could not list SamlServerProfiless: %s", err.Error()))
			return
		}
		if httpRes.StatusCode != 200 {
			resp.Diagnostics.AddError("Unexpected HTTP status code", fmt.Sprintf("Expected 200, got %d", httpRes.StatusCode))
		...
		var foundObject *identity_services.SamlServerProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SamlServerProfiles Not Found", fmt.Sprintf("No SamlServerProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSamlServerProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SamlServerProfiles data source.")
		return
	}
