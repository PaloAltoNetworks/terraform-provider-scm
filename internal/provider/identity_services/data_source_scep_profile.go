...
// DATA SOURCE for SCM ScepProfile (Package: identity_services)
var (
...
// ScepProfileDataSource defines the data source implementation.
...
func (d *ScepProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ScepProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ScepProfilesDataSourceSchema
}

func (d *ScepProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ScepProfilesDataSource.Configure ---")
...
func (d *ScepProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ScepProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ScepProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SCEPProfilesAPI.GetSCEPProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading ScepProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SCEPProfilesAPI.ListSCEPProfiles(ctx)
		...
		var foundObject *identity_services.ScepProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ScepProfiles Not Found", fmt.Sprintf("No ScepProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packScepProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ScepProfiles data source.")
		return
	}
