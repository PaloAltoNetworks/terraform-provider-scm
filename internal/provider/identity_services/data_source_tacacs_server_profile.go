...
// DATA SOURCE for SCM TacacsServerProfile (Package: identity_services)
var (
...
// TacacsServerProfileDataSource defines the data source implementation.
...
func (d *TacacsServerProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: TacacsServerProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.TacacsServerProfilesDataSourceSchema
}

func (d *TacacsServerProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: TacacsServerProfilesDataSource.Configure ---")
...
func (d *TacacsServerProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: TacacsServerProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading TacacsServerProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.TACACSServerProfilesAPI.GetTACACSServerProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading TacacsServerProfiles", fmt.Sprintf("Could not read TacacsServerProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading TacacsServerProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.TACACSServerProfilesAPI.ListTACACSServerProfiles(ctx)
		...
		var foundObject *identity_services.TacacsServerProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("TacacsServerProfiles Not Found", fmt.Sprintf("No TacacsServerProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packTacacsServerProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the TacacsServerProfiles data source.")
		return
	}
