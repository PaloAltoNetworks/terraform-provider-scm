...
// DATA SOURCE for SCM TlsServiceProfile (Package: identity_services)
var (
...
// TlsServiceProfileDataSource defines the data source implementation.
...
func (d *TlsServiceProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: TlsServiceProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.TlsServiceProfilesDataSourceSchema
}

func (d *TlsServiceProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: TlsServiceProfilesDataSource.Configure ---")
...
func (d *TlsServiceProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: TlsServiceProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading TlsServiceProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.TLSServiceProfilesAPI.GetTLSServiceProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading TlsServiceProfiles", fmt.Sprintf("Could not read TlsServiceProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading TlsServiceProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.TLSServiceProfilesAPI.ListTLSServiceProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing TlsServiceProfiless", fmt.Sprintf("Could not list TlsServiceProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *identity_services.TlsServiceProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("TlsServiceProfiles Not Found", fmt.Sprintf("No TlsServiceProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packTlsServiceProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the TlsServiceProfiles data source.")
		return
	}
