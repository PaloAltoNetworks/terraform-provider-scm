...
// AuthenticationPortalResource defines the resource implementation.
type AuthenticationPortalResource struct {
	client *identity_services.APIClient
}

func (r *AuthenticationPortalResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_authentication_portal"
}

func (r *AuthenticationPortalResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AuthenticationPortalsResourceSchema
}

func (r *AuthenticationPortalResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *AuthenticationPortalResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for AuthenticationPortal")
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating authentication_portals", err.Error())
	...
	tflog.Debug(ctx, "Created authentication_portals", map[string]interface{}{"tfid": data.Tfid.ValueString()})
...
func (r *AuthenticationPortalResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.AuthenticationPortals - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for AuthenticationPortal")
	var data, savestate models.AuthenticationPortals
	...
	tflog.Debug(ctx, "Reading authentication_portals from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.AuthenticationPortalsAPI.GetAuthenticationPortalsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no authentication_portals on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading authentication_portals", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *AuthenticationPortalResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.AuthenticationPortals which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for AuthenticationPortal")
	var plan, state models.AuthenticationPortals
	...
	updateReq := r.client.AuthenticationPortalsAPI.UpdateAuthenticationPortalsByID(ctx, objectId).AuthenticationPortals(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no authentication_portals on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating authentication_portals", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *AuthenticationPortalResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting authentication_portals", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AuthenticationPortalsAPI.DeleteAuthenticationPortalsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting authentication_portals", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.AuthenticationPortalsAPI.GetAuthenticationPortalsByID(ctx, objectId).Execute()
			if getErr != nil {
