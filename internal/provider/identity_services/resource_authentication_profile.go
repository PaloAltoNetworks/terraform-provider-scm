...
// RESOURCE for SCM AuthenticationProfile (Package: identity_services)
var (
...
// AuthenticationProfileResource defines the resource implementation.
type AuthenticationProfileResource struct {
	client *identity_services.APIClient
}

func (r *AuthenticationProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_authentication_profile"
}

func (r *AuthenticationProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AuthenticationProfilesResourceSchema
}

func (r *AuthenticationProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *AuthenticationProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for AuthenticationProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AuthenticationProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackAuthenticationProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating authentication_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.AuthenticationProfilesAPI.CreateAuthenticationProfiles(ctx).AuthenticationProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating authentication_profiles", err.Error())
	...
	packedObject, diags := packAuthenticationProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created authentication_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
...
func (r *AuthenticationProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.AuthenticationProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for AuthenticationProfile")
	var data, savestate models.AuthenticationProfiles
	...
	tflog.Debug(ctx, "Reading authentication_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.AuthenticationProfilesAPI.GetAuthenticationProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no authentication_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading authentication_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAuthenticationProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *AuthenticationProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.AuthenticationProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for AuthenticationProfile")
	var plan, state models.AuthenticationProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AuthenticationProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating authentication_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.AuthenticationProfilesAPI.UpdateAuthenticationProfilesByID(ctx, objectId).AuthenticationProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no authentication_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating authentication_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAuthenticationProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated authentication_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *AuthenticationProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting authentication_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AuthenticationProfilesAPI.DeleteAuthenticationProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting authentication_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.AuthenticationProfilesAPI.GetAuthenticationProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
