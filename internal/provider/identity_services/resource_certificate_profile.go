...
// RESOURCE for SCM CertificateProfile (Package: identity_services)
var (
...
// CertificateProfileResource defines the resource implementation.
...
func (r *CertificateProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.CertificateProfilesResourceSchema
}
...
func (r *CertificateProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for CertificateProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.CertificateProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackCertificateProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating certificate_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.CertificateProfilesAPI.CreateCertificateProfiles(ctx).CertificateProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating certificate_profiles", err.Error())
	...
	packedObject, diags := packCertificateProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created certificate_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *CertificateProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.CertificateProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for CertificateProfile")
	var data, savestate models.CertificateProfiles
	...
	tflog.Debug(ctx, "Reading certificate_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.CertificateProfilesAPI.GetCertificateProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no certificate_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading certificate_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packCertificateProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *CertificateProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.CertificateProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for CertificateProfile")
	var plan, state models.CertificateProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.CertificateProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating certificate_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.CertificateProfilesAPI.UpdateCertificateProfilesByID(ctx, objectId).CertificateProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no certificate_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating certificate_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packCertificateProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated certificate_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *CertificateProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting certificate_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.CertificateProfilesAPI.DeleteCertificateProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting certificate_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.CertificateProfilesAPI.GetCertificateProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
