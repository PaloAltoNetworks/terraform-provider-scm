...
// RESOURCE for SCM KerberosServerProfile (Package: identity_services)
var (
...
// KerberosServerProfileResource defines the resource implementation.
type KerberosServerProfileResource struct {
	client *identity_services.APIClient
}

func (r *KerberosServerProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_kerberos_server_profile"
}

func (r *KerberosServerProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.KerberosServerProfilesResourceSchema
}

func (r *KerberosServerProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *KerberosServerProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for KerberosServerProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.KerberosServerProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackKerberosServerProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating kerberos_server_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.KerberosServerProfilesAPI.CreateKerberosServerProfiles(ctx).KerberosServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating kerberos_server_profiles", err.Error())
	...
	packedObject, diags := packKerberosServerProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created kerberos_server_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
...
func (r *KerberosServerProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.KerberosServerProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for KerberosServerProfile")
	var data, savestate models.KerberosServerProfiles
	...
	tflog.Debug(ctx, "Reading kerberos_server_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.KerberosServerProfilesAPI.GetKerberosServerProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no kerberos_server_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading kerberos_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packKerberosServerProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *KerberosServerProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.KerberosServerProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for KerberosServerProfile")
	var plan, state models.KerberosServerProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.KerberosServerProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating kerberos_server_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.KerberosServerProfilesAPI.UpdateKerberosServerProfilesByID(ctx, objectId).KerberosServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no kerberos_server_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating kerberos_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packKerberosServerProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated kerberos_server_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *KerberosServerProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting kerberos_server_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.KerberosServerProfilesAPI.DeleteKerberosServerProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting kerberos_server_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.KerberosServerProfilesAPI.GetKerberosServerProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
