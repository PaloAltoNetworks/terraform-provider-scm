...
// RESOURCE for SCM LdapServerProfile (Package: identity_services)
var (
...
// LdapServerProfileResource defines the resource implementation.
...
func (r *LdapServerProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.LdapServerProfilesResourceSchema
}

func (r *LdapServerProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *LdapServerProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for LdapServerProfile")
	...
	patcher := &ldapServerProfilesSensitiveValuePatcher{}

	// Stash plaintext values from the plan.

	{ // Stash plaintext for BindPassword
		var finalVal basetypes.StringValue
		if !resp.Diagnostics.HasError() {

			// Innermost block
			finalVal = data.BindPassword

		}
		if !resp.Diagnostics.HasError() && !finalVal.IsUnknown() && !finalVal.IsNull() {
			patcher.bind_password_plaintext = finalVal
		}
	}

	// Unpack the plan to an SCM SDK object.
	planObject, diags := types.ObjectValueFrom(ctx, models.LdapServerProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackLdapServerProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating ldap_server_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.LDAPServerProfilesAPI.CreateLDAPServerProfiles(ctx).LdapServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating ldap_server_profiles", err.Error())
	...
	packedObject, diags := packLdapServerProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for BindPassword
		if !resp.Diagnostics.HasError() {

			// Innermost block
			patcher.bind_password_encrypted = data.BindPassword
			data.BindPassword = patcher.bind_password_plaintext
	...
	tflog.Debug(ctx, "Created ldap_server_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *LdapServerProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.LdapServerProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for LdapServerProfile")
	var data, savestate models.LdapServerProfiles
	...
	tflog.Debug(ctx, "Reading ldap_server_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.LDAPServerProfilesAPI.GetLDAPServerProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ldap_server_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading ldap_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	{ // Patch plaintext for BindPassword
		if !resp.Diagnostics.HasError() {
			...
			if patcher.bind_password_encrypted.Equal(data.BindPassword) {
				data.BindPassword = patcher.bind_password_plaintext
			} else {
				data.BindPassword = basetypes.NewStringNull()
			}
...
func (r *LdapServerProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.LdapServerProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for LdapServerProfile")
	var plan, state models.LdapServerProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LdapServerProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating ldap_server_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.LDAPServerProfilesAPI.UpdateLDAPServerProfiles(ctx, objectId).LdapServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ldap_server_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating ldap_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLdapServerProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for BindPassword
		if !resp.Diagnostics.HasError() {
			...
			// Innermost block
			patcher.bind_password_encrypted = plan.BindPassword
			plan.BindPassword = patcher.bind_password_plaintext
	...
	tflog.Debug(ctx, "Updated ldap_server_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *LdapServerProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting ldap_server_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.LDAPServerProfilesAPI.DeleteLDAPServerProfilesByID(ctx, objectId)
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.LDAPServerProfilesAPI.GetLDAPServerProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
