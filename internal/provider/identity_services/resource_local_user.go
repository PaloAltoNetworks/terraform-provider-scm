...
// RESOURCE for SCM LocalUser (Package: identity_services)
var (
...
func (r *LocalUserResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.LocalUsersResourceSchema
}
...
func (r *LocalUserResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for LocalUser")
	var data models.LocalUsers
	...
	patcher := &localUsersSensitiveValuePatcher{}
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LocalUsers{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackLocalUsersToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating local_users on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.LocalUsersAPI.CreateLocalUsers(ctx).LocalUsers(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating local_users", err.Error())
	...
	packedObject, diags := packLocalUsersFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created local_users", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *LocalUserResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.LocalUsers - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for LocalUser")
	var data, savestate models.LocalUsers
	...
	tflog.Debug(ctx, "Reading local_users from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.LocalUsersAPI.GetLocalUsersByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no local_users on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading local_users", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLocalUsersFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.LocalUsers{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *LocalUserResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.LocalUsers which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for LocalUser")
	var plan, state models.LocalUsers
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LocalUsers{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating local_users on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.LocalUsersAPI.UpdateLocalUsersByID(ctx, objectId).LocalUsers(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no local_users on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating local_users", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	tflog.Debug(ctx, "Updated local_users", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
