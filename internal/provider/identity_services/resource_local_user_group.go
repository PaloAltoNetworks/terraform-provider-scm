...
// RESOURCE for SCM LocalUserGroup (Package: identity_services)
var (
	_ resource.Resource                = &LocalUserGroupResource{}
	_ resource.ResourceWithConfigure   = &LocalUserGroupResource{}
	_ resource.ResourceWithImportState = &LocalUserGroupResource{}
)

func NewLocalUserGroupResource() resource.Resource {
	return &LocalUserGroupResource{}
}

// LocalUserGroupResource defines the resource implementation.
type LocalUserGroupResource struct {
	client *identity_services.APIClient
}

func (r *LocalUserGroupResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_local_user_group"
}

func (r *LocalUserGroupResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.LocalUserGroupsResourceSchema
}

func (r *LocalUserGroupResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *LocalUserGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for LocalUserGroup")
	var data models.LocalUserGroups
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LocalUserGroups{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackLocalUserGroupsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating local_user_groups on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.LocalUserGroupsAPI.CreateLocalUserGroups(ctx).LocalUserGroups(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating local_user_groups", err.Error())
	...
	packedObject, diags := packLocalUserGroupsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created local_user_groups", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *LocalUserGroupResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.LocalUserGroups - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for LocalUserGroup")
	var data, savestate models.LocalUserGroups
	...
	tflog.Debug(ctx, "Reading local_user_groups from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.LocalUserGroupsAPI.GetLocalUserGroupsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no local_user_groups on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading local_user_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLocalUserGroupsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.LocalUserGroups{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *LocalUserGroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.LocalUserGroups which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for LocalUserGroup")
	var plan, state models.LocalUserGroups
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LocalUserGroups{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating local_user_groups on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.LocalUserGroupsAPI.UpdateLocalUserGroupsByID(ctx, objectId).LocalUserGroups(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no local_user_groups on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating local_user_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLocalUserGroupsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated local_user_groups", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *LocalUserGroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.LocalUserGroups
	...
	tflog.Debug(ctx, "Deleting local_user_groups", map[string]interface{}{"id": objectId})
	deleteReq := r.client.LocalUserGroupsAPI.DeleteLocalUserGroupsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting local_user_groups", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *LocalUserGroupResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
