...
// RESOURCE for SCM RadiusServerProfile (Package: identity_services)
var (
	_ resource.Resource                = &RadiusServerProfileResource{}
	_ resource.ResourceWithConfigure   = &RadiusServerProfileResource{}
	_ resource.ResourceWithImportState = &RadiusServerProfileResource{}
)
...
// RadiusServerProfileResource defines the resource implementation.
...
func (r *RadiusServerProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.RadiusServerProfilesResourceSchema
}

func (r *RadiusServerProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *RadiusServerProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for RadiusServerProfile")
	...
	unpackedScmObject, diags := unpackRadiusServerProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating radius_server_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.RADIUSServerProfilesAPI.CreateRADIUSServerProfiles(ctx).RadiusServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating radius_server_profiles", err.Error())
	...
	packedObject, diags := packRadiusServerProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created radius_server_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *RadiusServerProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.RadiusServerProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for RadiusServerProfile")
	var data, savestate models.RadiusServerProfiles
	...
	tflog.Debug(ctx, "Reading radius_server_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.RADIUSServerProfilesAPI.GetRADIUSServerProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no radius_server_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading radius_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.RadiusServerProfiles{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *RadiusServerProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.RadiusServerProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for RadiusServerProfile")
	var plan, state models.RadiusServerProfiles
	...
	tflog.Debug(ctx, "Updating radius_server_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.RADIUSServerProfilesAPI.UpdateRADIUSServerProfilesByID(ctx, objectId).RadiusServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no radius_server_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating radius_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packRadiusServerProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated radius_server_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *RadiusServerProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
