...
// RESOURCE for SCM TacacsServerProfile (Package: identity_services)
var (
	_ resource.Resource                = &TacacsServerProfileResource{}
	_ resource.ResourceWithConfigure   = &TacacsServerProfileResource{}
	_ resource.ResourceWithImportState = &TacacsServerProfileResource{}
)
...
// TacacsServerProfileResource defines the resource implementation.
type TacacsServerProfileResource struct {
	client *identity_services.APIClient
}

func (r *TacacsServerProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_tacacs_server_profile"
}

func (r *TacacsServerProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.TacacsServerProfilesResourceSchema
}

func (r *TacacsServerProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *TacacsServerProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for TacacsServerProfile")
	...
	patcher := &tacacsServerProfilesSensitiveValuePatcher{}
	...
	{ // Stash plaintext for array field Secret
		if !resp.Diagnostics.HasError() && !data.Server.IsNull() && !data.Server.IsUnknown() {
			var arrayItems []models.TacacsServerProfilesServerInner
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TacacsServerProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackTacacsServerProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating tacacs_server_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.TACACSServerProfilesAPI.CreateTACACSServerProfiles(ctx).TacacsServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating tacacs_server_profiles", err.Error())
	...
	packedObject, diags := packTacacsServerProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for array field Secret
		if !resp.Diagnostics.HasError() && !data.Server.IsNull() && !data.Server.IsUnknown() {
			...
			if !resp.Diagnostics.HasError() {
				...
				data.Server, diags = basetypes.NewListValueFrom(ctx, models.TacacsServerProfilesServerInner{}.AttrType(), arrayItems)
				resp.Diagnostics.Append(diags...)
			}
		}
	}
	...
	tflog.Debug(ctx, "Created tacacs_server_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *TacacsServerProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.TacacsServerProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for TacacsServerProfile")
	var data, savestate models.TacacsServerProfiles
	...
	tflog.Debug(ctx, "Reading tacacs_server_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.TACACSServerProfilesAPI.GetTACACSServerProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tacacs_server_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading tacacs_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTacacsServerProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *TacacsServerProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.TacacsServerProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for TacacsServerProfile")
	var plan, state models.TacacsServerProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TacacsServerProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating tacacs_server_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.TACACSServerProfilesAPI.UpdateTACACSServerProfilesByID(ctx, objectId).TacacsServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tacacs_server_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating tacacs_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTacacsServerProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for array field Secret
		if !resp.Diagnostics.HasError() && !plan.Server.IsNull() && !plan.Server.IsUnknown() {
			...
			if !resp.Diagnostics.HasError() {
				...
				plan.Server, diags = basetypes.NewListValueFrom(ctx, models.TacacsServerProfilesServerInner{}.AttrType(), arrayItems)
				resp.Diagnostics.Append(diags...)
			}
		}
	}
	...
	tflog.Debug(ctx, "Updated tacacs_server_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *TacacsServerProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting tacacs_server_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.TACACSServerProfilesAPI.DeleteTACACSServerProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting tacacs_server_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.TACACSServerProfilesAPI.GetTACACSServerProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *TacacsServerProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
