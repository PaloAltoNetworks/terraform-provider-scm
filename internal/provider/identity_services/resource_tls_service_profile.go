...
// RESOURCE for SCM TlsServiceProfile (Package: identity_services)
var (
...
// TlsServiceProfileResource defines the resource implementation.
type TlsServiceProfileResource struct {
	client *identity_services.APIClient
}

func (r *TlsServiceProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_tls_service_profile"
}

func (r *TlsServiceProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.TlsServiceProfilesResourceSchema
}

func (r *TlsServiceProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *TlsServiceProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for TlsServiceProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TlsServiceProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackTlsServiceProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating tls_service_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.TLSServiceProfilesAPI.CreateTLSServiceProfiles(ctx).TlsServiceProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating tls_service_profiles", err.Error())
	...
	packedObject, diags := packTlsServiceProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created tls_service_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *TlsServiceProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.TlsServiceProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for TlsServiceProfile")
	var data, savestate models.TlsServiceProfiles
	...
	tflog.Debug(ctx, "Reading tls_service_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.TLSServiceProfilesAPI.GetTLSServiceProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tls_service_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading tls_service_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTlsServiceProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *TlsServiceProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.TlsServiceProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for TlsServiceProfile")
	var plan, state models.TlsServiceProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TlsServiceProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating tls_service_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.TLSServiceProfilesAPI.UpdateTLSServiceProfilesByID(ctx, objectId).TlsServiceProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tls_service_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating tls_service_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTlsServiceProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated tls_service_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *TlsServiceProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting tls_service_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.TLSServiceProfilesAPI.DeleteTLSServiceProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting tls_service_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.TLSServiceProfilesAPI.GetTLSServiceProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
