...
// DATA SOURCE for SCM ForwardingProfile (Package: mobile_agent)
var (
...
// ForwardingProfileDataSource defines the data source implementation.
...
func (d *ForwardingProfileDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: ForwardingProfileDataSource.Metadata ---")
...
func (d *ForwardingProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ForwardingProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ForwardingProfilesDataSourceSchema
}
...
func (d *ForwardingProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ForwardingProfilesDataSource.Configure ---")
...
func (d *ForwardingProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ForwardingProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ForwardingProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ForwardingProfilesAPI.GetGlobalProtectForwardingProfileByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ForwardingProfiles", fmt.Sprintf("Could not read ForwardingProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading ForwardingProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ForwardingProfilesAPI.ListGlobalProtectForwardingProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing ForwardingProfiless", fmt.Sprintf("Could not list ForwardingProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *mobile_agent.ForwardingProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ForwardingProfiles Not Found", fmt.Sprintf("No ForwardingProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packForwardingProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ForwardingProfiles data source.")
		return
	}
