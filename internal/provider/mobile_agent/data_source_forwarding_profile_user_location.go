...
// DATA SOURCE for SCM ForwardingProfileUserLocation (Package: mobile_agent)
var (
	_ datasource.DataSource              = &ForwardingProfileUserLocationDataSource{}
	_ datasource.DataSourceWithConfigure = &ForwardingProfileUserLocationDataSource{}
)

func NewForwardingProfileUserLocationDataSource() datasource.DataSource {
	return &ForwardingProfileUserLocationDataSource{}
}

// ForwardingProfileUserLocationDataSource defines the data source implementation.
type ForwardingProfileUserLocationDataSource struct {
...
func (d *ForwardingProfileUserLocationDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ForwardingProfileUserLocationDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ForwardingProfileUserLocationsDataSourceSchema
}

func (d *ForwardingProfileUserLocationDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ForwardingProfileUserLocationsDataSource.Configure ---")
...
func (d *ForwardingProfileUserLocationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ForwardingProfileUserLocationDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ForwardingProfileUserLocations data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.UserLocationsAPI.GetGlobalProtectUserLocationByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ForwardingProfileUserLocations", fmt.Sprintf("Could not read ForwardingProfileUserLocations with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading ForwardingProfileUserLocations data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.UserLocationsAPI.ListGlobalProtectUserLocations(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing ForwardingProfileUserLocationss", fmt.Sprintf("Could not list ForwardingProfileUserLocationss: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *mobile_agent.ForwardingProfileUserLocations
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ForwardingProfileUserLocations Not Found", fmt.Sprintf("No ForwardingProfileUserLocations found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packForwardingProfileUserLocationsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ForwardingProfileUserLocations data source.")
		return
	}
