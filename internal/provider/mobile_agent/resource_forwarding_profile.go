...
// RESOURCE for SCM ForwardingProfile (Package: mobile_agent)
var (
...
// ForwardingProfileResource defines the resource implementation.
...
func (r *ForwardingProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ForwardingProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackForwardingProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating forwarding_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ForwardingProfilesAPI.CreateGlobalProtectForwardingProfile(ctx).ForwardingProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating forwarding_profiles", err.Error())
	...
	packedObject, diags := packForwardingProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created forwarding_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ForwardingProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ForwardingProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ForwardingProfile")
	var data, savestate models.ForwardingProfiles
	...
	tflog.Debug(ctx, "Reading forwarding_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ForwardingProfilesAPI.GetGlobalProtectForwardingProfileByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading forwarding_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packForwardingProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ForwardingProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.ForwardingProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ForwardingProfile")
	var plan, state models.ForwardingProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating forwarding_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ForwardingProfilesAPI.UpdateGlobalProtectForwardingProfileByID(ctx, objectId).ForwardingProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating forwarding_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packForwardingProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated forwarding_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *ForwardingProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting forwarding_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ForwardingProfilesAPI.DeleteGlobalProtectForwardingProfile(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting forwarding_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.ForwardingProfilesAPI.GetGlobalProtectForwardingProfileByID(ctx, objectId).Execute()
			if getErr != nil {
