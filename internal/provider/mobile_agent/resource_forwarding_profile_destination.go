...
// RESOURCE for SCM ForwardingProfileDestination (Package: mobile_agent)
var (
	_ resource.Resource                = &ForwardingProfileDestinationResource{}
	_ resource.ResourceWithConfigure   = &ForwardingProfileDestinationResource{}
	_ resource.ResourceWithImportState = &ForwardingProfileDestinationResource{}
)

func NewForwardingProfileDestinationResource() resource.Resource {
	return &ForwardingProfileDestinationResource{}
}

// ForwardingProfileDestinationResource defines the resource implementation.
type ForwardingProfileDestinationResource struct {
	client *mobile_agent.APIClient
}

func (r *ForwardingProfileDestinationResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_forwarding_profile_destination"
}

func (r *ForwardingProfileDestinationResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ForwardingProfileDestinationsResourceSchema
}

func (r *ForwardingProfileDestinationResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *ForwardingProfileDestinationResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ForwardingProfileDestination")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileDestinations{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackForwardingProfileDestinationsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating forwarding_profile_destinations on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DestinationsAPI.CreateGlobalProtectDestination(ctx).ForwardingProfileDestinations(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating forwarding_profile_destinations", err.Error())
	...
	packedObject, diags := packForwardingProfileDestinationsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created forwarding_profile_destinations", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ForwardingProfileDestinationResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ForwardingProfileDestinations - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ForwardingProfileDestination")
	var data, savestate models.ForwardingProfileDestinations
	...
	tflog.Debug(ctx, "Reading forwarding_profile_destinations from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DestinationsAPI.GetGlobalProtectDestinationByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profile_destinations on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading forwarding_profile_destinations", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packForwardingProfileDestinationsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileDestinations{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ForwardingProfileDestinationResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.ForwardingProfileDestinations which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ForwardingProfileDestination")
	var plan, state models.ForwardingProfileDestinations
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileDestinations{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating forwarding_profile_destinations on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DestinationsAPI.UpdateGlobalProtectDestinationByID(ctx, objectId).ForwardingProfileDestinations(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profile_destinations on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating forwarding_profile_destinations", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packForwardingProfileDestinationsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated forwarding_profile_destinations", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *ForwardingProfileDestinationResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.ForwardingProfileDestinations
	...
	tflog.Debug(ctx, "Deleting forwarding_profile_destinations", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DestinationsAPI.DeleteGlobalProtectDestination(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting forwarding_profile_destinations", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *ForwardingProfileDestinationResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
