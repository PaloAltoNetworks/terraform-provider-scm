...
// RESOURCE for SCM ForwardingProfileRegionalAndCustomProxy (Package: mobile_agent)
var (
	_ resource.Resource                = &ForwardingProfileRegionalAndCustomProxyResource{}
	_ resource.ResourceWithConfigure   = &ForwardingProfileRegionalAndCustomProxyResource{}
	_ resource.ResourceWithImportState = &ForwardingProfileRegionalAndCustomProxyResource{}
)

func NewForwardingProfileRegionalAndCustomProxyResource() resource.Resource {
	return &ForwardingProfileRegionalAndCustomProxyResource{}
}

// ForwardingProfileRegionalAndCustomProxyResource defines the resource implementation.
type ForwardingProfileRegionalAndCustomProxyResource struct {
...
func (r *ForwardingProfileRegionalAndCustomProxyResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ForwardingProfileRegionalAndCustomProxiesResourceSchema
}

func (r *ForwardingProfileRegionalAndCustomProxyResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *ForwardingProfileRegionalAndCustomProxyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ForwardingProfileRegionalAndCustomProxy")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileRegionalAndCustomProxies{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackForwardingProfileRegionalAndCustomProxiesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating forwarding_profile_regional_and_custom_proxies on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.RegionalAndCustomProxiesAPI.CreateGlobalProtectRegionalAndCustomProxies(ctx).ForwardingProfileRegionalAndCustomProxies(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating forwarding_profile_regional_and_custom_proxies", err.Error())
	...
	packedObject, diags := packForwardingProfileRegionalAndCustomProxiesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created forwarding_profile_regional_and_custom_proxies", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ForwardingProfileRegionalAndCustomProxyResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ForwardingProfileRegionalAndCustomProxies - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ForwardingProfileRegionalAndCustomProxy")
	var data, savestate models.ForwardingProfileRegionalAndCustomProxies
	...
	tflog.Debug(ctx, "Reading forwarding_profile_regional_and_custom_proxies from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.RegionalAndCustomProxiesAPI.GetGlobalProtectRegionalAndCustomProxyByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profile_regional_and_custom_proxies on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading forwarding_profile_regional_and_custom_proxies", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packForwardingProfileRegionalAndCustomProxiesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileRegionalAndCustomProxies{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ForwardingProfileRegionalAndCustomProxyResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.ForwardingProfileRegionalAndCustomProxies which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ForwardingProfileRegionalAndCustomProxy")
	var plan, state models.ForwardingProfileRegionalAndCustomProxies
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileRegionalAndCustomProxies{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating forwarding_profile_regional_and_custom_proxies on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.RegionalAndCustomProxiesAPI.UpdateGlobalProtectRegionalAndCustomProxyByID(ctx, objectId).ForwardingProfileRegionalAndCustomProxies(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profile_regional_and_custom_proxies on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating forwarding_profile_regional_and_custom_proxies", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packForwardingProfileRegionalAndCustomProxiesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated forwarding_profile_regional_and_custom_proxies", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *ForwardingProfileRegionalAndCustomProxyResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.ForwardingProfileRegionalAndCustomProxies
...
func (r *ForwardingProfileRegionalAndCustomProxyResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if len(tokens) > 0 && tokens[0] != "" {
		...
		if folderValue == "All Firewalls" {
			resp.Diagnostics.AddError(
				...
				"The folder value 'All Firewalls' is not allowed in import IDs. Please use 'ngfw-shared' instead.\nExample: terraform import scm_forwarding_profile_regional_and_custom_proxy.example \"ngfw-shared\":::\"<id>\"",
			)
			return
		}
		if folderValue == "Global" {
			resp.Diagnostics.AddError(
				"Invalid Folder Value in Import ID",
				"The folder value 'Global' is not allowed in import IDs. Please use 'All' instead.\nExample: terraform import scm_forwarding_profile_regional_and_custom_proxy.example \"All\":::\"<id>\"",
			)
			return
		}
		if folderValue == "Explicit Proxy" {
			resp.Diagnostics.AddError(
				"Invalid Folder Value in Import ID",
				"The folder value 'Explicit Proxy' is not allowed in import IDs. Please use 'Mobile Users Explicit Proxy' instead.\nExample: terraform import scm_forwarding_profile_regional_and_custom_proxy.example \"Mobile Users Explicit Proxy\":::\"<id>\"",
			)
			return
		}
		if folderValue == "Access Agent" {
			resp.Diagnostics.AddError(
				"Invalid Folder Value in Import ID",
				"The folder value 'Access Agent' is not allowed in import IDs. Please use 'Mobile Users' instead.\nExample: terraform import scm_forwarding_profile_regional_and_custom_proxy.example \"Mobile Users\":::\"<id>\"",
			)
			return
		}
	}
