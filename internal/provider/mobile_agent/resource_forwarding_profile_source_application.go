...
// RESOURCE for SCM ForwardingProfileSourceApplication (Package: mobile_agent)
var (
	_ resource.Resource                = &ForwardingProfileSourceApplicationResource{}
	_ resource.ResourceWithConfigure   = &ForwardingProfileSourceApplicationResource{}
	_ resource.ResourceWithImportState = &ForwardingProfileSourceApplicationResource{}
)

func NewForwardingProfileSourceApplicationResource() resource.Resource {
	return &ForwardingProfileSourceApplicationResource{}
}

// ForwardingProfileSourceApplicationResource defines the resource implementation.
type ForwardingProfileSourceApplicationResource struct {
	client *mobile_agent.APIClient
}

func (r *ForwardingProfileSourceApplicationResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_forwarding_profile_source_application"
}

func (r *ForwardingProfileSourceApplicationResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ForwardingProfileSourceApplicationsResourceSchema
}

func (r *ForwardingProfileSourceApplicationResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *ForwardingProfileSourceApplicationResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ForwardingProfileSourceApplication")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileSourceApplications{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackForwardingProfileSourceApplicationsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating forwarding_profile_source_applications on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SourceApplicationsAPI.CreateGlobalProtectSourceApplication(ctx).ForwardingProfileSourceApplications(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating forwarding_profile_source_applications", err.Error())
	...
	packedObject, diags := packForwardingProfileSourceApplicationsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created forwarding_profile_source_applications", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ForwardingProfileSourceApplicationResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ForwardingProfileSourceApplications - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ForwardingProfileSourceApplication")
	var data, savestate models.ForwardingProfileSourceApplications
	...
	tflog.Debug(ctx, "Reading forwarding_profile_source_applications from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SourceApplicationsAPI.GetGlobalProtectSourceApplicationByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profile_source_applications on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading forwarding_profile_source_applications", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileSourceApplications{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ForwardingProfileSourceApplicationResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.ForwardingProfileSourceApplications which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ForwardingProfileSourceApplication")
	var plan, state models.ForwardingProfileSourceApplications
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ForwardingProfileSourceApplications{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating forwarding_profile_source_applications on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SourceApplicationsAPI.UpdateGlobalProtectSourceApplicationByID(ctx, objectId).ForwardingProfileSourceApplications(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no forwarding_profile_source_applications on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating forwarding_profile_source_applications", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packForwardingProfileSourceApplicationsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	// Restore parameter folder from plan
	_ = req.Plan.GetAttribute(ctx, path.Root("folder"), &plan.Folder)

	tflog.Debug(ctx, "Updated forwarding_profile_source_applications", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *ForwardingProfileSourceApplicationResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.ForwardingProfileSourceApplications
	...
	tflog.Debug(ctx, "Deleting forwarding_profile_source_applications", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SourceApplicationsAPI.DeleteGlobalProtectSourceApplication(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting forwarding_profile_source_applications", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *ForwardingProfileSourceApplicationResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
	...
	if len(tokens) > 0 && tokens[0] != "" {
		...
		if folderValue == "Global" {
			resp.Diagnostics.AddError(
				"Invalid Folder Value in Import ID",
				"The folder value 'Global' is not allowed in import IDs. Please use 'All' instead.\nExample: terraform import scm_forwarding_profile_source_application.example \"All\":::\"<id>\"",
			)
			return
		}
		...
		if folderValue == "Access Agent" {
			resp.Diagnostics.AddError(
				"Invalid Folder Value in Import ID",
				"The folder value 'Access Agent' is not allowed in import IDs. Please use 'Mobile Users' instead.\nExample: terraform import scm_forwarding_profile_source_application.example \"Mobile Users\":::\"<id>\"",
			)
			return
		}
	}
