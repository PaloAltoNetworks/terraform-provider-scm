...
var (
	_ datasource.DataSource              = &AutoVpnClusterDataSource{}
	_ datasource.DataSourceWithConfigure = &AutoVpnClusterDataSource{}
)

func NewAutoVpnClusterDataSource() datasource.DataSource {
	return &AutoVpnClusterDataSource{}
}

// AutoVpnClusterDataSource defines the data source implementation.
type AutoVpnClusterDataSource struct {
...
func (d *AutoVpnClusterDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: AutoVpnClusterDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.AutoVpnClustersDataSourceSchema
}

func (d *AutoVpnClusterDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: AutoVpnClustersDataSource.Configure ---")
...
func (d *AutoVpnClusterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: AutoVpnClusterDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.AutoVPNClustersAPI.GetAutoVPNClustersByID(ctx, objectId)
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("AutoVpnClusters Not Found", fmt.Sprintf("No AutoVpnClusters found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packAutoVpnClustersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
