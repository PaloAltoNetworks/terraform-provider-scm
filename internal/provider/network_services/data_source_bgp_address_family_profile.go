...
// DATA SOURCE for SCM BgpAddressFamilyProfile (Package: network_services)
var (
	_ datasource.DataSource              = &BgpAddressFamilyProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &BgpAddressFamilyProfileDataSource{}
)

func NewBgpAddressFamilyProfileDataSource() datasource.DataSource {
	return &BgpAddressFamilyProfileDataSource{}
}

// BgpAddressFamilyProfileDataSource defines the data source implementation.
type BgpAddressFamilyProfileDataSource struct {
...
func (d *BgpAddressFamilyProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpAddressFamilyProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.BgpAddressFamilyProfilesDataSourceSchema
}

func (d *BgpAddressFamilyProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpAddressFamilyProfilesDataSource.Configure ---")
...
func (d *BgpAddressFamilyProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpAddressFamilyProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading BgpAddressFamilyProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.BGPAddressFamilyProfilesAPI.GetBGPAddressFamilyProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading BgpAddressFamilyProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.BGPAddressFamilyProfilesAPI.ListBGPAddressFamilyProfiles(ctx)
		...
		var foundObject *network_services.BgpAddressFamilyProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("BgpAddressFamilyProfiles Not Found", fmt.Sprintf("No BgpAddressFamilyProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packBgpAddressFamilyProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the BgpAddressFamilyProfiles data source.")
		return
	}
