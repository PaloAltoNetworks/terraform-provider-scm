...
// DATA SOURCE for SCM BgpAuthProfile (Package: network_services)
var (
	_ datasource.DataSource              = &BgpAuthProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &BgpAuthProfileDataSource{}
)

func NewBgpAuthProfileDataSource() datasource.DataSource {
	return &BgpAuthProfileDataSource{}
}

// BgpAuthProfileDataSource defines the data source implementation.
type BgpAuthProfileDataSource struct {
...
func (d *BgpAuthProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpAuthProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.BgpAuthProfilesDataSourceSchema
}

func (d *BgpAuthProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpAuthProfilesDataSource.Configure ---")
...
func (d *BgpAuthProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpAuthProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading BgpAuthProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.BGPAuthenticationProfilesAPI.GetBGPAuthenticationProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading BgpAuthProfiles", fmt.Sprintf("Could not read BgpAuthProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading BgpAuthProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.BGPAuthenticationProfilesAPI.ListBGPAuthenticationProfiles(ctx)
		...
		var foundObject *network_services.BgpAuthProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("BgpAuthProfiles Not Found", fmt.Sprintf("No BgpAuthProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packBgpAuthProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the BgpAuthProfiles data source.")
		return
	}
