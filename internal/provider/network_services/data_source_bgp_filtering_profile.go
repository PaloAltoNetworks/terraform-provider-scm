...
// DATA SOURCE for SCM BgpFilteringProfile (Package: network_services)
var (
	_ datasource.DataSource              = &BgpFilteringProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &BgpFilteringProfileDataSource{}
)

func NewBgpFilteringProfileDataSource() datasource.DataSource {
	return &BgpFilteringProfileDataSource{}
}

// BgpFilteringProfileDataSource defines the data source implementation.
type BgpFilteringProfileDataSource struct {
...
func (d *BgpFilteringProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpFilteringProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.BgpFilteringProfilesDataSourceSchema
}

func (d *BgpFilteringProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpFilteringProfilesDataSource.Configure ---")
...
func (d *BgpFilteringProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpFilteringProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading BgpFilteringProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.BGPFilteringProfilesAPI.GetBGPFilteringProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading BgpFilteringProfiles", fmt.Sprintf("Could not read BgpFilteringProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading BgpFilteringProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.BGPFilteringProfilesAPI.ListBGPFilteringProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing BgpFilteringProfiless", fmt.Sprintf("Could not list BgpFilteringProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.BgpFilteringProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("BgpFilteringProfiles Not Found", fmt.Sprintf("No BgpFilteringProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packBgpFilteringProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the BgpFilteringProfiles data source.")
		return
	}
