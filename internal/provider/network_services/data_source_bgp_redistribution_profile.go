...
// DATA SOURCE for SCM BgpRedistributionProfile (Package: network_services)
var (
	_ datasource.DataSource              = &BgpRedistributionProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &BgpRedistributionProfileDataSource{}
)

func NewBgpRedistributionProfileDataSource() datasource.DataSource {
	return &BgpRedistributionProfileDataSource{}
}

// BgpRedistributionProfileDataSource defines the data source implementation.
type BgpRedistributionProfileDataSource struct {
...
func (d *BgpRedistributionProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpRedistributionProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.BgpRedistributionProfilesDataSourceSchema
}

func (d *BgpRedistributionProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpRedistributionProfilesDataSource.Configure ---")
...
func (d *BgpRedistributionProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpRedistributionProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.BGPRedistributionProfilesAPI.GetBGPRedistributionProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading BgpRedistributionProfiles", fmt.Sprintf("Could not read BgpRedistributionProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading BgpRedistributionProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.BGPRedistributionProfilesAPI.ListBGPRedistributionProfiles(ctx)
		...
		var foundObject *network_services.BgpRedistributionProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("BgpRedistributionProfiles Not Found", fmt.Sprintf("No BgpRedistributionProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packBgpRedistributionProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the BgpRedistributionProfiles data source.")
		return
	}
