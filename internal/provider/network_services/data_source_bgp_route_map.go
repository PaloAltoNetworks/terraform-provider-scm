...
// DATA SOURCE for SCM BgpRouteMap (Package: network_services)
var (
	_ datasource.DataSource              = &BgpRouteMapDataSource{}
	_ datasource.DataSourceWithConfigure = &BgpRouteMapDataSource{}
)
...
func (d *BgpRouteMapDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpRouteMapDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.BgpRouteMapsDataSourceSchema
}
...
func (d *BgpRouteMapDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpRouteMapsDataSource.Configure ---")
...
func (d *BgpRouteMapDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: BgpRouteMapDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading BgpRouteMaps data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.BGPRouteMapsAPI.GetBGPRouteMapsByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading BgpRouteMaps data source by Name", map[string]interface{}{"name": objectName})
		...
		var foundObject *network_services.BgpRouteMaps
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("BgpRouteMaps Not Found", fmt.Sprintf("No BgpRouteMaps found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packBgpRouteMapsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the BgpRouteMaps data source.")
		return
	}
