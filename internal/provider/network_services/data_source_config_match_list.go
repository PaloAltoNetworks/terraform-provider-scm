...
// DATA SOURCE for SCM ConfigMatchList (Package: network_services)
var (
	_ datasource.DataSource              = &ConfigMatchListDataSource{}
	_ datasource.DataSourceWithConfigure = &ConfigMatchListDataSource{}
)

func NewConfigMatchListDataSource() datasource.DataSource {
	return &ConfigMatchListDataSource{}
}

// ConfigMatchListDataSource defines the data source implementation.
type ConfigMatchListDataSource struct {
...
func (d *ConfigMatchListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ConfigMatchListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ConfigMatchListDataSourceSchema
}

func (d *ConfigMatchListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ConfigMatchListDataSource.Configure ---")
...
func (d *ConfigMatchListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ConfigMatchListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ConfigMatchList data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ConfigMatchListAPI.GetConfigMatchListByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ConfigMatchList", fmt.Sprintf("Could not read ConfigMatchList with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading ConfigMatchList data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ConfigMatchListAPI.ListConfigMatchList(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing ConfigMatchLists", fmt.Sprintf("Could not list ConfigMatchLists: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.ConfigMatchList
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ConfigMatchList Not Found", fmt.Sprintf("No ConfigMatchList found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packConfigMatchListFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ConfigMatchList data source.")
		return
	}
