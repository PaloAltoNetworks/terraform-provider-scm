...
// DATA SOURCE for SCM DhcpInterface (Package: network_services)
var (
	_ datasource.DataSource              = &DhcpInterfaceDataSource{}
	_ datasource.DataSourceWithConfigure = &DhcpInterfaceDataSource{}
)

func NewDhcpInterfaceDataSource() datasource.DataSource {
	return &DhcpInterfaceDataSource{}
}

// DhcpInterfaceDataSource defines the data source implementation.
type DhcpInterfaceDataSource struct {
...
func (d *DhcpInterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DhcpInterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DhcpInterfacesDataSourceSchema
}

func (d *DhcpInterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DhcpInterfacesDataSource.Configure ---")
...
func (d *DhcpInterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DhcpInterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DhcpInterfaces data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DHCPInterfacesAPI.GetDHCPInterfacesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading DhcpInterfaces data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DHCPInterfacesAPI.ListDHCPInterfaces(ctx)
		...
		var foundObject *network_services.DhcpInterfaces
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DhcpInterfaces Not Found", fmt.Sprintf("No DhcpInterfaces found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDhcpInterfacesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DhcpInterfaces data source.")
		return
	}
