...
// DATA SOURCE for SCM DnsProxy (Package: network_services)
var (
...
// DnsProxyDataSource defines the data source implementation.
...
func (d *DnsProxyDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: DnsProxyDataSource.Metadata ---")
...
func (d *DnsProxyDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DnsProxyDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DnsProxiesDataSourceSchema
}
...
func (d *DnsProxyDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DnsProxiesDataSource.Configure ---")
...
func (d *DnsProxyDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DnsProxyDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DnsProxies data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DNSProxiesAPI.GetDNSProxiesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading DnsProxies data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DNSProxiesAPI.ListDNSProxies(ctx)
		...
		var foundObject *network_services.DnsProxies
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DnsProxies Not Found", fmt.Sprintf("No DnsProxies found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDnsProxiesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DnsProxies data source.")
		return
	}
