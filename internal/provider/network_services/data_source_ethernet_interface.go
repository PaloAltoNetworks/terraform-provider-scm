...
// DATA SOURCE for SCM EthernetInterface (Package: network_services)
var (
...
// EthernetInterfaceDataSource defines the data source implementation.
...
func (d *EthernetInterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: EthernetInterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.EthernetInterfacesDataSourceSchema
}

func (d *EthernetInterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: EthernetInterfacesDataSource.Configure ---")
...
func (d *EthernetInterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: EthernetInterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading EthernetInterfaces data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.EthernetInterfacesAPI.GetEthernetInterfacesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading EthernetInterfaces", fmt.Sprintf("Could not read EthernetInterfaces with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading EthernetInterfaces data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.EthernetInterfacesAPI.ListEthernetInterfaces(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing EthernetInterfacess", fmt.Sprintf("Could not list EthernetInterfacess: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.EthernetInterfaces
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("EthernetInterfaces Not Found", fmt.Sprintf("No EthernetInterfaces found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packEthernetInterfacesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the EthernetInterfaces data source.")
		return
	}
