...
// DATA SOURCE for SCM HipmatchMatchList (Package: network_services)
var (
	_ datasource.DataSource              = &HipmatchMatchListDataSource{}
	_ datasource.DataSourceWithConfigure = &HipmatchMatchListDataSource{}
)

func NewHipmatchMatchListDataSource() datasource.DataSource {
	return &HipmatchMatchListDataSource{}
}

// HipmatchMatchListDataSource defines the data source implementation.
type HipmatchMatchListDataSource struct {
...
func (d *HipmatchMatchListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: HipmatchMatchListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.HipmatchMatchListDataSourceSchema
}

func (d *HipmatchMatchListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: HipmatchMatchListDataSource.Configure ---")
...
func (d *HipmatchMatchListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: HipmatchMatchListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.HipmatchMatchListAPI.GetHipmatchMatchListByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading HipmatchMatchList", fmt.Sprintf("Could not read HipmatchMatchList with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading HipmatchMatchList data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.HipmatchMatchListAPI.ListHipmatchMatchList(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing HipmatchMatchLists", fmt.Sprintf("Could not list HipmatchMatchLists: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.HipmatchMatchList
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("HipmatchMatchList Not Found", fmt.Sprintf("No HipmatchMatchList found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packHipmatchMatchListFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the HipmatchMatchList data source.")
		return
	}
