...
// DATA SOURCE for SCM IkeCryptoProfile (Package: network_services)
var (
...
// IkeCryptoProfileDataSource defines the data source implementation.
type IkeCryptoProfileDataSource struct {
	client *network_services.APIClient
}

func (d *IkeCryptoProfileDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: IkeCryptoProfileDataSource.Metadata ---")
	resp.TypeName = "scm_ike_crypto_profile"
}

func (d *IkeCryptoProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: IkeCryptoProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.IkeCryptoProfilesDataSourceSchema
}

func (d *IkeCryptoProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: IkeCryptoProfilesDataSource.Configure ---")
...
func (d *IkeCryptoProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: IkeCryptoProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		objectId := data.Id.ValueString()
		tflog.Debug(ctx, "Reading IkeCryptoProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.IKECryptoProfilesAPI.GetIKECryptoProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading IkeCryptoProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.IKECryptoProfilesAPI.ListIKECryptoProfiles(ctx)
		...
		var foundObject *network_services.IkeCryptoProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("IkeCryptoProfiles Not Found", fmt.Sprintf("No IkeCryptoProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packIkeCryptoProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the IkeCryptoProfiles data source.")
		return
	}
