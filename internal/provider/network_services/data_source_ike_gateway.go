...
// DATA SOURCE for SCM IkeGateway (Package: network_services)
var (
...
// IkeGatewayDataSource defines the data source implementation.
...
func (d *IkeGatewayDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: IkeGatewayDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.IkeGatewaysDataSourceSchema
}

func (d *IkeGatewayDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: IkeGatewaysDataSource.Configure ---")
...
func (d *IkeGatewayDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: IkeGatewayDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading IkeGateways data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.IKEGatewaysAPI.GetIKEGatewaysByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading IkeGateways", fmt.Sprintf("Could not read IkeGateways with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading IkeGateways data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.IKEGatewaysAPI.ListIKEGateways(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing IkeGatewayss", fmt.Sprintf("Could not list IkeGatewayss: %s", err.Error()))
			return
		}
		if httpRes.StatusCode != 200 {
			resp.Diagnostics.AddError("Unexpected HTTP status code", fmt.Sprintf("Expected 200, got %d", httpRes.StatusCode))
		...
		var foundObject *network_services.IkeGateways
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("IkeGateways Not Found", fmt.Sprintf("No IkeGateways found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packIkeGatewaysFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the IkeGateways data source.")
		return
	}
