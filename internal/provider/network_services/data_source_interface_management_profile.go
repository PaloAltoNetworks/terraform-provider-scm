...
// DATA SOURCE for SCM InterfaceManagementProfile (Package: network_services)
var (
...
// InterfaceManagementProfileDataSource defines the data source implementation.
...
func (d *InterfaceManagementProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: InterfaceManagementProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.InterfaceManagementProfilesDataSourceSchema
}

func (d *InterfaceManagementProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: InterfaceManagementProfilesDataSource.Configure ---")
...
func (d *InterfaceManagementProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: InterfaceManagementProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.InterfaceManagementProfilesAPI.GetInterfaceManagementProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading InterfaceManagementProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.InterfaceManagementProfilesAPI.ListInterfaceManagementProfiles(ctx)
		...
		var foundObject *network_services.InterfaceManagementProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("InterfaceManagementProfiles Not Found", fmt.Sprintf("No InterfaceManagementProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packInterfaceManagementProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the InterfaceManagementProfiles data source.")
		return
	}
