...
// DATA SOURCE for SCM IpsecCryptoProfile (Package: network_services)
var (
...
// IpsecCryptoProfileDataSource defines the data source implementation.
...
func (d *IpsecCryptoProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: IpsecCryptoProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.IpsecCryptoProfilesDataSourceSchema
}

func (d *IpsecCryptoProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: IpsecCryptoProfilesDataSource.Configure ---")
...
func (d *IpsecCryptoProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: IpsecCryptoProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading IpsecCryptoProfiles", fmt.Sprintf("Could not read IpsecCryptoProfiles with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading IpsecCryptoProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.IPsecCryptoProfilesAPI.ListIPsecCryptoProfiles(ctx)
		...
		var foundObject *network_services.IpsecCryptoProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("IpsecCryptoProfiles Not Found", fmt.Sprintf("No IpsecCryptoProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packIpsecCryptoProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the IpsecCryptoProfiles data source.")
		return
	}
