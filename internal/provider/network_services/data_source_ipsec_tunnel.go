...
// DATA SOURCE for SCM IpsecTunnel (Package: network_services)
var (
...
// IpsecTunnelDataSource defines the data source implementation.
...
func (d *IpsecTunnelDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: IpsecTunnelDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.IpsecTunnelsDataSourceSchema
}
...
func (d *IpsecTunnelDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: IpsecTunnelsDataSource.Configure ---")
...
func (d *IpsecTunnelDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: IpsecTunnelDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.IPsecTunnelsAPI.GetIPsecTunnelsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading IpsecTunnels", fmt.Sprintf("Could not read IpsecTunnels with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading IpsecTunnels data source by Name", map[string]interface{}{"name": objectName})
		...
		var foundObject *network_services.IpsecTunnels
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("IpsecTunnels Not Found", fmt.Sprintf("No IpsecTunnels found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packIpsecTunnelsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the IpsecTunnels data source.")
		return
	}
