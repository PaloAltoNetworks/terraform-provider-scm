...
// DATA SOURCE for SCM IptagMatchList (Package: network_services)
var (
	_ datasource.DataSource              = &IptagMatchListDataSource{}
	_ datasource.DataSourceWithConfigure = &IptagMatchListDataSource{}
)

func NewIptagMatchListDataSource() datasource.DataSource {
	return &IptagMatchListDataSource{}
}

// IptagMatchListDataSource defines the data source implementation.
type IptagMatchListDataSource struct {
	client *network_services.APIClient
}

func (d *IptagMatchListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: IptagMatchListDataSource.Metadata ---")
	resp.TypeName = req.ProviderTypeName + "_iptag_match_list"
}

func (d *IptagMatchListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: IptagMatchListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.IptagMatchListDataSourceSchema
}

func (d *IptagMatchListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: IptagMatchListDataSource.Configure ---")
...
func (d *IptagMatchListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: IptagMatchListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading IptagMatchList data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.IptagMatchListAPI.GetIptagMatchListByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading IptagMatchList", fmt.Sprintf("Could not read IptagMatchList with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading IptagMatchList data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.IptagMatchListAPI.ListIptagMatchList(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing IptagMatchLists", fmt.Sprintf("Could not list IptagMatchLists: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.IptagMatchList
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("IptagMatchList Not Found", fmt.Sprintf("No IptagMatchList found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packIptagMatchListFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the IptagMatchList data source.")
		return
	}
