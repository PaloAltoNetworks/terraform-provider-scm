...
// DATA SOURCE for SCM Layer2Subinterface (Package: network_services)
var (
	_ datasource.DataSource              = &Layer2SubinterfaceDataSource{}
	_ datasource.DataSourceWithConfigure = &Layer2SubinterfaceDataSource{}
)

func NewLayer2SubinterfaceDataSource() datasource.DataSource {
	return &Layer2SubinterfaceDataSource{}
}

// Layer2SubinterfaceDataSource defines the data source implementation.
type Layer2SubinterfaceDataSource struct {
...
func (d *Layer2SubinterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: Layer2SubinterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.Layer2SubinterfacesDataSourceSchema
}

func (d *Layer2SubinterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: Layer2SubinterfacesDataSource.Configure ---")
...
func (d *Layer2SubinterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: Layer2SubinterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading Layer2Subinterfaces data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.Layer2SubinterfacesAPI.GetLayer2SubinterfacesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading Layer2Subinterfaces data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.Layer2SubinterfacesAPI.ListLayer2Subinterfaces(ctx)
		...
		var foundObject *network_services.Layer2Subinterfaces
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("Layer2Subinterfaces Not Found", fmt.Sprintf("No Layer2Subinterfaces found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLayer2SubinterfacesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Layer2Subinterfaces data source.")
		return
	}
