...
// DATA SOURCE for SCM Layer3Subinterface (Package: network_services)
var (
	_ datasource.DataSource              = &Layer3SubinterfaceDataSource{}
	_ datasource.DataSourceWithConfigure = &Layer3SubinterfaceDataSource{}
)

func NewLayer3SubinterfaceDataSource() datasource.DataSource {
	return &Layer3SubinterfaceDataSource{}
}

// Layer3SubinterfaceDataSource defines the data source implementation.
type Layer3SubinterfaceDataSource struct {
...
func (d *Layer3SubinterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: Layer3SubinterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.Layer3SubinterfacesDataSourceSchema
}

func (d *Layer3SubinterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: Layer3SubinterfacesDataSource.Configure ---")
...
func (d *Layer3SubinterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: Layer3SubinterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading Layer3Subinterfaces data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.Layer3SubinterfacesAPI.GetLayer3SubinterfacesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading Layer3Subinterfaces", fmt.Sprintf("Could not read Layer3Subinterfaces with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading Layer3Subinterfaces data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.Layer3SubinterfacesAPI.ListLayer3Subinterfaces(ctx)
		...
		var foundObject *network_services.Layer3Subinterfaces
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("Layer3Subinterfaces Not Found", fmt.Sprintf("No Layer3Subinterfaces found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLayer3SubinterfacesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Layer3Subinterfaces data source.")
		return
	}
