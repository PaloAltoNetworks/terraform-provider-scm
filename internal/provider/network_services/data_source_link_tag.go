...
var (
	_ datasource.DataSource              = &LinkTagDataSource{}
	_ datasource.DataSourceWithConfigure = &LinkTagDataSource{}
)

func NewLinkTagDataSource() datasource.DataSource {
	return &LinkTagDataSource{}
}

// LinkTagDataSource defines the data source implementation.
type LinkTagDataSource struct {
	client *network_services.APIClient
}

func (d *LinkTagDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: LinkTagDataSource.Metadata ---")
	resp.TypeName = req.ProviderTypeName + "_link_tag"
}

func (d *LinkTagDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: LinkTagDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.LinkTagsDataSourceSchema
}

func (d *LinkTagDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: LinkTagsDataSource.Configure ---")
...
func (d *LinkTagDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LinkTagDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading LinkTags data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.LinkTagsAPI.GetLinkTagsByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading LinkTags data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.LinkTagsAPI.ListLinkTags(ctx)
		...
		var foundObject *network_services.LinkTags
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("LinkTags Not Found", fmt.Sprintf("No LinkTags found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLinkTagsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the LinkTags data source.")
		return
	}
