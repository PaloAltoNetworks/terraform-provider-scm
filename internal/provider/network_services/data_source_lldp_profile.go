...
// DATA SOURCE for SCM LldpProfile (Package: network_services)
var (
...
// LldpProfileDataSource defines the data source implementation.
...
func (d *LldpProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: LldpProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.LldpProfilesDataSourceSchema
}

func (d *LldpProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: LldpProfilesDataSource.Configure ---")
...
func (d *LldpProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LldpProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading LldpProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.LLDPProfilesAPI.GetLLDPProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading LldpProfiles", fmt.Sprintf("Could not read LldpProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading LldpProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.LLDPProfilesAPI.ListLLDPProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing LldpProfiless", fmt.Sprintf("Could not list LldpProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.LldpProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("LldpProfiles Not Found", fmt.Sprintf("No LldpProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLldpProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the LldpProfiles data source.")
		return
	}
