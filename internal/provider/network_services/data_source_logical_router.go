...
// DATA SOURCE for SCM LogicalRouter (Package: network_services)
var (
...
// LogicalRouterDataSource defines the data source implementation.
...
func (d *LogicalRouterDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: LogicalRouterDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.LogicalRoutersDataSourceSchema
}

func (d *LogicalRouterDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: LogicalRoutersDataSource.Configure ---")
...
func (d *LogicalRouterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LogicalRouterDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading LogicalRouters data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.LogicalRoutersAPI.GetLogicalRoutersByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading LogicalRouters", fmt.Sprintf("Could not read LogicalRouters with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading LogicalRouters data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.LogicalRoutersAPI.ListLogicalRouters(ctx)
		...
		var foundObject *network_services.LogicalRouters
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("LogicalRouters Not Found", fmt.Sprintf("No LogicalRouters found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLogicalRoutersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the LogicalRouters data source.")
		return
	}
