...
// DATA SOURCE for SCM LoopbackInterface (Package: network_services)
var (
...
// LoopbackInterfaceDataSource defines the data source implementation.
...
func (d *LoopbackInterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: LoopbackInterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.LoopbackInterfacesDataSourceSchema
}

func (d *LoopbackInterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: LoopbackInterfacesDataSource.Configure ---")
...
func (d *LoopbackInterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LoopbackInterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading LoopbackInterfaces data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.LoopbackInterfacesAPI.GetLoopbackInterfacesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading LoopbackInterfaces data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.LoopbackInterfacesAPI.ListLoopbackInterfaces(ctx)
		...
		var foundObject *network_services.LoopbackInterfaces
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("LoopbackInterfaces Not Found", fmt.Sprintf("No LoopbackInterfaces found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLoopbackInterfacesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the LoopbackInterfaces data source.")
		return
	}
