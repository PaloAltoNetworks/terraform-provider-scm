...
// DATA SOURCE for SCM NatRule (Package: network_services)
var (
...
// NatRuleDataSource defines the data source implementation.
...
func (d *NatRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: NatRuleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.NATRulesAPI.GetNatRulesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading NatRules", fmt.Sprintf("Could not read NatRules with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading NatRules data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.NATRulesAPI.ListNatRules(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing NatRuless", fmt.Sprintf("Could not list NatRuless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.NatRules
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("NatRules Not Found", fmt.Sprintf("No NatRules found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packNatRulesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the NatRules data source.")
		return
	}
