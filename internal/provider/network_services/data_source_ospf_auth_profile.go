...
// DATA SOURCE for SCM OspfAuthProfile (Package: network_services)
var (
	_ datasource.DataSource              = &OspfAuthProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &OspfAuthProfileDataSource{}
)

func NewOspfAuthProfileDataSource() datasource.DataSource {
	return &OspfAuthProfileDataSource{}
}

// OspfAuthProfileDataSource defines the data source implementation.
type OspfAuthProfileDataSource struct {
...
func (d *OspfAuthProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: OspfAuthProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.OspfAuthProfilesDataSourceSchema
}

func (d *OspfAuthProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: OspfAuthProfilesDataSource.Configure ---")
...
func (d *OspfAuthProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: OspfAuthProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.OSPFAuthenticationProfilesAPI.GetOSPFAuthenticationProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading OspfAuthProfiles", fmt.Sprintf("Could not read OspfAuthProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading OspfAuthProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.OSPFAuthenticationProfilesAPI.ListOSPFAuthenticationProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing OspfAuthProfiless", fmt.Sprintf("Could not list OspfAuthProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *network_services.OspfAuthProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("OspfAuthProfiles Not Found", fmt.Sprintf("No OspfAuthProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packOspfAuthProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the OspfAuthProfiles data source.")
		return
	}
