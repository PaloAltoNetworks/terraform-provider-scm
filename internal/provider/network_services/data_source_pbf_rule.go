...
// DATA SOURCE for SCM PbfRule (Package: network_services)
var (
	_ datasource.DataSource              = &PbfRuleDataSource{}
	_ datasource.DataSourceWithConfigure = &PbfRuleDataSource{}
)

func NewPbfRuleDataSource() datasource.DataSource {
	return &PbfRuleDataSource{}
}

// PbfRuleDataSource defines the data source implementation.
type PbfRuleDataSource struct {
	client *network_services.APIClient
}

func (d *PbfRuleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: PbfRuleDataSource.Metadata ---")
	resp.TypeName = req.ProviderTypeName + "_pbf_rule"
}

func (d *PbfRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: PbfRuleDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.PbfRulesDataSourceSchema
}

func (d *PbfRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: PbfRulesDataSource.Configure ---")
...
func (d *PbfRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: PbfRuleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading PbfRules data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.PBFRulesAPI.GetPBFRulesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading PbfRules", fmt.Sprintf("Could not read PbfRules with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading PbfRules data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.PBFRulesAPI.ListPBFRules(ctx)
		...
		var foundObject *network_services.PbfRules
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("PbfRules Not Found", fmt.Sprintf("No PbfRules found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packPbfRulesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the PbfRules data source.")
		return
	}
