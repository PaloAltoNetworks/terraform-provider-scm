...
// DATA SOURCE for SCM QosPolicyRule (Package: network_services)
var (
...
// QosPolicyRuleDataSource defines the data source implementation.
type QosPolicyRuleDataSource struct {
	client *network_services.APIClient
}

func (d *QosPolicyRuleDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: QosPolicyRuleDataSource.Metadata ---")
	resp.TypeName = "scm_qos_policy_rule"
}

func (d *QosPolicyRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: QosPolicyRuleDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.QosPolicyRulesDataSourceSchema
}

func (d *QosPolicyRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: QosPolicyRulesDataSource.Configure ---")
...
func (d *QosPolicyRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: QosPolicyRuleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		objectId := data.Id.ValueString()
		tflog.Debug(ctx, "Reading QosPolicyRules data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.QoSRulesAPI.GetQoSPolicyRulesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading QosPolicyRules", fmt.Sprintf("Could not read QosPolicyRules with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading QosPolicyRules data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.QoSRulesAPI.ListQoSPolicyRules(ctx)
		...
		var foundObject *network_services.QosPolicyRules
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("QosPolicyRules Not Found", fmt.Sprintf("No QosPolicyRules found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packQosPolicyRulesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the QosPolicyRules data source.")
		return
	}
