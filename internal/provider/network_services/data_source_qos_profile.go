...
// DATA SOURCE for SCM QosProfile (Package: network_services)
var (
...
// QosProfileDataSource defines the data source implementation.
type QosProfileDataSource struct {
	client *network_services.APIClient
}

func (d *QosProfileDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: QosProfileDataSource.Metadata ---")
	resp.TypeName = req.ProviderTypeName + "_qos_profile"
}

func (d *QosProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: QosProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.QosProfilesDataSourceSchema
}

func (d *QosProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: QosProfilesDataSource.Configure ---")
...
func (d *QosProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: QosProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading QosProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.QoSProfilesAPI.GetQoSProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading QosProfiles data source by Name", map[string]interface{}{"name": objectName})
		...
		var foundObject *network_services.QosProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("QosProfiles Not Found", fmt.Sprintf("No QosProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packQosProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the QosProfiles data source.")
		return
	}
