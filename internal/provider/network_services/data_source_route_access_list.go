...
// DATA SOURCE for SCM RouteAccessList (Package: network_services)
var (
	_ datasource.DataSource              = &RouteAccessListDataSource{}
	_ datasource.DataSourceWithConfigure = &RouteAccessListDataSource{}
)

func NewRouteAccessListDataSource() datasource.DataSource {
	return &RouteAccessListDataSource{}
}

// RouteAccessListDataSource defines the data source implementation.
type RouteAccessListDataSource struct {
	client *network_services.APIClient
}

func (d *RouteAccessListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: RouteAccessListDataSource.Metadata ---")
	resp.TypeName = req.ProviderTypeName + "_route_access_list"
}

func (d *RouteAccessListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: RouteAccessListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.RouteAccessListsDataSourceSchema
}

func (d *RouteAccessListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: RouteAccessListsDataSource.Configure ---")
...
func (d *RouteAccessListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: RouteAccessListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading RouteAccessLists data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.RouteAccessListsAPI.GetRouteAccessListsByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading RouteAccessLists data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.RouteAccessListsAPI.ListRouteAccessLists(ctx)
		...
		var foundObject *network_services.RouteAccessLists
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("RouteAccessLists Not Found", fmt.Sprintf("No RouteAccessLists found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packRouteAccessListsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the RouteAccessLists data source.")
		return
	}
