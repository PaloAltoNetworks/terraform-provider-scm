...
// DATA SOURCE for SCM RouteCommunityList (Package: network_services)
var (
	_ datasource.DataSource              = &RouteCommunityListDataSource{}
	_ datasource.DataSourceWithConfigure = &RouteCommunityListDataSource{}
)

func NewRouteCommunityListDataSource() datasource.DataSource {
	return &RouteCommunityListDataSource{}
}

// RouteCommunityListDataSource defines the data source implementation.
type RouteCommunityListDataSource struct {
...
func (d *RouteCommunityListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: RouteCommunityListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.RouteCommunityListsDataSourceSchema
}

func (d *RouteCommunityListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: RouteCommunityListsDataSource.Configure ---")
...
func (d *RouteCommunityListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: RouteCommunityListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading RouteCommunityLists data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.RouteCommunityListsAPI.GetRouteCommunityListsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading RouteCommunityLists", fmt.Sprintf("Could not read RouteCommunityLists with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading RouteCommunityLists data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.RouteCommunityListsAPI.ListRouteCommunityLists(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing RouteCommunityListss", fmt.Sprintf("Could not list RouteCommunityListss: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *network_services.RouteCommunityLists
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("RouteCommunityLists Not Found", fmt.Sprintf("No RouteCommunityLists found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packRouteCommunityListsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the RouteCommunityLists data source.")
		return
	}
