...
// DATA SOURCE for SCM RoutePathAccessList (Package: network_services)
var (
	_ datasource.DataSource              = &RoutePathAccessListDataSource{}
	_ datasource.DataSourceWithConfigure = &RoutePathAccessListDataSource{}
)

func NewRoutePathAccessListDataSource() datasource.DataSource {
	return &RoutePathAccessListDataSource{}
}

// RoutePathAccessListDataSource defines the data source implementation.
type RoutePathAccessListDataSource struct {
...
func (d *RoutePathAccessListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: RoutePathAccessListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.RoutePathAccessListsDataSourceSchema
}

func (d *RoutePathAccessListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: RoutePathAccessListsDataSource.Configure ---")
...
func (d *RoutePathAccessListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: RoutePathAccessListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.RoutePathAccessListsAPI.GetRoutePathAccessListsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading RoutePathAccessLists", fmt.Sprintf("Could not read RoutePathAccessLists with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading RoutePathAccessLists data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.RoutePathAccessListsAPI.ListRoutePathAccessLists(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing RoutePathAccessListss", fmt.Sprintf("Could not list RoutePathAccessListss: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *network_services.RoutePathAccessLists
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("RoutePathAccessLists Not Found", fmt.Sprintf("No RoutePathAccessLists found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packRoutePathAccessListsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the RoutePathAccessLists data source.")
		return
	}
