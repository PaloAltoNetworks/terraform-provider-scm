...
// DATA SOURCE for SCM RoutePrefixList (Package: network_services)
var (
	_ datasource.DataSource              = &RoutePrefixListDataSource{}
	_ datasource.DataSourceWithConfigure = &RoutePrefixListDataSource{}
)

func NewRoutePrefixListDataSource() datasource.DataSource {
	return &RoutePrefixListDataSource{}
}

// RoutePrefixListDataSource defines the data source implementation.
type RoutePrefixListDataSource struct {
...
func (d *RoutePrefixListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: RoutePrefixListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.RoutePrefixListsDataSourceSchema
}

func (d *RoutePrefixListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: RoutePrefixListsDataSource.Configure ---")
...
func (d *RoutePrefixListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: RoutePrefixListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading RoutePrefixLists data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.RoutePrefixListsAPI.GetRoutePrefixListsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading RoutePrefixLists", fmt.Sprintf("Could not read RoutePrefixLists with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading RoutePrefixLists data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.RoutePrefixListsAPI.ListRoutePrefixLists(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing RoutePrefixListss", fmt.Sprintf("Could not list RoutePrefixListss: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.RoutePrefixLists
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("RoutePrefixLists Not Found", fmt.Sprintf("No RoutePrefixLists found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packRoutePrefixListsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the RoutePrefixLists data source.")
		return
	}
