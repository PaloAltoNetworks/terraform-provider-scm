...
// DATA SOURCE for SCM SdwanErrorCorrectionProfile (Package: network_services)
var (
	_ datasource.DataSource              = &SdwanErrorCorrectionProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &SdwanErrorCorrectionProfileDataSource{}
)

func NewSdwanErrorCorrectionProfileDataSource() datasource.DataSource {
	return &SdwanErrorCorrectionProfileDataSource{}
}

// SdwanErrorCorrectionProfileDataSource defines the data source implementation.
type SdwanErrorCorrectionProfileDataSource struct {
...
func (d *SdwanErrorCorrectionProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanErrorCorrectionProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SdwanErrorCorrectionProfilesDataSourceSchema
}

func (d *SdwanErrorCorrectionProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanErrorCorrectionProfilesDataSource.Configure ---")
...
func (d *SdwanErrorCorrectionProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanErrorCorrectionProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading SdwanErrorCorrectionProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SDWANErrorCorrectionProfilesAPI.GetSDWANErrorCorrectionProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading SdwanErrorCorrectionProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SDWANErrorCorrectionProfilesAPI.ListSDWANErrorCorrectionProfiles(ctx)
		...
		var foundObject *network_services.SdwanErrorCorrectionProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SdwanErrorCorrectionProfiles Not Found", fmt.Sprintf("No SdwanErrorCorrectionProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSdwanErrorCorrectionProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SdwanErrorCorrectionProfiles data source.")
		return
	}
