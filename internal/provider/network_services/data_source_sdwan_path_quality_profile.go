...
// DATA SOURCE for SCM SdwanPathQualityProfile (Package: network_services)
var (
	_ datasource.DataSource              = &SdwanPathQualityProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &SdwanPathQualityProfileDataSource{}
)

func NewSdwanPathQualityProfileDataSource() datasource.DataSource {
	return &SdwanPathQualityProfileDataSource{}
}

// SdwanPathQualityProfileDataSource defines the data source implementation.
type SdwanPathQualityProfileDataSource struct {
...
func (d *SdwanPathQualityProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanPathQualityProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SdwanPathQualityProfilesDataSourceSchema
}

func (d *SdwanPathQualityProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanPathQualityProfilesDataSource.Configure ---")
...
func (d *SdwanPathQualityProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanPathQualityProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading SdwanPathQualityProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SDWANPathQualityProfilesAPI.GetSDWANPathQualityProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SdwanPathQualityProfiles", fmt.Sprintf("Could not read SdwanPathQualityProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading SdwanPathQualityProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SDWANPathQualityProfilesAPI.ListSDWANPathQualityProfiles(ctx)
		...
		var foundObject *network_services.SdwanPathQualityProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SdwanPathQualityProfiles Not Found", fmt.Sprintf("No SdwanPathQualityProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSdwanPathQualityProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SdwanPathQualityProfiles data source.")
		return
	}
