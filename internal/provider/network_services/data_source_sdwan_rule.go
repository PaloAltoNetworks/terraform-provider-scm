...
// DATA SOURCE for SCM SdwanRule (Package: network_services)
var (
	_ datasource.DataSource              = &SdwanRuleDataSource{}
	_ datasource.DataSourceWithConfigure = &SdwanRuleDataSource{}
)

func NewSdwanRuleDataSource() datasource.DataSource {
	return &SdwanRuleDataSource{}
}

// SdwanRuleDataSource defines the data source implementation.
type SdwanRuleDataSource struct {
...
func (d *SdwanRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanRuleDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SdwanRulesDataSourceSchema
}

func (d *SdwanRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanRulesDataSource.Configure ---")
...
func (d *SdwanRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanRuleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading SdwanRules data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SDWANRulesAPI.GetSDWANRulesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SdwanRules", fmt.Sprintf("Could not read SdwanRules with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading SdwanRules data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SDWANRulesAPI.ListSDWANRules(ctx)
		...
		var foundObject *network_services.SdwanRules
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SdwanRules Not Found", fmt.Sprintf("No SdwanRules found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSdwanRulesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SdwanRules data source.")
		return
	}
