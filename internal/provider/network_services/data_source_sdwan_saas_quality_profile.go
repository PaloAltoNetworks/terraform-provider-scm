...
// DATA SOURCE for SCM SdwanSaasQualityProfile (Package: network_services)
var (
	_ datasource.DataSource              = &SdwanSaasQualityProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &SdwanSaasQualityProfileDataSource{}
)

func NewSdwanSaasQualityProfileDataSource() datasource.DataSource {
	return &SdwanSaasQualityProfileDataSource{}
}

// SdwanSaasQualityProfileDataSource defines the data source implementation.
type SdwanSaasQualityProfileDataSource struct {
...
func (d *SdwanSaasQualityProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanSaasQualityProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SdwanSaasQualityProfilesDataSourceSchema
}

func (d *SdwanSaasQualityProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanSaasQualityProfilesDataSource.Configure ---")
...
func (d *SdwanSaasQualityProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanSaasQualityProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.SDWANSaaSQualityProfilesAPI.GetSDWANSaaSQualityProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading SdwanSaasQualityProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SDWANSaaSQualityProfilesAPI.ListSDWANSaaSQualityProfiles(ctx)
		...
		var foundObject *network_services.SdwanSaasQualityProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SdwanSaasQualityProfiles Not Found", fmt.Sprintf("No SdwanSaasQualityProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSdwanSaasQualityProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SdwanSaasQualityProfiles data source.")
		return
	}
