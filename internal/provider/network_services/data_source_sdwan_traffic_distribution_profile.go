...
// DATA SOURCE for SCM SdwanTrafficDistributionProfile (Package: network_services)
var (
	_ datasource.DataSource              = &SdwanTrafficDistributionProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &SdwanTrafficDistributionProfileDataSource{}
)

func NewSdwanTrafficDistributionProfileDataSource() datasource.DataSource {
	return &SdwanTrafficDistributionProfileDataSource{}
}

// SdwanTrafficDistributionProfileDataSource defines the data source implementation.
type SdwanTrafficDistributionProfileDataSource struct {
...
func (d *SdwanTrafficDistributionProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanTrafficDistributionProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SdwanTrafficDistributionProfilesDataSourceSchema
}

func (d *SdwanTrafficDistributionProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanTrafficDistributionProfilesDataSource.Configure ---")
...
func (d *SdwanTrafficDistributionProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SdwanTrafficDistributionProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading SdwanTrafficDistributionProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SDWANTrafficDistributionProfilesAPI.GetSDWANTrafficDistributionProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SdwanTrafficDistributionProfiles", fmt.Sprintf("Could not read SdwanTrafficDistributionProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading SdwanTrafficDistributionProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SDWANTrafficDistributionProfilesAPI.ListSDWANTrafficDistributionProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing SdwanTrafficDistributionProfiless", fmt.Sprintf("Could not list SdwanTrafficDistributionProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *network_services.SdwanTrafficDistributionProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SdwanTrafficDistributionProfiles Not Found", fmt.Sprintf("No SdwanTrafficDistributionProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSdwanTrafficDistributionProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SdwanTrafficDistributionProfiles data source.")
		return
	}
