...
var (
	_ datasource.DataSource              = &SslDecryptionSettingListDataSource{}
	_ datasource.DataSourceWithConfigure = &SslDecryptionSettingListDataSource{}
)

func NewSslDecryptionSettingListDataSource() datasource.DataSource {
	return &SslDecryptionSettingListDataSource{}
}

// SslDecryptionSettingListDataSource defines the data source implementation.
type SslDecryptionSettingListDataSource struct {
	client *network_services.APIClient
}

func (d *SslDecryptionSettingListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ssl_decryption_setting_list"
}

func (d *SslDecryptionSettingListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.SslDecryptionSettingsListDataSourceSchema
}

func (d *SslDecryptionSettingListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *SslDecryptionSettingListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.SslDecryptionSettingsListModel
	...
	listReq := d.client.SslDecryptionSettingsAPI.getSslDecryptionSettings(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing SslDecryptionSettingss", fmt.Sprintf("Could not list SslDecryptionSettingss: %s", err.Error()))
	...
	packedList, diags := packSslDecryptionSettingsListFromSdk(ctx, listResponse.GetData())
