...
// DATA SOURCE for SCM SystemMatchList (Package: network_services)
var (
	_ datasource.DataSource              = &SystemMatchListDataSource{}
	_ datasource.DataSourceWithConfigure = &SystemMatchListDataSource{}
)

func NewSystemMatchListDataSource() datasource.DataSource {
	return &SystemMatchListDataSource{}
}

// SystemMatchListDataSource defines the data source implementation.
type SystemMatchListDataSource struct {
...
func (d *SystemMatchListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SystemMatchListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SystemMatchListDataSourceSchema
}

func (d *SystemMatchListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SystemMatchListDataSource.Configure ---")
...
func (d *SystemMatchListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SystemMatchListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.SystemMatchListAPI.GetSystemMatchListByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SystemMatchList", fmt.Sprintf("Could not read SystemMatchList with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading SystemMatchList data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SystemMatchListAPI.ListSystemMatchList(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing SystemMatchLists", fmt.Sprintf("Could not list SystemMatchLists: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.SystemMatchList
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SystemMatchList Not Found", fmt.Sprintf("No SystemMatchList found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSystemMatchListFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SystemMatchList data source.")
		return
	}
