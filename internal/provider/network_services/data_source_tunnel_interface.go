...
// DATA SOURCE for SCM TunnelInterface (Package: network_services)
var (
...
// TunnelInterfaceDataSource defines the data source implementation.
...
func (d *TunnelInterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: TunnelInterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.TunnelInterfacesDataSourceSchema
}

func (d *TunnelInterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: TunnelInterfacesDataSource.Configure ---")
...
func (d *TunnelInterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: TunnelInterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading TunnelInterfaces data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.TunnelInterfacesAPI.GetTunnelInterfacesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading TunnelInterfaces data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.TunnelInterfacesAPI.ListTunnelInterfaces(ctx)
		...
		var foundObject *network_services.TunnelInterfaces
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("TunnelInterfaces Not Found", fmt.Sprintf("No TunnelInterfaces found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packTunnelInterfacesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the TunnelInterfaces data source.")
		return
	}
