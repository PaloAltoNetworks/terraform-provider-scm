...
// DATA SOURCE for SCM UseridMatchList (Package: network_services)
var (
	_ datasource.DataSource              = &UseridMatchListDataSource{}
	_ datasource.DataSourceWithConfigure = &UseridMatchListDataSource{}
)

func NewUseridMatchListDataSource() datasource.DataSource {
	return &UseridMatchListDataSource{}
}

// UseridMatchListDataSource defines the data source implementation.
type UseridMatchListDataSource struct {
...
func (d *UseridMatchListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: UseridMatchListDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.UseridMatchListDataSourceSchema
}

func (d *UseridMatchListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: UseridMatchListDataSource.Configure ---")
...
func (d *UseridMatchListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: UseridMatchListDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading UseridMatchList data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.UseridMatchListAPI.GetUseridMatchListByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading UseridMatchList", fmt.Sprintf("Could not read UseridMatchList with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading UseridMatchList data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.UseridMatchListAPI.ListUseridMatchList(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing UseridMatchLists", fmt.Sprintf("Could not list UseridMatchLists: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *network_services.UseridMatchList
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("UseridMatchList Not Found", fmt.Sprintf("No UseridMatchList found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packUseridMatchListFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the UseridMatchList data source.")
		return
	}
