...
// DATA SOURCE for SCM VlanInterface (Package: network_services)
var (
...
// VlanInterfaceDataSource defines the data source implementation.
...
func (d *VlanInterfaceDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: VlanInterfaceDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.VlanInterfacesDataSourceSchema
}

func (d *VlanInterfaceDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: VlanInterfacesDataSource.Configure ---")
...
func (d *VlanInterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: VlanInterfaceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.VLANInterfacesAPI.GetVLANInterfacesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading VlanInterfaces data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.VLANInterfacesAPI.ListVLANInterfaces(ctx)
		...
		var foundObject *network_services.VlanInterfaces
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("VlanInterfaces Not Found", fmt.Sprintf("No VlanInterfaces found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packVlanInterfacesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the VlanInterfaces data source.")
		return
	}
