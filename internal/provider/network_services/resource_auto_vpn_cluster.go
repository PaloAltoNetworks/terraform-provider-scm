...
// RESOURCE for SCM AutoVpnCluster (Package: network_services)
var (
	_ resource.Resource                = &AutoVpnClusterResource{}
	_ resource.ResourceWithConfigure   = &AutoVpnClusterResource{}
	_ resource.ResourceWithImportState = &AutoVpnClusterResource{}
)

func NewAutoVpnClusterResource() resource.Resource {
	return &AutoVpnClusterResource{}
}

// AutoVpnClusterResource defines the resource implementation.
type AutoVpnClusterResource struct {
...
func (r *AutoVpnClusterResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AutoVpnClustersResourceSchema
}

func (r *AutoVpnClusterResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *AutoVpnClusterResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for AutoVpnCluster")
	...
	unpackedScmObject, diags := unpackAutoVpnClustersToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating auto_vpn_clusters", err.Error())
	...
	tflog.Debug(ctx, "Created auto_vpn_clusters", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *AutoVpnClusterResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.AutoVpnClusters - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for AutoVpnCluster")
	var data, savestate models.AutoVpnClusters
	...
	packedObject, diags := packAutoVpnClustersFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.AutoVpnClusters{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *AutoVpnClusterResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.AutoVpnClusters which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for AutoVpnCluster")
	var plan, state models.AutoVpnClusters
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AutoVpnClusters{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating auto_vpn_clusters on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.AutoVPNClustersAPI.UpdateAutoVPNClustersByID(ctx, objectId).AutoVpnClusters(*unpackedScmObject)
...
func (r *AutoVpnClusterResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.AutoVpnClusters
	...
	tflog.Debug(ctx, "Deleting auto_vpn_clusters", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AutoVPNClustersAPI.DeleteAutoVPNClustersByID(ctx, objectId)
