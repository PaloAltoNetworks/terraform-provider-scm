...
// RESOURCE for SCM BgpAddressFamilyProfile (Package: network_services)
var (
	_ resource.Resource                = &BgpAddressFamilyProfileResource{}
	_ resource.ResourceWithConfigure   = &BgpAddressFamilyProfileResource{}
	_ resource.ResourceWithImportState = &BgpAddressFamilyProfileResource{}
)

func NewBgpAddressFamilyProfileResource() resource.Resource {
	return &BgpAddressFamilyProfileResource{}
}

// BgpAddressFamilyProfileResource defines the resource implementation.
type BgpAddressFamilyProfileResource struct {
	client *network_services.APIClient
}

func (r *BgpAddressFamilyProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_bgp_address_family_profile"
}

func (r *BgpAddressFamilyProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.BgpAddressFamilyProfilesResourceSchema
}

func (r *BgpAddressFamilyProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *BgpAddressFamilyProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for BgpAddressFamilyProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpAddressFamilyProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackBgpAddressFamilyProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating bgp_address_family_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.BGPAddressFamilyProfilesAPI.CreateBGPAddressFamilyProfiles(ctx).BgpAddressFamilyProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating bgp_address_family_profiles", err.Error())
	...
	packedObject, diags := packBgpAddressFamilyProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created bgp_address_family_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *BgpAddressFamilyProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.BgpAddressFamilyProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for BgpAddressFamilyProfile")
	var data, savestate models.BgpAddressFamilyProfiles
	...
	tflog.Debug(ctx, "Reading bgp_address_family_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.BGPAddressFamilyProfilesAPI.GetBGPAddressFamilyProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_address_family_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading bgp_address_family_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpAddressFamilyProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *BgpAddressFamilyProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.BgpAddressFamilyProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for BgpAddressFamilyProfile")
	var plan, state models.BgpAddressFamilyProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpAddressFamilyProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating bgp_address_family_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.BGPAddressFamilyProfilesAPI.UpdateBGPAddressFamilyProfilesByID(ctx, objectId).BgpAddressFamilyProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_address_family_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating bgp_address_family_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpAddressFamilyProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated bgp_address_family_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *BgpAddressFamilyProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.BgpAddressFamilyProfiles
	...
	tflog.Debug(ctx, "Deleting bgp_address_family_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.BGPAddressFamilyProfilesAPI.DeleteBGPAddressFamilyProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting bgp_address_family_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.BGPAddressFamilyProfilesAPI.GetBGPAddressFamilyProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *BgpAddressFamilyProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
