...
// RESOURCE for SCM BgpAuthProfile (Package: network_services)
var (
	_ resource.Resource                = &BgpAuthProfileResource{}
	_ resource.ResourceWithConfigure   = &BgpAuthProfileResource{}
	_ resource.ResourceWithImportState = &BgpAuthProfileResource{}
)

func NewBgpAuthProfileResource() resource.Resource {
	return &BgpAuthProfileResource{}
}

// BgpAuthProfileResource defines the resource implementation.
type BgpAuthProfileResource struct {
	client *network_services.APIClient
}

func (r *BgpAuthProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_bgp_auth_profile"
}

func (r *BgpAuthProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.BgpAuthProfilesResourceSchema
}

func (r *BgpAuthProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *BgpAuthProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for BgpAuthProfile")
	...
	patcher := &bgpAuthProfilesSensitiveValuePatcher{}

	// Stash plaintext values from the plan.

	{ // Stash plaintext for Secret
		var finalVal basetypes.StringValue
		if !resp.Diagnostics.HasError() {

			// Innermost block
			finalVal = data.Secret

		}
		if !resp.Diagnostics.HasError() && !finalVal.IsUnknown() && !finalVal.IsNull() {
			patcher.secret_plaintext = finalVal
		}
	}

	// Unpack the plan to an SCM SDK object.
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpAuthProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackBgpAuthProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating bgp_auth_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.BGPAuthenticationProfilesAPI.CreateBGPAuthenticationProfiles(ctx).BgpAuthProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating bgp_auth_profiles", err.Error())
	...
	packedObject, diags := packBgpAuthProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for Secret
		if !resp.Diagnostics.HasError() {

			// Innermost block
			patcher.secret_encrypted = data.Secret
			data.Secret = patcher.secret_plaintext
	...
	tflog.Debug(ctx, "Created bgp_auth_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *BgpAuthProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.BgpAuthProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for BgpAuthProfile")
	var data, savestate models.BgpAuthProfiles
	...
	tflog.Debug(ctx, "Reading bgp_auth_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.BGPAuthenticationProfilesAPI.GetBGPAuthenticationProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_auth_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading bgp_auth_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpAuthProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for Secret
		if !resp.Diagnostics.HasError() {
			...
			if patcher.secret_encrypted.Equal(data.Secret) {
				data.Secret = patcher.secret_plaintext
			} else {
				data.Secret = basetypes.NewStringNull()
			}
...
func (r *BgpAuthProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.BgpAuthProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for BgpAuthProfile")
	var plan, state models.BgpAuthProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpAuthProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating bgp_auth_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.BGPAuthenticationProfilesAPI.UpdateBGPAuthenticationProfilesByID(ctx, objectId).BgpAuthProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_auth_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating bgp_auth_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpAuthProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for Secret
		if !resp.Diagnostics.HasError() {
			...
			// Innermost block
			patcher.secret_encrypted = plan.Secret
			plan.Secret = patcher.secret_plaintext
	...
	tflog.Debug(ctx, "Updated bgp_auth_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *BgpAuthProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.BgpAuthProfiles
	...
	tflog.Debug(ctx, "Deleting bgp_auth_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.BGPAuthenticationProfilesAPI.DeleteBGPAuthenticationProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting bgp_auth_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.BGPAuthenticationProfilesAPI.GetBGPAuthenticationProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *BgpAuthProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
