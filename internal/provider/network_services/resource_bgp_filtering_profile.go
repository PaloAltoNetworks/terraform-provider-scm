...
// RESOURCE for SCM BgpFilteringProfile (Package: network_services)
var (
	_ resource.Resource                = &BgpFilteringProfileResource{}
	_ resource.ResourceWithConfigure   = &BgpFilteringProfileResource{}
	_ resource.ResourceWithImportState = &BgpFilteringProfileResource{}
)

func NewBgpFilteringProfileResource() resource.Resource {
	return &BgpFilteringProfileResource{}
}

// BgpFilteringProfileResource defines the resource implementation.
type BgpFilteringProfileResource struct {
	client *network_services.APIClient
}

func (r *BgpFilteringProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_bgp_filtering_profile"
}

func (r *BgpFilteringProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.BgpFilteringProfilesResourceSchema
}

func (r *BgpFilteringProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *BgpFilteringProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for BgpFilteringProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpFilteringProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackBgpFilteringProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating bgp_filtering_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.BGPFilteringProfilesAPI.CreateBGPFilteringProfiles(ctx).BgpFilteringProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating bgp_filtering_profiles", err.Error())
	...
	packedObject, diags := packBgpFilteringProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created bgp_filtering_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *BgpFilteringProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.BgpFilteringProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for BgpFilteringProfile")
	var data, savestate models.BgpFilteringProfiles
	...
	tflog.Debug(ctx, "Reading bgp_filtering_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.BGPFilteringProfilesAPI.GetBGPFilteringProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_filtering_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading bgp_filtering_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpFilteringProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *BgpFilteringProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.BgpFilteringProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for BgpFilteringProfile")
	var plan, state models.BgpFilteringProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpFilteringProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating bgp_filtering_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.BGPFilteringProfilesAPI.UpdateBGPFilteringProfilesByID(ctx, objectId).BgpFilteringProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_filtering_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating bgp_filtering_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpFilteringProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated bgp_filtering_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *BgpFilteringProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.BgpFilteringProfiles
	...
	tflog.Debug(ctx, "Deleting bgp_filtering_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.BGPFilteringProfilesAPI.DeleteBGPFilteringProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting bgp_filtering_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.BGPFilteringProfilesAPI.GetBGPFilteringProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *BgpFilteringProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
