...
// RESOURCE for SCM BgpRedistributionProfile (Package: network_services)
var (
	_ resource.Resource                = &BgpRedistributionProfileResource{}
	_ resource.ResourceWithConfigure   = &BgpRedistributionProfileResource{}
	_ resource.ResourceWithImportState = &BgpRedistributionProfileResource{}
)

func NewBgpRedistributionProfileResource() resource.Resource {
	return &BgpRedistributionProfileResource{}
}

// BgpRedistributionProfileResource defines the resource implementation.
type BgpRedistributionProfileResource struct {
	client *network_services.APIClient
}

func (r *BgpRedistributionProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_bgp_redistribution_profile"
}

func (r *BgpRedistributionProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.BgpRedistributionProfilesResourceSchema
}

func (r *BgpRedistributionProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *BgpRedistributionProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for BgpRedistributionProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpRedistributionProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackBgpRedistributionProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating bgp_redistribution_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.BGPRedistributionProfilesAPI.CreateBGPRedistributionProfiles(ctx).BgpRedistributionProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating bgp_redistribution_profiles", err.Error())
	...
	packedObject, diags := packBgpRedistributionProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created bgp_redistribution_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *BgpRedistributionProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.BgpRedistributionProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for BgpRedistributionProfile")
	var data, savestate models.BgpRedistributionProfiles
	...
	tflog.Debug(ctx, "Reading bgp_redistribution_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.BGPRedistributionProfilesAPI.GetBGPRedistributionProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_redistribution_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading bgp_redistribution_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpRedistributionProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *BgpRedistributionProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.BgpRedistributionProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for BgpRedistributionProfile")
	var plan, state models.BgpRedistributionProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpRedistributionProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating bgp_redistribution_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.BGPRedistributionProfilesAPI.UpdateBGPRedistributionProfilesByID(ctx, objectId).BgpRedistributionProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_redistribution_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating bgp_redistribution_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpRedistributionProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated bgp_redistribution_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *BgpRedistributionProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.BgpRedistributionProfiles
	...
	tflog.Debug(ctx, "Deleting bgp_redistribution_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.BGPRedistributionProfilesAPI.DeleteBGPRedistributionProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting bgp_redistribution_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.BGPRedistributionProfilesAPI.GetBGPRedistributionProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *BgpRedistributionProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
