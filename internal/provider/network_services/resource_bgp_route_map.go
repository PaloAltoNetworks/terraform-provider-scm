...
// RESOURCE for SCM BgpRouteMap (Package: network_services)
var (
	_ resource.Resource                = &BgpRouteMapResource{}
	_ resource.ResourceWithConfigure   = &BgpRouteMapResource{}
	_ resource.ResourceWithImportState = &BgpRouteMapResource{}
)
...
func (r *BgpRouteMapResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.BgpRouteMapsResourceSchema
}
...
func (r *BgpRouteMapResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for BgpRouteMap")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpRouteMaps{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackBgpRouteMapsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating bgp_route_maps on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.BGPRouteMapsAPI.CreateBGPRouteMaps(ctx).BgpRouteMaps(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating bgp_route_maps", err.Error())
	...
	packedObject, diags := packBgpRouteMapsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created bgp_route_maps", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *BgpRouteMapResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.BgpRouteMaps - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for BgpRouteMap")
	var data, savestate models.BgpRouteMaps
	...
	tflog.Debug(ctx, "Reading bgp_route_maps from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.BGPRouteMapsAPI.GetBGPRouteMapsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_route_maps on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading bgp_route_maps", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpRouteMapsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.BgpRouteMaps{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *BgpRouteMapResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.BgpRouteMaps which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for BgpRouteMap")
	var plan, state models.BgpRouteMaps
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.BgpRouteMaps{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating bgp_route_maps on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.BGPRouteMapsAPI.UpdateBGPRouteMapsByID(ctx, objectId).BgpRouteMaps(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no bgp_route_maps on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating bgp_route_maps", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packBgpRouteMapsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated bgp_route_maps", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *BgpRouteMapResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting bgp_route_maps", map[string]interface{}{"id": objectId})
	deleteReq := r.client.BGPRouteMapsAPI.DeleteBGPRouteMapsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting bgp_route_maps", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
