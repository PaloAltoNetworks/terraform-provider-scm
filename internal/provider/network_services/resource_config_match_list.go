...
// RESOURCE for SCM ConfigMatchList (Package: network_services)
var (
	_ resource.Resource                = &ConfigMatchListResource{}
	_ resource.ResourceWithConfigure   = &ConfigMatchListResource{}
	_ resource.ResourceWithImportState = &ConfigMatchListResource{}
)

func NewConfigMatchListResource() resource.Resource {
	return &ConfigMatchListResource{}
}

// ConfigMatchListResource defines the resource implementation.
type ConfigMatchListResource struct {
	client *network_services.APIClient
}

func (r *ConfigMatchListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_config_match_list"
}

func (r *ConfigMatchListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ConfigMatchListResourceSchema
}

func (r *ConfigMatchListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *ConfigMatchListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ConfigMatchList")
	var data models.ConfigMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ConfigMatchList{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackConfigMatchListToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating config_match_list on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ConfigMatchListAPI.CreateConfigMatchList(ctx).ConfigMatchList(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating config_match_list", err.Error())
	...
	packedObject, diags := packConfigMatchListFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created config_match_list", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ConfigMatchListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ConfigMatchList - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ConfigMatchList")
	var data, savestate models.ConfigMatchList
	...
	tflog.Debug(ctx, "Reading config_match_list from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ConfigMatchListAPI.GetConfigMatchListByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no config_match_list on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading config_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packConfigMatchListFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.ConfigMatchList{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ConfigMatchListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.ConfigMatchList which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ConfigMatchList")
	var plan, state models.ConfigMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ConfigMatchList{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating config_match_list on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ConfigMatchListAPI.UpdateConfigMatchListByID(ctx, objectId).ConfigMatchList(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no config_match_list on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating config_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packConfigMatchListFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated config_match_list", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *ConfigMatchListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.ConfigMatchList
	...
	tflog.Debug(ctx, "Deleting config_match_list", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ConfigMatchListAPI.DeleteConfigMatchListByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting config_match_list", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *ConfigMatchListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
