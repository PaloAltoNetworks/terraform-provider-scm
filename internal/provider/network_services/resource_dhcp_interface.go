...
// RESOURCE for SCM DhcpInterface (Package: network_services)
var (
	_ resource.Resource                = &DhcpInterfaceResource{}
	_ resource.ResourceWithConfigure   = &DhcpInterfaceResource{}
	_ resource.ResourceWithImportState = &DhcpInterfaceResource{}
)

func NewDhcpInterfaceResource() resource.Resource {
	return &DhcpInterfaceResource{}
}

// DhcpInterfaceResource defines the resource implementation.
type DhcpInterfaceResource struct {
	client *network_services.APIClient
}

func (r *DhcpInterfaceResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_dhcp_interface"
}

func (r *DhcpInterfaceResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DhcpInterfacesResourceSchema
}

func (r *DhcpInterfaceResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DhcpInterfaceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DhcpInterface")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DhcpInterfaces{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDhcpInterfacesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating dhcp_interfaces on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DHCPInterfacesAPI.CreateDHCPInterfaces(ctx).DhcpInterfaces(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating dhcp_interfaces", err.Error())
	...
	packedObject, diags := packDhcpInterfacesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created dhcp_interfaces", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DhcpInterfaceResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DhcpInterfaces - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DhcpInterface")
	var data, savestate models.DhcpInterfaces
	...
	tflog.Debug(ctx, "Reading dhcp_interfaces from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DHCPInterfacesAPI.GetDHCPInterfacesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dhcp_interfaces on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading dhcp_interfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDhcpInterfacesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.DhcpInterfaces{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DhcpInterfaceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.DhcpInterfaces which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DhcpInterface")
	var plan, state models.DhcpInterfaces
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DhcpInterfaces{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating dhcp_interfaces on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DHCPInterfacesAPI.UpdateDHCPInterfacesByID(ctx, objectId).DhcpInterfaces(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dhcp_interfaces on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating dhcp_interfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDhcpInterfacesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated dhcp_interfaces", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *DhcpInterfaceResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.DhcpInterfaces
	...
	tflog.Debug(ctx, "Deleting dhcp_interfaces", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DHCPInterfacesAPI.DeleteDHCPInterfacesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting dhcp_interfaces", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *DhcpInterfaceResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
