...
// RESOURCE for SCM DnsProxy (Package: network_services)
var (
...
// DnsProxyResource defines the resource implementation.
...
func (r *DnsProxyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DnsProxy")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DnsProxies{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDnsProxiesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating dns_proxies on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DNSProxiesAPI.CreateDNSProxies(ctx).DnsProxies(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating dns_proxies", err.Error())
	...
	packedObject, diags := packDnsProxiesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created dns_proxies", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DnsProxyResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DnsProxies - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DnsProxy")
	var data, savestate models.DnsProxies
	...
	tflog.Debug(ctx, "Reading dns_proxies from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DNSProxiesAPI.GetDNSProxiesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dns_proxies on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading dns_proxies", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDnsProxiesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.DnsProxies{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DnsProxyResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.DnsProxies which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DnsProxy")
	var plan, state models.DnsProxies
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DnsProxies{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating dns_proxies on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DNSProxiesAPI.UpdateDNSProxiesByID(ctx, objectId).DnsProxies(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dns_proxies on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating dns_proxies", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDnsProxiesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated dns_proxies", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *DnsProxyResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting dns_proxies", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DNSProxiesAPI.DeleteDNSProxiesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting dns_proxies", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
