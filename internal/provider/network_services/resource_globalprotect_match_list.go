...
// RESOURCE for SCM GlobalprotectMatchList (Package: network_services)
var (
	_ resource.Resource                = &GlobalprotectMatchListResource{}
	_ resource.ResourceWithConfigure   = &GlobalprotectMatchListResource{}
	_ resource.ResourceWithImportState = &GlobalprotectMatchListResource{}
)

func NewGlobalprotectMatchListResource() resource.Resource {
	return &GlobalprotectMatchListResource{}
}

// GlobalprotectMatchListResource defines the resource implementation.
type GlobalprotectMatchListResource struct {
	client *network_services.APIClient
}

func (r *GlobalprotectMatchListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_globalprotect_match_list"
}

func (r *GlobalprotectMatchListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.GlobalprotectMatchListResourceSchema
}

func (r *GlobalprotectMatchListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *GlobalprotectMatchListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for GlobalprotectMatchList")
	var data models.GlobalprotectMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.GlobalprotectMatchList{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackGlobalprotectMatchListToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating globalprotect_match_list on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.GlobalprotectMatchListAPI.CreateGlobalprotectMatchList(ctx).GlobalprotectMatchList(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating globalprotect_match_list", err.Error())
	...
	packedObject, diags := packGlobalprotectMatchListFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created globalprotect_match_list", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *GlobalprotectMatchListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.GlobalprotectMatchList - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for GlobalprotectMatchList")
	var data, savestate models.GlobalprotectMatchList
	...
	tflog.Debug(ctx, "Reading globalprotect_match_list from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.GlobalprotectMatchListAPI.GetGlobalprotectMatchListByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no globalprotect_match_list on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading globalprotect_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packGlobalprotectMatchListFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *GlobalprotectMatchListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.GlobalprotectMatchList which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for GlobalprotectMatchList")
	var plan, state models.GlobalprotectMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.GlobalprotectMatchList{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating globalprotect_match_list on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.GlobalprotectMatchListAPI.UpdateGlobalprotectMatchListByID(ctx, objectId).GlobalprotectMatchList(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no globalprotect_match_list on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating globalprotect_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packGlobalprotectMatchListFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated globalprotect_match_list", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *GlobalprotectMatchListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.GlobalprotectMatchList
	...
	tflog.Debug(ctx, "Deleting globalprotect_match_list", map[string]interface{}{"id": objectId})
	deleteReq := r.client.GlobalprotectMatchListAPI.DeleteGlobalprotectMatchListByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting globalprotect_match_list", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.GlobalprotectMatchListAPI.GetGlobalprotectMatchListByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *GlobalprotectMatchListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
