...
// RESOURCE for SCM HipmatchMatchList (Package: network_services)
var (
	_ resource.Resource                = &HipmatchMatchListResource{}
	_ resource.ResourceWithConfigure   = &HipmatchMatchListResource{}
	_ resource.ResourceWithImportState = &HipmatchMatchListResource{}
)

func NewHipmatchMatchListResource() resource.Resource {
	return &HipmatchMatchListResource{}
}

// HipmatchMatchListResource defines the resource implementation.
type HipmatchMatchListResource struct {
	client *network_services.APIClient
}

func (r *HipmatchMatchListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_hipmatch_match_list"
}

func (r *HipmatchMatchListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.HipmatchMatchListResourceSchema
}

func (r *HipmatchMatchListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *HipmatchMatchListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for HipmatchMatchList")
	var data models.HipmatchMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HipmatchMatchList{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackHipmatchMatchListToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating hipmatch_match_list on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.HipmatchMatchListAPI.CreateHipmatchMatchList(ctx).HipmatchMatchList(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating hipmatch_match_list", err.Error())
	...
	packedObject, diags := packHipmatchMatchListFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created hipmatch_match_list", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *HipmatchMatchListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.HipmatchMatchList - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for HipmatchMatchList")
	var data, savestate models.HipmatchMatchList
	...
	tflog.Debug(ctx, "Reading hipmatch_match_list from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.HipmatchMatchListAPI.GetHipmatchMatchListByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no hipmatch_match_list on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading hipmatch_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packHipmatchMatchListFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *HipmatchMatchListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.HipmatchMatchList which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for HipmatchMatchList")
	var plan, state models.HipmatchMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HipmatchMatchList{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating hipmatch_match_list on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.HipmatchMatchListAPI.UpdateHipmatchMatchListByID(ctx, objectId).HipmatchMatchList(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no hipmatch_match_list on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating hipmatch_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packHipmatchMatchListFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated hipmatch_match_list", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *HipmatchMatchListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.HipmatchMatchList
	...
	tflog.Debug(ctx, "Deleting hipmatch_match_list", map[string]interface{}{"id": objectId})
	deleteReq := r.client.HipmatchMatchListAPI.DeleteHipmatchMatchListByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting hipmatch_match_list", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.HipmatchMatchListAPI.GetHipmatchMatchListByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *HipmatchMatchListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
