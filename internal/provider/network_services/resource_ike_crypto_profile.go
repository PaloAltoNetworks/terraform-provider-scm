...
// RESOURCE for SCM IkeCryptoProfile (Package: network_services)
var (
...
// IkeCryptoProfileResource defines the resource implementation.
...
func (r *IkeCryptoProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_ike_crypto_profile"
}

func (r *IkeCryptoProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.IkeCryptoProfilesResourceSchema
}
...
func (r *IkeCryptoProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for IkeCryptoProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IkeCryptoProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackIkeCryptoProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating ike_crypto_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.IKECryptoProfilesAPI.CreateIKECryptoProfiles(ctx).IkeCryptoProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating ike_crypto_profiles", err.Error())
	...
	packedObject, diags := packIkeCryptoProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created ike_crypto_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *IkeCryptoProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.IkeCryptoProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for IkeCryptoProfile")
	var data, savestate models.IkeCryptoProfiles
	...
	tflog.Debug(ctx, "Reading ike_crypto_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.IKECryptoProfilesAPI.GetIKECryptoProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ike_crypto_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading ike_crypto_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIkeCryptoProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *IkeCryptoProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.IkeCryptoProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for IkeCryptoProfile")
	var plan, state models.IkeCryptoProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IkeCryptoProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating ike_crypto_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.IKECryptoProfilesAPI.UpdateIKECryptoProfilesByID(ctx, objectId).IkeCryptoProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ike_crypto_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating ike_crypto_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIkeCryptoProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated ike_crypto_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *IkeCryptoProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting ike_crypto_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.IKECryptoProfilesAPI.DeleteIKECryptoProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting ike_crypto_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.IKECryptoProfilesAPI.GetIKECryptoProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
