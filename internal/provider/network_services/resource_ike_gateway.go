...
// RESOURCE for SCM IkeGateway (Package: network_services)
var (
...
// IkeGatewayResource defines the resource implementation.
...
func (r *IkeGatewayResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.IkeGatewaysResourceSchema
}
...
func (r *IkeGatewayResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for IkeGateway")
	...
	patcher := &ikeGatewaysSensitiveValuePatcher{}

	// Stash plaintext values from the plan.

	{ // Stash plaintext for Authentication.PreSharedKey.Key
		var finalVal basetypes.StringValue
		if !resp.Diagnostics.HasError() {

			if !data.Authentication.IsNull() && !data.Authentication.IsUnknown() {
				var temp_stash_Authentication_PreSharedKey_Key_0 models.IkeGatewaysAuthentication
				resp.Diagnostics.Append(data.Authentication.As(ctx, &temp_stash_Authentication_PreSharedKey_Key_0, basetypes.ObjectAsOptions{})...)
				if !resp.Diagnostics.HasError() {

					if !temp_stash_Authentication_PreSharedKey_Key_0.PreSharedKey.IsNull() && !temp_stash_Authentication_PreSharedKey_Key_0.PreSharedKey.IsUnknown() {
						var temp_stash_Authentication_PreSharedKey_Key_1 models.IkeGatewaysAuthenticationPreSharedKey
						resp.Diagnostics.Append(temp_stash_Authentication_PreSharedKey_Key_0.PreSharedKey.As(ctx, &temp_stash_Authentication_PreSharedKey_Key_1, basetypes.ObjectAsOptions{})...)
						if !resp.Diagnostics.HasError() {

							// Innermost block
							finalVal = temp_stash_Authentication_PreSharedKey_Key_1.Key
		...
		if !resp.Diagnostics.HasError() && !finalVal.IsUnknown() && !finalVal.IsNull() {
			patcher.authentication_pre_shared_key_key_plaintext = finalVal
		}
	}

	// Unpack the plan to an SCM SDK object.
	planObject, diags := types.ObjectValueFrom(ctx, models.IkeGateways{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackIkeGatewaysToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating ike_gateways on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.IKEGatewaysAPI.CreateIKEGateways(ctx).IkeGateways(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating ike_gateways", err.Error())
	...
	packedObject, diags := packIkeGatewaysFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for Authentication.PreSharedKey.Key
		if !resp.Diagnostics.HasError() {

			if !data.Authentication.IsNull() && !data.Authentication.IsUnknown() {
				var temp_patch_create_Authentication_PreSharedKey_Key_0 models.IkeGatewaysAuthentication
				resp.Diagnostics.Append(data.Authentication.As(ctx, &temp_patch_create_Authentication_PreSharedKey_Key_0, basetypes.ObjectAsOptions{})...)
				if !resp.Diagnostics.HasError() {

					if !temp_patch_create_Authentication_PreSharedKey_Key_0.PreSharedKey.IsNull() && !temp_patch_create_Authentication_PreSharedKey_Key_0.PreSharedKey.IsUnknown() {
						var temp_patch_create_Authentication_PreSharedKey_Key_1 models.IkeGatewaysAuthenticationPreSharedKey
						resp.Diagnostics.Append(temp_patch_create_Authentication_PreSharedKey_Key_0.PreSharedKey.As(ctx, &temp_patch_create_Authentication_PreSharedKey_Key_1, basetypes.ObjectAsOptions{})...)
						if !resp.Diagnostics.HasError() {

							// Innermost block
							patcher.authentication_pre_shared_key_key_encrypted = temp_patch_create_Authentication_PreSharedKey_Key_1.Key
							temp_patch_create_Authentication_PreSharedKey_Key_1.Key = patcher.authentication_pre_shared_key_key_plaintext

							// Repack the modified structs.

							if !resp.Diagnostics.HasError() {

								temp_patch_create_Authentication_PreSharedKey_Key_0.PreSharedKey, diags = types.ObjectValueFrom(ctx, models.IkeGatewaysAuthenticationPreSharedKey{}.AttrTypes(), &temp_patch_create_Authentication_PreSharedKey_Key_1)
							...
							if !resp.Diagnostics.HasError() {
								...
								data.Authentication, diags = types.ObjectValueFrom(ctx, models.IkeGatewaysAuthentication{}.AttrTypes(), &temp_patch_create_Authentication_PreSharedKey_Key_0)
	...
	tflog.Debug(ctx, "Created ike_gateways", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *IkeGatewayResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.IkeGateways - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for IkeGateway")
	var data, savestate models.IkeGateways
	...
	tflog.Debug(ctx, "Reading ike_gateways from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.IKEGatewaysAPI.GetIKEGatewaysByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ike_gateways on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading ike_gateways", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIkeGatewaysFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.IkeGateways{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for Authentication.PreSharedKey.Key
		if !resp.Diagnostics.HasError() {
			...
			if !data.Authentication.IsNull() && !data.Authentication.IsUnknown() {
				var temp_patch_read_Authentication_PreSharedKey_Key_0 models.IkeGatewaysAuthentication
				resp.Diagnostics.Append(data.Authentication.As(ctx, &temp_patch_read_Authentication_PreSharedKey_Key_0, basetypes.ObjectAsOptions{})...)
				if !resp.Diagnostics.HasError() {

					if !temp_patch_read_Authentication_PreSharedKey_Key_0.PreSharedKey.IsNull() && !temp_patch_read_Authentication_PreSharedKey_Key_0.PreSharedKey.IsUnknown() {
						var temp_patch_read_Authentication_PreSharedKey_Key_1 models.IkeGatewaysAuthenticationPreSharedKey
						resp.Diagnostics.Append(temp_patch_read_Authentication_PreSharedKey_Key_0.PreSharedKey.As(ctx, &temp_patch_read_Authentication_PreSharedKey_Key_1, basetypes.ObjectAsOptions{})...)
						if !resp.Diagnostics.HasError() {

							// Innermost block: Perform comparison and patch.
							if patcher.authentication_pre_shared_key_key_encrypted.Equal(temp_patch_read_Authentication_PreSharedKey_Key_1.Key) {
								temp_patch_read_Authentication_PreSharedKey_Key_1.Key = patcher.authentication_pre_shared_key_key_plaintext
							} else {
								temp_patch_read_Authentication_PreSharedKey_Key_1.Key = basetypes.NewStringNull()
							}
							...
							if !resp.Diagnostics.HasError() {
								...
								temp_patch_read_Authentication_PreSharedKey_Key_0.PreSharedKey, diags = types.ObjectValueFrom(ctx, models.IkeGatewaysAuthenticationPreSharedKey{}.AttrTypes(), &temp_patch_read_Authentication_PreSharedKey_Key_1)
							...
							if !resp.Diagnostics.HasError() {
								...
								data.Authentication, diags = types.ObjectValueFrom(ctx, models.IkeGatewaysAuthentication{}.AttrTypes(), &temp_patch_read_Authentication_PreSharedKey_Key_0)
...
func (r *IkeGatewayResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.IkeGateways which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for IkeGateway")
	var plan, state models.IkeGateways
	...
	{ // Stash plaintext for Authentication.PreSharedKey.Key
		...
		if !resp.Diagnostics.HasError() {
			...
			if !plan.Authentication.IsNull() && !plan.Authentication.IsUnknown() {
				var temp_stash_upd_Authentication_PreSharedKey_Key_0 models.IkeGatewaysAuthentication
				resp.Diagnostics.Append(plan.Authentication.As(ctx, &temp_stash_upd_Authentication_PreSharedKey_Key_0, basetypes.ObjectAsOptions{})...)
				if !resp.Diagnostics.HasError() {

					if !temp_stash_upd_Authentication_PreSharedKey_Key_0.PreSharedKey.IsNull() && !temp_stash_upd_Authentication_PreSharedKey_Key_0.PreSharedKey.IsUnknown() {
						var temp_stash_upd_Authentication_PreSharedKey_Key_1 models.IkeGatewaysAuthenticationPreSharedKey
						resp.Diagnostics.Append(temp_stash_upd_Authentication_PreSharedKey_Key_0.PreSharedKey.As(ctx, &temp_stash_upd_Authentication_PreSharedKey_Key_1, basetypes.ObjectAsOptions{})...)
						if !resp.Diagnostics.HasError() {

							// Innermost block
							finalVal = temp_stash_upd_Authentication_PreSharedKey_Key_1.Key
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IkeGateways{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating ike_gateways on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.IKEGatewaysAPI.UpdateIKEGatewaysByID(ctx, objectId).IkeGateways(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ike_gateways on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating ike_gateways", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIkeGatewaysFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	{ // Patch plaintext for Authentication.PreSharedKey.Key
		if !resp.Diagnostics.HasError() {
			...
			if !plan.Authentication.IsNull() && !plan.Authentication.IsUnknown() {
				var temp_patch_upd_Authentication_PreSharedKey_Key_0 models.IkeGatewaysAuthentication
				resp.Diagnostics.Append(plan.Authentication.As(ctx, &temp_patch_upd_Authentication_PreSharedKey_Key_0, basetypes.ObjectAsOptions{})...)
				if !resp.Diagnostics.HasError() {

					if !temp_patch_upd_Authentication_PreSharedKey_Key_0.PreSharedKey.IsNull() && !temp_patch_upd_Authentication_PreSharedKey_Key_0.PreSharedKey.IsUnknown() {
						var temp_patch_upd_Authentication_PreSharedKey_Key_1 models.IkeGatewaysAuthenticationPreSharedKey
						resp.Diagnostics.Append(temp_patch_upd_Authentication_PreSharedKey_Key_0.PreSharedKey.As(ctx, &temp_patch_upd_Authentication_PreSharedKey_Key_1, basetypes.ObjectAsOptions{})...)
						if !resp.Diagnostics.HasError() {

							// Innermost block
							patcher.authentication_pre_shared_key_key_encrypted = temp_patch_upd_Authentication_PreSharedKey_Key_1.Key
							temp_patch_upd_Authentication_PreSharedKey_Key_1.Key = patcher.authentication_pre_shared_key_key_plaintext

							// Repack the modified structs.

							if !resp.Diagnostics.HasError() {

								temp_patch_upd_Authentication_PreSharedKey_Key_0.PreSharedKey, diags = types.ObjectValueFrom(ctx, models.IkeGatewaysAuthenticationPreSharedKey{}.AttrTypes(), &temp_patch_upd_Authentication_PreSharedKey_Key_1)
							...
							if !resp.Diagnostics.HasError() {
								...
								plan.Authentication, diags = types.ObjectValueFrom(ctx, models.IkeGatewaysAuthentication{}.AttrTypes(), &temp_patch_upd_Authentication_PreSharedKey_Key_0)
	...
	tflog.Debug(ctx, "Updated ike_gateways", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *IkeGatewayResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting ike_gateways", map[string]interface{}{"id": objectId})
	deleteReq := r.client.IKEGatewaysAPI.DeleteIKEGatewaysByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting ike_gateways", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
