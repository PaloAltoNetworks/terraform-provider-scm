...
// RESOURCE for SCM IpsecCryptoProfile (Package: network_services)
var (
...
// IpsecCryptoProfileResource defines the resource implementation.
...
func (r *IpsecCryptoProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.IpsecCryptoProfilesResourceSchema
}
...
func (r *IpsecCryptoProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for IpsecCryptoProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IpsecCryptoProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackIpsecCryptoProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating ipsec_crypto_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.IPsecCryptoProfilesAPI.CreateIPsecCryptoProfiles(ctx).IpsecCryptoProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating ipsec_crypto_profiles", err.Error())
	...
	packedObject, diags := packIpsecCryptoProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created ipsec_crypto_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *IpsecCryptoProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.IpsecCryptoProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for IpsecCryptoProfile")
	var data, savestate models.IpsecCryptoProfiles
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ipsec_crypto_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading ipsec_crypto_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIpsecCryptoProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *IpsecCryptoProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.IpsecCryptoProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for IpsecCryptoProfile")
	var plan, state models.IpsecCryptoProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IpsecCryptoProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating ipsec_crypto_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.IPsecCryptoProfilesAPI.UpdateIPsecCryptoProfilesByID(ctx, objectId).IpsecCryptoProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ipsec_crypto_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating ipsec_crypto_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIpsecCryptoProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated ipsec_crypto_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *IpsecCryptoProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting ipsec_crypto_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.IPsecCryptoProfilesAPI.DeleteIPsecCryptoProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting ipsec_crypto_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.IPsecCryptoProfilesAPI.GetIPsecCryptoProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
