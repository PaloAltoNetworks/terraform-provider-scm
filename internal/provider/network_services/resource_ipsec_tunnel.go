...
// RESOURCE for SCM IpsecTunnel (Package: network_services)
var (
...
func (r *IpsecTunnelResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.IpsecTunnelsResourceSchema
}
...
func (r *IpsecTunnelResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for IpsecTunnel")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IpsecTunnels{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackIpsecTunnelsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating ipsec_tunnels on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.IPsecTunnelsAPI.CreateIPsecTunnels(ctx).IpsecTunnels(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating ipsec_tunnels", err.Error())
	...
	packedObject, diags := packIpsecTunnelsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created ipsec_tunnels", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *IpsecTunnelResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.IpsecTunnels - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for IpsecTunnel")
	var data, savestate models.IpsecTunnels
	...
	tflog.Debug(ctx, "Reading ipsec_tunnels from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.IPsecTunnelsAPI.GetIPsecTunnelsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ipsec_tunnels on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading ipsec_tunnels", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIpsecTunnelsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.IpsecTunnels{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *IpsecTunnelResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.IpsecTunnels which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for IpsecTunnel")
	var plan, state models.IpsecTunnels
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IpsecTunnels{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating ipsec_tunnels on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.IPsecTunnelsAPI.UpdateIPsecTunnelsByID(ctx, objectId).IpsecTunnels(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no ipsec_tunnels on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating ipsec_tunnels", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIpsecTunnelsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated ipsec_tunnels", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *IpsecTunnelResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting ipsec_tunnels", map[string]interface{}{"id": objectId})
	deleteReq := r.client.IPsecTunnelsAPI.DeleteIPsecTunnelsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting ipsec_tunnels", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
