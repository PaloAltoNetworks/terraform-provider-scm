...
// RESOURCE for SCM IptagMatchList (Package: network_services)
var (
	_ resource.Resource                = &IptagMatchListResource{}
	_ resource.ResourceWithConfigure   = &IptagMatchListResource{}
	_ resource.ResourceWithImportState = &IptagMatchListResource{}
)

func NewIptagMatchListResource() resource.Resource {
	return &IptagMatchListResource{}
}

// IptagMatchListResource defines the resource implementation.
type IptagMatchListResource struct {
	client *network_services.APIClient
}

func (r *IptagMatchListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_iptag_match_list"
}

func (r *IptagMatchListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.IptagMatchListResourceSchema
}

func (r *IptagMatchListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *IptagMatchListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for IptagMatchList")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IptagMatchList{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackIptagMatchListToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating iptag_match_list on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.IptagMatchListAPI.CreateIptagMatchList(ctx).IptagMatchList(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating iptag_match_list", err.Error())
	...
	packedObject, diags := packIptagMatchListFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created iptag_match_list", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *IptagMatchListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.IptagMatchList - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for IptagMatchList")
	var data, savestate models.IptagMatchList
	...
	tflog.Debug(ctx, "Reading iptag_match_list from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.IptagMatchListAPI.GetIptagMatchListByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no iptag_match_list on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading iptag_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIptagMatchListFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.IptagMatchList{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *IptagMatchListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.IptagMatchList which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for IptagMatchList")
	var plan, state models.IptagMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.IptagMatchList{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating iptag_match_list on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.IptagMatchListAPI.UpdateIptagMatchListByID(ctx, objectId).IptagMatchList(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no iptag_match_list on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating iptag_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packIptagMatchListFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated iptag_match_list", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *IptagMatchListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.IptagMatchList
	...
	tflog.Debug(ctx, "Deleting iptag_match_list", map[string]interface{}{"id": objectId})
	deleteReq := r.client.IptagMatchListAPI.DeleteIptagMatchListByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting iptag_match_list", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *IptagMatchListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
