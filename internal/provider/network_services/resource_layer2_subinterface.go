...
// RESOURCE for SCM Layer2Subinterface (Package: network_services)
var (
	_ resource.Resource                = &Layer2SubinterfaceResource{}
	_ resource.ResourceWithConfigure   = &Layer2SubinterfaceResource{}
	_ resource.ResourceWithImportState = &Layer2SubinterfaceResource{}
)

func NewLayer2SubinterfaceResource() resource.Resource {
	return &Layer2SubinterfaceResource{}
}

// Layer2SubinterfaceResource defines the resource implementation.
type Layer2SubinterfaceResource struct {
	client *network_services.APIClient
}

func (r *Layer2SubinterfaceResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_layer2_subinterface"
}

func (r *Layer2SubinterfaceResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.Layer2SubinterfacesResourceSchema
}

func (r *Layer2SubinterfaceResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *Layer2SubinterfaceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Layer2Subinterface")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Layer2Subinterfaces{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackLayer2SubinterfacesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating layer2_subinterfaces on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.Layer2SubinterfacesAPI.CreateLayer2Subinterfaces(ctx).Layer2Subinterfaces(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating layer2_subinterfaces", err.Error())
	...
	packedObject, diags := packLayer2SubinterfacesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created layer2_subinterfaces", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *Layer2SubinterfaceResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Layer2Subinterfaces - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Layer2Subinterface")
	var data, savestate models.Layer2Subinterfaces
	...
	tflog.Debug(ctx, "Reading layer2_subinterfaces from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.Layer2SubinterfacesAPI.GetLayer2SubinterfacesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no layer2_subinterfaces on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading layer2_subinterfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLayer2SubinterfacesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *Layer2SubinterfaceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.Layer2Subinterfaces which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Layer2Subinterface")
	var plan, state models.Layer2Subinterfaces
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Layer2Subinterfaces{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating layer2_subinterfaces on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.Layer2SubinterfacesAPI.UpdateLayer2SubinterfacesByID(ctx, objectId).Layer2Subinterfaces(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no layer2_subinterfaces on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating layer2_subinterfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLayer2SubinterfacesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated layer2_subinterfaces", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *Layer2SubinterfaceResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.Layer2Subinterfaces
	...
	tflog.Debug(ctx, "Deleting layer2_subinterfaces", map[string]interface{}{"id": objectId})
	deleteReq := r.client.Layer2SubinterfacesAPI.DeleteLayer2SubinterfacesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting layer2_subinterfaces", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.Layer2SubinterfacesAPI.GetLayer2SubinterfacesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *Layer2SubinterfaceResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
