...
// RESOURCE for SCM LinkTag (Package: network_services)
var (
	_ resource.Resource                = &LinkTagResource{}
	_ resource.ResourceWithConfigure   = &LinkTagResource{}
	_ resource.ResourceWithImportState = &LinkTagResource{}
)

func NewLinkTagResource() resource.Resource {
	return &LinkTagResource{}
}

// LinkTagResource defines the resource implementation.
type LinkTagResource struct {
	client *network_services.APIClient
}

func (r *LinkTagResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_link_tag"
}

func (r *LinkTagResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.LinkTagsResourceSchema
}

func (r *LinkTagResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *LinkTagResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for LinkTag")
	var data models.LinkTags
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LinkTags{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackLinkTagsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating link_tags on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.LinkTagsAPI.CreateLinkTags(ctx).LinkTags(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating link_tags", err.Error())
	...
	packedObject, diags := packLinkTagsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created link_tags", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *LinkTagResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.LinkTags - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for LinkTag")
	var data, savestate models.LinkTags
	...
	tflog.Debug(ctx, "Reading link_tags from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.LinkTagsAPI.GetLinkTagsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no link_tags on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading link_tags", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLinkTagsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.LinkTags{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *LinkTagResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.LinkTags which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for LinkTag")
	var plan, state models.LinkTags
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LinkTags{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating link_tags on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.LinkTagsAPI.UpdateLinkTagsByID(ctx, objectId).LinkTags(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no link_tags on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating link_tags", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLinkTagsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated link_tags", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *LinkTagResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.LinkTags
	...
	tflog.Debug(ctx, "Deleting link_tags", map[string]interface{}{"id": objectId})
	deleteReq := r.client.LinkTagsAPI.DeleteLinkTagsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting link_tags", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
