...
// RESOURCE for SCM LogicalRouter (Package: network_services)
var (
...
// LogicalRouterResource defines the resource implementation.
...
func (r *LogicalRouterResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.LogicalRoutersResourceSchema
}
...
func (r *LogicalRouterResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for LogicalRouter")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LogicalRouters{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackLogicalRoutersToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating logical_routers on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.LogicalRoutersAPI.CreateLogicalRouters(ctx).LogicalRouters(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating logical_routers", err.Error())
	...
	packedObject, diags := packLogicalRoutersFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created logical_routers", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *LogicalRouterResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.LogicalRouters - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for LogicalRouter")
	var data, savestate models.LogicalRouters
	...
	tflog.Debug(ctx, "Reading logical_routers from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.LogicalRoutersAPI.GetLogicalRoutersByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no logical_routers on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading logical_routers", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLogicalRoutersFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.LogicalRouters{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *LogicalRouterResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.LogicalRouters which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for LogicalRouter")
	var plan, state models.LogicalRouters
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LogicalRouters{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating logical_routers on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.LogicalRoutersAPI.UpdateLogicalRoutersByID(ctx, objectId).LogicalRouters(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no logical_routers on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating logical_routers", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLogicalRoutersFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated logical_routers", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *LogicalRouterResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting logical_routers", map[string]interface{}{"id": objectId})
	deleteReq := r.client.LogicalRoutersAPI.DeleteLogicalRoutersByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting logical_routers", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
