...
// RESOURCE for SCM PbfRule (Package: network_services)
var (
	_ resource.Resource                = &PbfRuleResource{}
	_ resource.ResourceWithConfigure   = &PbfRuleResource{}
	_ resource.ResourceWithImportState = &PbfRuleResource{}
)

func NewPbfRuleResource() resource.Resource {
	return &PbfRuleResource{}
}

// PbfRuleResource defines the resource implementation.
type PbfRuleResource struct {
	client *network_services.APIClient
}

func (r *PbfRuleResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_pbf_rule"
}

func (r *PbfRuleResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.PbfRulesResourceSchema
}

func (r *PbfRuleResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *PbfRuleResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for PbfRule")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.PbfRules{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackPbfRulesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating pbf_rules on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.PBFRulesAPI.CreatePBFRules(ctx).PbfRules(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating pbf_rules", err.Error())
	...
	packedObject, diags := packPbfRulesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created pbf_rules", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *PbfRuleResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.PbfRules - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for PbfRule")
	var data, savestate models.PbfRules
	...
	tflog.Debug(ctx, "Reading pbf_rules from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.PBFRulesAPI.GetPBFRulesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no pbf_rules on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading pbf_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packPbfRulesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.PbfRules{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *PbfRuleResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.PbfRules which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for PbfRule")
	var plan, state models.PbfRules
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.PbfRules{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating pbf_rules on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.PBFRulesAPI.UpdatePBFRulesByID(ctx, objectId).PbfRules(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no pbf_rules on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating pbf_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packPbfRulesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated pbf_rules", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *PbfRuleResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.PbfRules
	...
	tflog.Debug(ctx, "Deleting pbf_rules", map[string]interface{}{"id": objectId})
	deleteReq := r.client.PBFRulesAPI.DeletePBFRulesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting pbf_rules", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *PbfRuleResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
