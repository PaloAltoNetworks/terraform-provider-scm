...
// RESOURCE for SCM QosProfile (Package: network_services)
var (
...
// QosProfileResource defines the resource implementation.
...
func (r *QosProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.QosProfilesResourceSchema
}

func (r *QosProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *QosProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for QosProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.QosProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackQosProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating qos_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.QoSProfilesAPI.CreateQoSProfiles(ctx).QosProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating qos_profiles", err.Error())
	...
	packedObject, diags := packQosProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created qos_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *QosProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.QosProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for QosProfile")
	var data, savestate models.QosProfiles
	...
	tflog.Debug(ctx, "Reading qos_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.QoSProfilesAPI.GetQoSProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no qos_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading qos_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packQosProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.QosProfiles{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *QosProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.QosProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for QosProfile")
	var plan, state models.QosProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.QosProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating qos_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.QoSProfilesAPI.UpdateQoSProfilesByID(ctx, objectId).QosProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no qos_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating qos_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packQosProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated qos_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
