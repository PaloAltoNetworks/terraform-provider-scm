...
// RESOURCE for SCM RouteCommunityList (Package: network_services)
var (
	_ resource.Resource                = &RouteCommunityListResource{}
	_ resource.ResourceWithConfigure   = &RouteCommunityListResource{}
	_ resource.ResourceWithImportState = &RouteCommunityListResource{}
)

func NewRouteCommunityListResource() resource.Resource {
	return &RouteCommunityListResource{}
}

// RouteCommunityListResource defines the resource implementation.
type RouteCommunityListResource struct {
	client *network_services.APIClient
}

func (r *RouteCommunityListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_route_community_list"
}

func (r *RouteCommunityListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.RouteCommunityListsResourceSchema
}

func (r *RouteCommunityListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *RouteCommunityListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for RouteCommunityList")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.RouteCommunityLists{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackRouteCommunityListsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating route_community_lists on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.RouteCommunityListsAPI.CreateRouteCommunityLists(ctx).RouteCommunityLists(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating route_community_lists", err.Error())
	...
	packedObject, diags := packRouteCommunityListsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created route_community_lists", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *RouteCommunityListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.RouteCommunityLists - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for RouteCommunityList")
	var data, savestate models.RouteCommunityLists
	...
	tflog.Debug(ctx, "Reading route_community_lists from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.RouteCommunityListsAPI.GetRouteCommunityListsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no route_community_lists on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading route_community_lists", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packRouteCommunityListsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *RouteCommunityListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.RouteCommunityLists which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for RouteCommunityList")
	var plan, state models.RouteCommunityLists
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.RouteCommunityLists{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating route_community_lists on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.RouteCommunityListsAPI.UpdateRouteCommunityListsByID(ctx, objectId).RouteCommunityLists(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no route_community_lists on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating route_community_lists", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packRouteCommunityListsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated route_community_lists", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *RouteCommunityListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.RouteCommunityLists
	...
	tflog.Debug(ctx, "Deleting route_community_lists", map[string]interface{}{"id": objectId})
	deleteReq := r.client.RouteCommunityListsAPI.DeleteRouteCommunityListsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting route_community_lists", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.RouteCommunityListsAPI.GetRouteCommunityListsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *RouteCommunityListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
