...
// RESOURCE for SCM RoutePathAccessList (Package: network_services)
var (
	_ resource.Resource                = &RoutePathAccessListResource{}
	_ resource.ResourceWithConfigure   = &RoutePathAccessListResource{}
	_ resource.ResourceWithImportState = &RoutePathAccessListResource{}
)

func NewRoutePathAccessListResource() resource.Resource {
	return &RoutePathAccessListResource{}
}

// RoutePathAccessListResource defines the resource implementation.
type RoutePathAccessListResource struct {
	client *network_services.APIClient
}

func (r *RoutePathAccessListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_route_path_access_list"
}

func (r *RoutePathAccessListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.RoutePathAccessListsResourceSchema
}

func (r *RoutePathAccessListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *RoutePathAccessListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for RoutePathAccessList")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.RoutePathAccessLists{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackRoutePathAccessListsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating route_path_access_lists on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.RoutePathAccessListsAPI.CreateRoutePathAccessLists(ctx).RoutePathAccessLists(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating route_path_access_lists", err.Error())
	...
	packedObject, diags := packRoutePathAccessListsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created route_path_access_lists", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *RoutePathAccessListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.RoutePathAccessLists - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for RoutePathAccessList")
	var data, savestate models.RoutePathAccessLists
	...
	tflog.Debug(ctx, "Reading route_path_access_lists from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.RoutePathAccessListsAPI.GetRoutePathAccessListsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no route_path_access_lists on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading route_path_access_lists", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packRoutePathAccessListsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *RoutePathAccessListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.RoutePathAccessLists which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for RoutePathAccessList")
	var plan, state models.RoutePathAccessLists
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.RoutePathAccessLists{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating route_path_access_lists on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.RoutePathAccessListsAPI.UpdateRoutePathAccessListsByID(ctx, objectId).RoutePathAccessLists(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no route_path_access_lists on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating route_path_access_lists", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packRoutePathAccessListsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated route_path_access_lists", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *RoutePathAccessListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.RoutePathAccessLists
	...
	tflog.Debug(ctx, "Deleting route_path_access_lists", map[string]interface{}{"id": objectId})
	deleteReq := r.client.RoutePathAccessListsAPI.DeleteRoutePathAccessListsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting route_path_access_lists", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.RoutePathAccessListsAPI.GetRoutePathAccessListsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *RoutePathAccessListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
