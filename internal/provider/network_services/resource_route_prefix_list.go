...
// RESOURCE for SCM RoutePrefixList (Package: network_services)
var (
	_ resource.Resource                = &RoutePrefixListResource{}
	_ resource.ResourceWithConfigure   = &RoutePrefixListResource{}
	_ resource.ResourceWithImportState = &RoutePrefixListResource{}
)

func NewRoutePrefixListResource() resource.Resource {
	return &RoutePrefixListResource{}
}

// RoutePrefixListResource defines the resource implementation.
type RoutePrefixListResource struct {
	client *network_services.APIClient
}

func (r *RoutePrefixListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_route_prefix_list"
}

func (r *RoutePrefixListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.RoutePrefixListsResourceSchema
}

func (r *RoutePrefixListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *RoutePrefixListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for RoutePrefixList")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.RoutePrefixLists{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackRoutePrefixListsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating route_prefix_lists on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.RoutePrefixListsAPI.CreateRoutePrefixLists(ctx).RoutePrefixLists(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating route_prefix_lists", err.Error())
	...
	packedObject, diags := packRoutePrefixListsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created route_prefix_lists", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *RoutePrefixListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.RoutePrefixLists - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for RoutePrefixList")
	var data, savestate models.RoutePrefixLists
	...
	tflog.Debug(ctx, "Reading route_prefix_lists from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.RoutePrefixListsAPI.GetRoutePrefixListsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no route_prefix_lists on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading route_prefix_lists", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packRoutePrefixListsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.RoutePrefixLists{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *RoutePrefixListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.RoutePrefixLists which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for RoutePrefixList")
	var plan, state models.RoutePrefixLists
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.RoutePrefixLists{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating route_prefix_lists on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.RoutePrefixListsAPI.UpdateRoutePrefixListsByID(ctx, objectId).RoutePrefixLists(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no route_prefix_lists on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating route_prefix_lists", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packRoutePrefixListsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated route_prefix_lists", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *RoutePrefixListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.RoutePrefixLists
	...
	tflog.Debug(ctx, "Deleting route_prefix_lists", map[string]interface{}{"id": objectId})
	deleteReq := r.client.RoutePrefixListsAPI.DeleteRoutePrefixListsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting route_prefix_lists", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *RoutePrefixListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
