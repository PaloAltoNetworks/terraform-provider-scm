...
// RESOURCE for SCM SdwanErrorCorrectionProfile (Package: network_services)
var (
	_ resource.Resource                = &SdwanErrorCorrectionProfileResource{}
	_ resource.ResourceWithConfigure   = &SdwanErrorCorrectionProfileResource{}
	_ resource.ResourceWithImportState = &SdwanErrorCorrectionProfileResource{}
)

func NewSdwanErrorCorrectionProfileResource() resource.Resource {
	return &SdwanErrorCorrectionProfileResource{}
}

// SdwanErrorCorrectionProfileResource defines the resource implementation.
type SdwanErrorCorrectionProfileResource struct {
	client *network_services.APIClient
}

func (r *SdwanErrorCorrectionProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_sdwan_error_correction_profile"
}

func (r *SdwanErrorCorrectionProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SdwanErrorCorrectionProfilesResourceSchema
}

func (r *SdwanErrorCorrectionProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *SdwanErrorCorrectionProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SdwanErrorCorrectionProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SdwanErrorCorrectionProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSdwanErrorCorrectionProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating sdwan_error_correction_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SDWANErrorCorrectionProfilesAPI.CreateSDWANErrorCorrectionProfiles(ctx).SdwanErrorCorrectionProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating sdwan_error_correction_profiles", err.Error())
	...
	packedObject, diags := packSdwanErrorCorrectionProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created sdwan_error_correction_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SdwanErrorCorrectionProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.SdwanErrorCorrectionProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for SdwanErrorCorrectionProfile")
	var data, savestate models.SdwanErrorCorrectionProfiles
	...
	tflog.Debug(ctx, "Reading sdwan_error_correction_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SDWANErrorCorrectionProfilesAPI.GetSDWANErrorCorrectionProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no sdwan_error_correction_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading sdwan_error_correction_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSdwanErrorCorrectionProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SdwanErrorCorrectionProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.SdwanErrorCorrectionProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for SdwanErrorCorrectionProfile")
	var plan, state models.SdwanErrorCorrectionProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SdwanErrorCorrectionProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating sdwan_error_correction_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SDWANErrorCorrectionProfilesAPI.UpdateSDWANErrorCorrectionProfilesByID(ctx, objectId).SdwanErrorCorrectionProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no sdwan_error_correction_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating sdwan_error_correction_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSdwanErrorCorrectionProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated sdwan_error_correction_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *SdwanErrorCorrectionProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.SdwanErrorCorrectionProfiles
	...
	tflog.Debug(ctx, "Deleting sdwan_error_correction_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SDWANErrorCorrectionProfilesAPI.DeleteSDWANErrorCorrectionProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting sdwan_error_correction_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.SDWANErrorCorrectionProfilesAPI.GetSDWANErrorCorrectionProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *SdwanErrorCorrectionProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
