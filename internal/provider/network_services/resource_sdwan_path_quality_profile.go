...
// RESOURCE for SCM SdwanPathQualityProfile (Package: network_services)
var (
	_ resource.Resource                = &SdwanPathQualityProfileResource{}
	_ resource.ResourceWithConfigure   = &SdwanPathQualityProfileResource{}
	_ resource.ResourceWithImportState = &SdwanPathQualityProfileResource{}
)

func NewSdwanPathQualityProfileResource() resource.Resource {
	return &SdwanPathQualityProfileResource{}
}

// SdwanPathQualityProfileResource defines the resource implementation.
type SdwanPathQualityProfileResource struct {
	client *network_services.APIClient
}

func (r *SdwanPathQualityProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_sdwan_path_quality_profile"
}

func (r *SdwanPathQualityProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SdwanPathQualityProfilesResourceSchema
}

func (r *SdwanPathQualityProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *SdwanPathQualityProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SdwanPathQualityProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SdwanPathQualityProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSdwanPathQualityProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating sdwan_path_quality_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SDWANPathQualityProfilesAPI.CreateSDWANPathQualityProfiles(ctx).SdwanPathQualityProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating sdwan_path_quality_profiles", err.Error())
	...
	packedObject, diags := packSdwanPathQualityProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created sdwan_path_quality_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SdwanPathQualityProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.SdwanPathQualityProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for SdwanPathQualityProfile")
	var data, savestate models.SdwanPathQualityProfiles
	...
	tflog.Debug(ctx, "Reading sdwan_path_quality_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SDWANPathQualityProfilesAPI.GetSDWANPathQualityProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no sdwan_path_quality_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading sdwan_path_quality_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSdwanPathQualityProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SdwanPathQualityProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.SdwanPathQualityProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for SdwanPathQualityProfile")
	var plan, state models.SdwanPathQualityProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SdwanPathQualityProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating sdwan_path_quality_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SDWANPathQualityProfilesAPI.UpdateSDWANPathQualityProfilesByID(ctx, objectId).SdwanPathQualityProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no sdwan_path_quality_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating sdwan_path_quality_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSdwanPathQualityProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated sdwan_path_quality_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *SdwanPathQualityProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.SdwanPathQualityProfiles
	...
	tflog.Debug(ctx, "Deleting sdwan_path_quality_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SDWANPathQualityProfilesAPI.DeleteSDWANPathQualityProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting sdwan_path_quality_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.SDWANPathQualityProfilesAPI.GetSDWANPathQualityProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *SdwanPathQualityProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
