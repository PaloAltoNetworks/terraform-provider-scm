...
// RESOURCE for SCM SdwanRule (Package: network_services)
var (
	_ resource.Resource                = &SdwanRuleResource{}
	_ resource.ResourceWithConfigure   = &SdwanRuleResource{}
	_ resource.ResourceWithImportState = &SdwanRuleResource{}
)

func NewSdwanRuleResource() resource.Resource {
	return &SdwanRuleResource{}
}

// SdwanRuleResource defines the resource implementation.
type SdwanRuleResource struct {
	client *network_services.APIClient
}

func (r *SdwanRuleResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_sdwan_rule"
}

func (r *SdwanRuleResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SdwanRulesResourceSchema
}

func (r *SdwanRuleResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *SdwanRuleResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SdwanRule")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SdwanRules{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSdwanRulesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating sdwan_rules on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SDWANRulesAPI.CreateSDWANRules(ctx).SdwanRules(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating sdwan_rules", err.Error())
	...
	packedObject, diags := packSdwanRulesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created sdwan_rules", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SdwanRuleResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.SdwanRules - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for SdwanRule")
	var data, savestate models.SdwanRules
	...
	tflog.Debug(ctx, "Reading sdwan_rules from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SDWANRulesAPI.GetSDWANRulesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no sdwan_rules on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading sdwan_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSdwanRulesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.SdwanRules{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SdwanRuleResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.SdwanRules which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for SdwanRule")
	var plan, state models.SdwanRules
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SdwanRules{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating sdwan_rules on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SDWANRulesAPI.UpdateSDWANRulesByID(ctx, objectId).SdwanRules(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no sdwan_rules on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating sdwan_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSdwanRulesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated sdwan_rules", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *SdwanRuleResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.SdwanRules
	...
	tflog.Debug(ctx, "Deleting sdwan_rules", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SDWANRulesAPI.DeleteSDWANRulesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting sdwan_rules", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *SdwanRuleResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
