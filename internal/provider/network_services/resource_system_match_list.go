...
// RESOURCE for SCM SystemMatchList (Package: network_services)
var (
	_ resource.Resource                = &SystemMatchListResource{}
	_ resource.ResourceWithConfigure   = &SystemMatchListResource{}
	_ resource.ResourceWithImportState = &SystemMatchListResource{}
)

func NewSystemMatchListResource() resource.Resource {
	return &SystemMatchListResource{}
}

// SystemMatchListResource defines the resource implementation.
type SystemMatchListResource struct {
	client *network_services.APIClient
}

func (r *SystemMatchListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_system_match_list"
}

func (r *SystemMatchListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SystemMatchListResourceSchema
}

func (r *SystemMatchListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *SystemMatchListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SystemMatchList")
	var data models.SystemMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SystemMatchList{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSystemMatchListToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating system_match_list on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SystemMatchListAPI.CreateSystemMatchList(ctx).SystemMatchList(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating system_match_list", err.Error())
	...
	packedObject, diags := packSystemMatchListFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created system_match_list", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SystemMatchListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.SystemMatchList - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for SystemMatchList")
	var data, savestate models.SystemMatchList
	...
	tflog.Debug(ctx, "Reading system_match_list from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SystemMatchListAPI.GetSystemMatchListByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no system_match_list on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading system_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSystemMatchListFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.SystemMatchList{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SystemMatchListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.SystemMatchList which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for SystemMatchList")
	var plan, state models.SystemMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SystemMatchList{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating system_match_list on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SystemMatchListAPI.UpdateSystemMatchListByID(ctx, objectId).SystemMatchList(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no system_match_list on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating system_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSystemMatchListFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated system_match_list", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *SystemMatchListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.SystemMatchList
	...
	tflog.Debug(ctx, "Deleting system_match_list", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SystemMatchListAPI.DeleteSystemMatchListByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting system_match_list", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *SystemMatchListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
