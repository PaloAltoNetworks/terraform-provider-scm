...
// RESOURCE for SCM TunnelInterface (Package: network_services)
var (
...
// TunnelInterfaceResource defines the resource implementation.
...
func (r *TunnelInterfaceResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.TunnelInterfacesResourceSchema
}
...
func (r *TunnelInterfaceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for TunnelInterface")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TunnelInterfaces{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackTunnelInterfacesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating tunnel_interfaces on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.TunnelInterfacesAPI.CreateTunnelInterfaces(ctx).TunnelInterfaces(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating tunnel_interfaces", err.Error())
	...
	packedObject, diags := packTunnelInterfacesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created tunnel_interfaces", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *TunnelInterfaceResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.TunnelInterfaces - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for TunnelInterface")
	var data, savestate models.TunnelInterfaces
	...
	tflog.Debug(ctx, "Reading tunnel_interfaces from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.TunnelInterfacesAPI.GetTunnelInterfacesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tunnel_interfaces on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading tunnel_interfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTunnelInterfacesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.TunnelInterfaces{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *TunnelInterfaceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.TunnelInterfaces which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for TunnelInterface")
	var plan, state models.TunnelInterfaces
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.TunnelInterfaces{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating tunnel_interfaces on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.TunnelInterfacesAPI.UpdateTunnelInterfacesByID(ctx, objectId).TunnelInterfaces(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tunnel_interfaces on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating tunnel_interfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packTunnelInterfacesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated tunnel_interfaces", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *TunnelInterfaceResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting tunnel_interfaces", map[string]interface{}{"id": objectId})
	deleteReq := r.client.TunnelInterfacesAPI.DeleteTunnelInterfacesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting tunnel_interfaces", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
