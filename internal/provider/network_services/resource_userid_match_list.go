...
// RESOURCE for SCM UseridMatchList (Package: network_services)
var (
	_ resource.Resource                = &UseridMatchListResource{}
	_ resource.ResourceWithConfigure   = &UseridMatchListResource{}
	_ resource.ResourceWithImportState = &UseridMatchListResource{}
)

func NewUseridMatchListResource() resource.Resource {
	return &UseridMatchListResource{}
}

// UseridMatchListResource defines the resource implementation.
type UseridMatchListResource struct {
	client *network_services.APIClient
}

func (r *UseridMatchListResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_userid_match_list"
}

func (r *UseridMatchListResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.UseridMatchListResourceSchema
}

func (r *UseridMatchListResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *UseridMatchListResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for UseridMatchList")
	var data models.UseridMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UseridMatchList{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackUseridMatchListToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating userid_match_list on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.UseridMatchListAPI.CreateUseridMatchList(ctx).UseridMatchList(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating userid_match_list", err.Error())
	...
	packedObject, diags := packUseridMatchListFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created userid_match_list", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *UseridMatchListResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.UseridMatchList - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for UseridMatchList")
	var data, savestate models.UseridMatchList
	...
	tflog.Debug(ctx, "Reading userid_match_list from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.UseridMatchListAPI.GetUseridMatchListByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no userid_match_list on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading userid_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packUseridMatchListFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.UseridMatchList{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *UseridMatchListResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.UseridMatchList which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for UseridMatchList")
	var plan, state models.UseridMatchList
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UseridMatchList{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating userid_match_list on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.UseridMatchListAPI.UpdateUseridMatchListByID(ctx, objectId).UseridMatchList(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no userid_match_list on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating userid_match_list", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packUseridMatchListFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated userid_match_list", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *UseridMatchListResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.UseridMatchList
	...
	tflog.Debug(ctx, "Deleting userid_match_list", map[string]interface{}{"id": objectId})
	deleteReq := r.client.UseridMatchListAPI.DeleteUseridMatchListByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting userid_match_list", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *UseridMatchListResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
