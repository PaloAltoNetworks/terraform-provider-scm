...
// RESOURCE for SCM VlanInterface (Package: network_services)
var (
...
// VlanInterfaceResource defines the resource implementation.
...
func (r *VlanInterfaceResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.VlanInterfacesResourceSchema
}
...
func (r *VlanInterfaceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for VlanInterface")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.VlanInterfaces{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackVlanInterfacesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating vlan_interfaces on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.VLANInterfacesAPI.CreateVLANInterfaces(ctx).VlanInterfaces(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating vlan_interfaces", err.Error())
	...
	packedObject, diags := packVlanInterfacesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created vlan_interfaces", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *VlanInterfaceResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.VlanInterfaces - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for VlanInterface")
	var data, savestate models.VlanInterfaces
	...
	tflog.Debug(ctx, "Reading vlan_interfaces from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.VLANInterfacesAPI.GetVLANInterfacesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no vlan_interfaces on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading vlan_interfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packVlanInterfacesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.VlanInterfaces{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *VlanInterfaceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.VlanInterfaces which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for VlanInterface")
	var plan, state models.VlanInterfaces
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.VlanInterfaces{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating vlan_interfaces on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.VLANInterfacesAPI.UpdateVLANlInterfacesByID(ctx, objectId).VlanInterfaces(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no vlan_interfaces on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating vlan_interfaces", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packVlanInterfacesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated vlan_interfaces", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *VlanInterfaceResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting vlan_interfaces", map[string]interface{}{"id": objectId})
	deleteReq := r.client.VLANInterfacesAPI.DeleteVLANInterfacesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting vlan_interfaces", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
