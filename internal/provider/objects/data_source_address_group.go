...
// DATA SOURCE for SCM AddressGroup (Package: objects)
var (
...
// AddressGroupDataSource defines the data source implementation.
...
func (d *AddressGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: AddressGroupDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.AddressGroupsDataSourceSchema
}

func (d *AddressGroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: AddressGroupsDataSource.Configure ---")
...
func (d *AddressGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: AddressGroupDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading AddressGroups data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.AddressGroupsAPI.GetAddressGroupsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading AddressGroups", fmt.Sprintf("Could not read AddressGroups with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading AddressGroups data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.AddressGroupsAPI.ListAddressGroups(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing AddressGroupss", fmt.Sprintf("Could not list AddressGroupss: %s", err.Error()))
			return
		}
		if httpRes.StatusCode != 200 {
			resp.Diagnostics.AddError("Unexpected HTTP status code", fmt.Sprintf("Expected 200, got %d", httpRes.StatusCode))
		...
		var foundObject *objects.AddressGroups
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("AddressGroups Not Found", fmt.Sprintf("No AddressGroups found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packAddressGroupsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the AddressGroups data source.")
		return
	}
