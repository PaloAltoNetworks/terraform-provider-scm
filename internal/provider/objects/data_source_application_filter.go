...
// DATA SOURCE for SCM ApplicationFilter (Package: objects)
var (
...
// ApplicationFilterDataSource defines the data source implementation.
...
func (d *ApplicationFilterDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ApplicationFilterDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ApplicationFiltersDataSourceSchema
}

func (d *ApplicationFilterDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ApplicationFiltersDataSource.Configure ---")
...
func (d *ApplicationFilterDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ApplicationFilterDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.ApplicationFiltersAPI.GetApplicationFiltersByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ApplicationFilters", fmt.Sprintf("Could not read ApplicationFilters with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading ApplicationFilters data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ApplicationFiltersAPI.ListApplicationFilters(ctx)
		...
		var foundObject *objects.ApplicationFilters
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ApplicationFilters Not Found", fmt.Sprintf("No ApplicationFilters found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packApplicationFiltersFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ApplicationFilters data source.")
		return
	}
