...
// DATA SOURCE for SCM ApplicationGroup (Package: objects)
var (
...
// ApplicationGroupDataSource defines the data source implementation.
...
func (d *ApplicationGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ApplicationGroupDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ApplicationGroupsDataSourceSchema
}

func (d *ApplicationGroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ApplicationGroupsDataSource.Configure ---")
...
func (d *ApplicationGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ApplicationGroupDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ApplicationGroups data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ApplicationGroupsAPI.GetApplicationGroupsByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading ApplicationGroups data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ApplicationGroupsAPI.ListApplicationGroups(ctx)
		...
		var foundObject *objects.ApplicationGroups
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ApplicationGroups Not Found", fmt.Sprintf("No ApplicationGroups found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packApplicationGroupsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ApplicationGroups data source.")
		return
	}
