...
// DATA SOURCE for SCM DynamicUserGroup (Package: objects)
var (
...
// DynamicUserGroupDataSource defines the data source implementation.
...
func (d *DynamicUserGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DynamicUserGroupDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DynamicUserGroupsDataSourceSchema
}

func (d *DynamicUserGroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DynamicUserGroupsDataSource.Configure ---")
...
func (d *DynamicUserGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DynamicUserGroupDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DynamicUserGroups data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DynamicUserGroupsAPI.GetDynamicUserGroupsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading DynamicUserGroups", fmt.Sprintf("Could not read DynamicUserGroups with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading DynamicUserGroups data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DynamicUserGroupsAPI.ListDynamicUserGroups(ctx)
		...
		var foundObject *objects.DynamicUserGroups
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DynamicUserGroups Not Found", fmt.Sprintf("No DynamicUserGroups found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDynamicUserGroupsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DynamicUserGroups data source.")
		return
	}
