...
// DATA SOURCE for SCM HipObject (Package: objects)
var (
...
// HipObjectDataSource defines the data source implementation.
type HipObjectDataSource struct {
	client *objects.APIClient
}

func (d *HipObjectDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: HipObjectDataSource.Metadata ---")
	resp.TypeName = "scm_hip_object"
}

func (d *HipObjectDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: HipObjectDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.HipObjectsDataSourceSchema
}

func (d *HipObjectDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: HipObjectsDataSource.Configure ---")
...
func (d *HipObjectDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: HipObjectDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading HipObjects data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.HIPObjectsAPI.GetHIPObjectsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading HipObjects", fmt.Sprintf("Could not read HipObjects with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading HipObjects data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.HIPObjectsAPI.ListHIPObjects(ctx)
		...
		var foundObject *objects.HipObjects
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("HipObjects Not Found", fmt.Sprintf("No HipObjects found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packHipObjectsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the HipObjects data source.")
		return
	}
