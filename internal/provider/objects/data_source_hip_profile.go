...
// HipProfileDataSource defines the data source implementation.
...
func (d *HipProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: HipProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.HipProfilesDataSourceSchema
}

func (d *HipProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: HipProfilesDataSource.Configure ---")
...
func (d *HipProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: HipProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading HipProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.HIPProfilesAPI.GetHIPProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading HipProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.HIPProfilesAPI.ListHIPProfiles(ctx)
		...
		var foundObject *objects.HipProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("HipProfiles Not Found", fmt.Sprintf("No HipProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packHipProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the HipProfiles data source.")
		return
	}
