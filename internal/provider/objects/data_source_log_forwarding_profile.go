...
// LogForwardingProfileDataSource defines the data source implementation.
...
func (d *LogForwardingProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: LogForwardingProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.LogForwardingProfilesDataSourceSchema
}

func (d *LogForwardingProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: LogForwardingProfilesDataSource.Configure ---")
...
func (d *LogForwardingProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LogForwardingProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading LogForwardingProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.LogForwardingProfilesAPI.GetLogForwardingProfilesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading LogForwardingProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.LogForwardingProfilesAPI.ListLogForwardingProfiles(ctx)
		...
		var foundObject *objects.LogForwardingProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("LogForwardingProfiles Not Found", fmt.Sprintf("No LogForwardingProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packLogForwardingProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the LogForwardingProfiles data source.")
		return
	}
