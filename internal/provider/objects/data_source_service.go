...
// DATA SOURCE for SCM Service (Package: objects)
var (
...
func (d *ServiceDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceDataSource.Metadata ---")
...
func (d *ServiceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		objectId := data.Id.ValueString()
		tflog.Debug(ctx, "Reading Services data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ServicesAPI.GetServicesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading Services", fmt.Sprintf("Could not read Services with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading Services data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ServicesAPI.ListServices(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing Servicess", fmt.Sprintf("Could not list Servicess: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *objects.Services
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("Services Not Found", fmt.Sprintf("No Services found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packServicesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Services data source.")
		return
	}
