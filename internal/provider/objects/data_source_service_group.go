...
// DATA SOURCE for SCM ServiceGroup (Package: objects)
var (
...
// ServiceGroupDataSource defines the data source implementation.
...
func (d *ServiceGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceGroupDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ServiceGroupsDataSourceSchema
}
...
func (d *ServiceGroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceGroupsDataSource.Configure ---")
...
func (d *ServiceGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ServiceGroupDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading ServiceGroups data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ServiceGroupsAPI.GetServiceGroupsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ServiceGroups", fmt.Sprintf("Could not read ServiceGroups with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading ServiceGroups data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ServiceGroupsAPI.ListServiceGroups(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing ServiceGroupss", fmt.Sprintf("Could not list ServiceGroupss: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *objects.ServiceGroups
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ServiceGroups Not Found", fmt.Sprintf("No ServiceGroups found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packServiceGroupsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ServiceGroups data source.")
		return
	}
