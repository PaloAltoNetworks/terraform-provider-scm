...
// DATA SOURCE for SCM SyslogServerProfile (Package: objects)
var (
	_ datasource.DataSource              = &SyslogServerProfileDataSource{}
	_ datasource.DataSourceWithConfigure = &SyslogServerProfileDataSource{}
)

func NewSyslogServerProfileDataSource() datasource.DataSource {
	return &SyslogServerProfileDataSource{}
}

// SyslogServerProfileDataSource defines the data source implementation.
type SyslogServerProfileDataSource struct {
...
func (d *SyslogServerProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: SyslogServerProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.SyslogServerProfilesDataSourceSchema
}

func (d *SyslogServerProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: SyslogServerProfilesDataSource.Configure ---")
...
func (d *SyslogServerProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: SyslogServerProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading SyslogServerProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.SyslogServerProfilesAPI.GetSyslogServerProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading SyslogServerProfiles", fmt.Sprintf("Could not read SyslogServerProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading SyslogServerProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.SyslogServerProfilesAPI.ListSyslogServerProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing SyslogServerProfiless", fmt.Sprintf("Could not list SyslogServerProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *objects.SyslogServerProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("SyslogServerProfiles Not Found", fmt.Sprintf("No SyslogServerProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packSyslogServerProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the SyslogServerProfiles data source.")
		return
	}
