...
// DATA SOURCE for SCM Tag (Package: objects)
var (
...
func (d *TagDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: TagDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading Tags data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.TagsAPI.GetTagsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading Tags", fmt.Sprintf("Could not read Tags with ID %s: %s", objectId, err.Error()))
			return
		}
		...
		tflog.Debug(ctx, "Reading Tags data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.TagsAPI.ListTags(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing Tagss", fmt.Sprintf("Could not list Tagss: %s", err.Error()))
			return
		}
		if httpRes.StatusCode != 200 {
			resp.Diagnostics.AddError("Unexpected HTTP status code", fmt.Sprintf("Expected 200, got %d", httpRes.StatusCode))
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("Tags Not Found", fmt.Sprintf("No Tags found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packTagsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Tags data source.")
		return
	}
