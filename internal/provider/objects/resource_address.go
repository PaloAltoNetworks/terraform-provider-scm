...
// RESOURCE for SCM Address (Package: objects)
var (
...
func (r *AddressResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AddressesResourceSchema
}
...
func (r *AddressResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Address")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Addresses{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackAddressesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating addresses on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.AddressesAPI.CreateAddresses(ctx).Addresses(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating addresses", err.Error())
	...
	packedObject, diags := packAddressesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created addresses", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *AddressResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Addresses - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Address")
	var data, savestate models.Addresses
	...
	tflog.Debug(ctx, "Reading addresses from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.AddressesAPI.GetAddressesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no addresses on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading addresses", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Addresses{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *AddressResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Addresses which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Address")
	var plan, state models.Addresses
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Addresses{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating addresses on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.AddressesAPI.UpdateAddressesByID(ctx, objectId).Addresses(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no addresses on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating addresses", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAddressesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated addresses", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *AddressResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting addresses", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AddressesAPI.DeleteAddressesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting addresses", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
