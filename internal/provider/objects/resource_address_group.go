...
// RESOURCE for SCM AddressGroup (Package: objects)
var (
...
// AddressGroupResource defines the resource implementation.
...
func (r *AddressGroupResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AddressGroupsResourceSchema
}
...
func (r *AddressGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for AddressGroup")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AddressGroups{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackAddressGroupsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating address_groups on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.AddressGroupsAPI.CreateAddressGroups(ctx).AddressGroups(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating address_groups", err.Error())
	...
	packedObject, diags := packAddressGroupsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created address_groups", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *AddressGroupResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.AddressGroups - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for AddressGroup")
	var data, savestate models.AddressGroups
	...
	tflog.Debug(ctx, "Reading address_groups from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.AddressGroupsAPI.GetAddressGroupsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no address_groups on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading address_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAddressGroupsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.AddressGroups{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *AddressGroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.AddressGroups which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for AddressGroup")
	var plan, state models.AddressGroups
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AddressGroups{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating address_groups on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.AddressGroupsAPI.UpdateAddressGroupsByID(ctx, objectId).AddressGroups(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no address_groups on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating address_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAddressGroupsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated address_groups", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *AddressGroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting address_groups", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AddressGroupsAPI.DeleteAddressGroupsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting address_groups", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
