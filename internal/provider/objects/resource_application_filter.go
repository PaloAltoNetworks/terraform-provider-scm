...
// RESOURCE for SCM ApplicationFilter (Package: objects)
var (
...
func (r *ApplicationFilterResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_application_filter"
}

func (r *ApplicationFilterResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ApplicationFiltersResourceSchema
}

func (r *ApplicationFilterResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *ApplicationFilterResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ApplicationFilter")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ApplicationFilters{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackApplicationFiltersToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating application_filters on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ApplicationFiltersAPI.CreateApplicationFilters(ctx).ApplicationFilters(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating application_filters", err.Error())
	...
	packedObject, diags := packApplicationFiltersFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created application_filters", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ApplicationFilterResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ApplicationFilters - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ApplicationFilter")
	var data, savestate models.ApplicationFilters
	...
	tflog.Debug(ctx, "Reading application_filters from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ApplicationFiltersAPI.GetApplicationFiltersByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no application_filters on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading application_filters", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packApplicationFiltersFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ApplicationFilterResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.ApplicationFilters which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ApplicationFilter")
	var plan, state models.ApplicationFilters
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ApplicationFilters{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating application_filters on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ApplicationFiltersAPI.UpdateApplicationFiltersByID(ctx, objectId).ApplicationFilters(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no application_filters on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating application_filters", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packApplicationFiltersFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated application_filters", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *ApplicationFilterResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting application_filters", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ApplicationFiltersAPI.DeleteApplicationFiltersByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting application_filters", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.ApplicationFiltersAPI.GetApplicationFiltersByID(ctx, objectId).Execute()
			if getErr != nil {
