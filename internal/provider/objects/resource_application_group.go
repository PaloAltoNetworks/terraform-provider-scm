...
// RESOURCE for SCM ApplicationGroup (Package: objects)
var (
...
// ApplicationGroupResource defines the resource implementation.
...
func (r *ApplicationGroupResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ApplicationGroupsResourceSchema
}
...
func (r *ApplicationGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ApplicationGroup")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ApplicationGroups{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackApplicationGroupsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating application_groups on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ApplicationGroupsAPI.CreateApplicationGroups(ctx).ApplicationGroups(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating application_groups", err.Error())
	...
	packedObject, diags := packApplicationGroupsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created application_groups", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ApplicationGroupResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ApplicationGroups - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ApplicationGroup")
	var data, savestate models.ApplicationGroups
	...
	tflog.Debug(ctx, "Reading application_groups from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ApplicationGroupsAPI.GetApplicationGroupsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no application_groups on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading application_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packApplicationGroupsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ApplicationGroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.ApplicationGroups which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ApplicationGroup")
	var plan, state models.ApplicationGroups
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ApplicationGroups{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating application_groups on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ApplicationGroupsAPI.UpdateApplicationGroupsByID(ctx, objectId).ApplicationGroups(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no application_groups on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating application_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packApplicationGroupsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated application_groups", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *ApplicationGroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting application_groups", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ApplicationGroupsAPI.DeleteApplicationGroupsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting application_groups", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.ApplicationGroupsAPI.GetApplicationGroupsByID(ctx, objectId).Execute()
			if getErr != nil {
