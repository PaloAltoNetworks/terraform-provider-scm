...
// RESOURCE for SCM HipObject (Package: objects)
var (
...
// HipObjectResource defines the resource implementation.
type HipObjectResource struct {
	client *objects.APIClient
}

func (r *HipObjectResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_hip_object"
}

func (r *HipObjectResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.HipObjectsResourceSchema
}

func (r *HipObjectResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *HipObjectResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for HipObject")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HipObjects{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackHipObjectsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating hip_objects on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.HIPObjectsAPI.CreateHIPObjects(ctx).HipObjects(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating hip_objects", err.Error())
	...
	packedObject, diags := packHipObjectsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created hip_objects", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *HipObjectResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.HipObjects - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for HipObject")
	var data, savestate models.HipObjects
	...
	tflog.Debug(ctx, "Reading hip_objects from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.HIPObjectsAPI.GetHIPObjectsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no hip_objects on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading hip_objects", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packHipObjectsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.HipObjects{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *HipObjectResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.HipObjects which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for HipObject")
	var plan, state models.HipObjects
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HipObjects{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating hip_objects on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.HIPObjectsAPI.UpdateHIPObjectsByID(ctx, objectId).HipObjects(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no hip_objects on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating hip_objects", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packHipObjectsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated hip_objects", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *HipObjectResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting hip_objects", map[string]interface{}{"id": objectId})
	deleteReq := r.client.HIPObjectsAPI.DeleteHIPObjectsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting hip_objects", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
