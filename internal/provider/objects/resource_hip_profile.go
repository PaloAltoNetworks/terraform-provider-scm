...
// HipProfileResource defines the resource implementation.
type HipProfileResource struct {
	client *objects.APIClient
}

func (r *HipProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_hip_profile"
}

func (r *HipProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.HipProfilesResourceSchema
}

func (r *HipProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *HipProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for HipProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HipProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackHipProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating hip_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.HIPProfilesAPI.CreateHIPProfiles(ctx).HipProfiles(*unpackedScmObject)
	...
	packedObject, diags := packHipProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created hip_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *HipProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.HipProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for HipProfile")
	var data, savestate models.HipProfiles
	...
	tflog.Debug(ctx, "Reading hip_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.HIPProfilesAPI.GetHIPProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no hip_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
	...
	packedObject, diags := packHipProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.HipProfiles{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *HipProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.HipProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for HipProfile")
	var plan, state models.HipProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HipProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating hip_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.HIPProfilesAPI.UpdateHIPProfilesByID(ctx, objectId).HipProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no hip_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
	...
	packedObject, diags := packHipProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated hip_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *HipProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting hip_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.HIPProfilesAPI.DeleteHIPProfilesByID(ctx, objectId)
