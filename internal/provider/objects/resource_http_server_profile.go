...
// RESOURCE for SCM HttpServerProfile (Package: objects)
var (
	_ resource.Resource                = &HttpServerProfileResource{}
	_ resource.ResourceWithConfigure   = &HttpServerProfileResource{}
	_ resource.ResourceWithImportState = &HttpServerProfileResource{}
)

func NewHttpServerProfileResource() resource.Resource {
	return &HttpServerProfileResource{}
}

// HttpServerProfileResource defines the resource implementation.
type HttpServerProfileResource struct {
...
func (r *HttpServerProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.HttpServerProfilesResourceSchema
}

func (r *HttpServerProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *HttpServerProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for HttpServerProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HttpServerProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackHttpServerProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating http_server_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.HTTPServerProfilesAPI.CreateHTTPServerProfiles(ctx).HttpServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating http_server_profiles", err.Error())
	...
	packedObject, diags := packHttpServerProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created http_server_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *HttpServerProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.HttpServerProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for HttpServerProfile")
	var data, savestate models.HttpServerProfiles
	...
	tflog.Debug(ctx, "Reading http_server_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.HTTPServerProfilesAPI.GetHTTPServerProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no http_server_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading http_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *HttpServerProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.HttpServerProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for HttpServerProfile")
	var plan, state models.HttpServerProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HttpServerProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating http_server_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.HTTPServerProfilesAPI.UpdateHTTPServerProfilesByID(ctx, objectId).HttpServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no http_server_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating http_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packHttpServerProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated http_server_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *HttpServerProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.HttpServerProfiles
	...
	tflog.Debug(ctx, "Deleting http_server_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.HTTPServerProfilesAPI.DeleteHTTPServerProfilesByID(ctx, objectId)
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.HTTPServerProfilesAPI.GetHTTPServerProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *HttpServerProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
