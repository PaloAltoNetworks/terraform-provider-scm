...
// RESOURCE for SCM LogForwardingProfile (Package: objects)
var (
...
// LogForwardingProfileResource defines the resource implementation.
...
func (r *LogForwardingProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.LogForwardingProfilesResourceSchema
}
...
func (r *LogForwardingProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for LogForwardingProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LogForwardingProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackLogForwardingProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating log_forwarding_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.LogForwardingProfilesAPI.CreateLogForwardingProfiles(ctx).LogForwardingProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating log_forwarding_profiles", err.Error())
	...
	packedObject, diags := packLogForwardingProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created log_forwarding_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *LogForwardingProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.LogForwardingProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for LogForwardingProfile")
	var data, savestate models.LogForwardingProfiles
	...
	tflog.Debug(ctx, "Reading log_forwarding_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.LogForwardingProfilesAPI.GetLogForwardingProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no log_forwarding_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading log_forwarding_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLogForwardingProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *LogForwardingProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.LogForwardingProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for LogForwardingProfile")
	var plan, state models.LogForwardingProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.LogForwardingProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating log_forwarding_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.LogForwardingProfilesAPI.UpdateLogForwardingProfilesByID(ctx, objectId).LogForwardingProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no log_forwarding_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating log_forwarding_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packLogForwardingProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated log_forwarding_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *LogForwardingProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting log_forwarding_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.LogForwardingProfilesAPI.DeleteLogForwardingProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting log_forwarding_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.LogForwardingProfilesAPI.GetLogForwardingProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
