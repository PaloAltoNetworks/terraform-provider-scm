...
// RESOURCE for SCM Service (Package: objects)
var (
...
func (r *ServiceResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_service"
}

func (r *ServiceResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ServicesResourceSchema
}
...
func (r *ServiceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Service")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Services{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackServicesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating services on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ServicesAPI.CreateServices(ctx).Services(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating services", err.Error())
	...
	packedObject, diags := packServicesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created services", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ServiceResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Services - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Service")
	var data, savestate models.Services
	...
	tflog.Debug(ctx, "Reading services from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ServicesAPI.GetServicesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no services on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading services", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packServicesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Services{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ServiceResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Services which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Service")
	var plan, state models.Services
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Services{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating services on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ServicesAPI.UpdateServicesByID(ctx, objectId).Services(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no services on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating services", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packServicesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated services", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *ServiceResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting services", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ServicesAPI.DeleteServicesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting services", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
