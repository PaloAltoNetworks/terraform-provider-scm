...
func (r *ServiceGroupResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ServiceGroupsResourceSchema
}
...
func (r *ServiceGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ServiceGroup")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ServiceGroups{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackServiceGroupsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating service_groups on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ServiceGroupsAPI.CreateServiceGroups(ctx).ServiceGroups(*unpackedScmObject)
	...
	packedObject, diags := packServiceGroupsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created service_groups", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ServiceGroupResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ServiceGroups - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ServiceGroup")
	var data, savestate models.ServiceGroups
	...
	tflog.Debug(ctx, "Reading service_groups from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ServiceGroupsAPI.GetServiceGroupsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no service_groups on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
	...
	packedObject, diags := packServiceGroupsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.ServiceGroups{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ServiceGroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.ServiceGroups which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ServiceGroup")
	var plan, state models.ServiceGroups
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ServiceGroups{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating service_groups on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ServiceGroupsAPI.UpdateServiceGroupsByID(ctx, objectId).ServiceGroups(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no service_groups on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
	...
	packedObject, diags := packServiceGroupsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated service_groups", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *ServiceGroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting service_groups", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ServiceGroupsAPI.DeleteServiceGroupsByID(ctx, objectId)
