...
// RESOURCE for SCM SyslogServerProfile (Package: objects)
var (
	_ resource.Resource                = &SyslogServerProfileResource{}
	_ resource.ResourceWithConfigure   = &SyslogServerProfileResource{}
	_ resource.ResourceWithImportState = &SyslogServerProfileResource{}
)

func NewSyslogServerProfileResource() resource.Resource {
	return &SyslogServerProfileResource{}
}

// SyslogServerProfileResource defines the resource implementation.
type SyslogServerProfileResource struct {
	client *objects.APIClient
}

func (r *SyslogServerProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_syslog_server_profile"
}

func (r *SyslogServerProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SyslogServerProfilesResourceSchema
}

func (r *SyslogServerProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *SyslogServerProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for SyslogServerProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SyslogServerProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSyslogServerProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating syslog_server_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SyslogServerProfilesAPI.CreateSyslogServerProfiles(ctx).SyslogServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating syslog_server_profiles", err.Error())
	...
	packedObject, diags := packSyslogServerProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created syslog_server_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SyslogServerProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.SyslogServerProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for SyslogServerProfile")
	var data, savestate models.SyslogServerProfiles
	...
	tflog.Debug(ctx, "Reading syslog_server_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SyslogServerProfilesAPI.GetSyslogServerProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no syslog_server_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading syslog_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSyslogServerProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SyslogServerProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.SyslogServerProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for SyslogServerProfile")
	var plan, state models.SyslogServerProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.SyslogServerProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating syslog_server_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SyslogServerProfilesAPI.UpdateSyslogServerProfilesByID(ctx, objectId).SyslogServerProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no syslog_server_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating syslog_server_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packSyslogServerProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated syslog_server_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *SyslogServerProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.SyslogServerProfiles
	...
	tflog.Debug(ctx, "Deleting syslog_server_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SyslogServerProfilesAPI.DeleteSyslogServerProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting syslog_server_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.SyslogServerProfilesAPI.GetSyslogServerProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *SyslogServerProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
