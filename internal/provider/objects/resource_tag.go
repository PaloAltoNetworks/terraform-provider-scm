...
// RESOURCE for SCM Tag (Package: objects)
var (
...
func (r *TagResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.TagsResourceSchema
}
...
func (r *TagResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Tag")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Tags{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackTagsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating tags on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.TagsAPI.CreateTags(ctx).Tags(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating tags", err.Error())
	...
	packedObject, diags := packTagsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created tags", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *TagResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Tags - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Tag")
	var data, savestate models.Tags
	...
	tflog.Debug(ctx, "Reading tags from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.TagsAPI.GetTagsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tags on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
	...
	packedObject, diags := packTagsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Tags{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *TagResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Tags which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Tag")
	var plan, state models.Tags
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Tags{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating tags on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.TagsAPI.UpdateTagsByID(ctx, objectId).Tags(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no tags on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
	...
	packedObject, diags := packTagsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated tags", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *TagResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting tags", map[string]interface{}{"id": objectId})
	deleteReq := r.client.TagsAPI.DeleteTagsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting tags", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
