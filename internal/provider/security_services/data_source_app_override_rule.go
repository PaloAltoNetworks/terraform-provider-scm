...
// DATA SOURCE for SCM AppOverrideRule (Package: security_services)
var (
...
// AppOverrideRuleDataSource defines the data source implementation.
...
func (d *AppOverrideRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: AppOverrideRuleDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.AppOverrideRulesDataSourceSchema
}

func (d *AppOverrideRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: AppOverrideRulesDataSource.Configure ---")
...
func (d *AppOverrideRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: AppOverrideRuleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading AppOverrideRules data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ApplicationOverrideRulesAPI.GetApplicationOverrideRulesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading AppOverrideRules data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ApplicationOverrideRulesAPI.ListApplicationOverrideRules(ctx)
		...
		var foundObject *security_services.AppOverrideRules
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("AppOverrideRules Not Found", fmt.Sprintf("No AppOverrideRules found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packAppOverrideRulesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the AppOverrideRules data source.")
		return
	}
