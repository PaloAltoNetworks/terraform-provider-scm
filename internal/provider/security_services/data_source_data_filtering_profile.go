...
// DATA SOURCE for SCM DataFilteringProfile (Package: security_services)
var (
...
// DataFilteringProfileDataSource defines the data source implementation.
...
func (d *DataFilteringProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DataFilteringProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DataFilteringProfilesDataSourceSchema
}

func (d *DataFilteringProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DataFilteringProfilesDataSource.Configure ---")
...
func (d *DataFilteringProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DataFilteringProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DataFilteringProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DataFilteringAPI.GetDataFilteringProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading DataFilteringProfiles", fmt.Sprintf("Could not read DataFilteringProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading DataFilteringProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DataFilteringAPI.ListDataFilteringProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing DataFilteringProfiless", fmt.Sprintf("Could not list DataFilteringProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *security_services.DataFilteringProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DataFilteringProfiles Not Found", fmt.Sprintf("No DataFilteringProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDataFilteringProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DataFilteringProfiles data source.")
		return
	}
