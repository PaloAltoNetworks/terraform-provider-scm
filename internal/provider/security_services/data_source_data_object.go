...
// DATA SOURCE for SCM DataObject (Package: security_services)
var (
	_ datasource.DataSource              = &DataObjectDataSource{}
	_ datasource.DataSourceWithConfigure = &DataObjectDataSource{}
)
...
func (d *DataObjectDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DataObjectDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DataObjectsDataSourceSchema
}
...
func (d *DataObjectDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DataObjectsDataSource.Configure ---")
...
func (d *DataObjectDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DataObjectDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DataObjects data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DataObjectsAPI.GetDataObjectsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading DataObjects", fmt.Sprintf("Could not read DataObjects with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading DataObjects data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DataObjectsAPI.ListDataObjects(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing DataObjectss", fmt.Sprintf("Could not list DataObjectss: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
		...
		var foundObject *security_services.DataObjects
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DataObjects Not Found", fmt.Sprintf("No DataObjects found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDataObjectsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DataObjects data source.")
		return
	}
