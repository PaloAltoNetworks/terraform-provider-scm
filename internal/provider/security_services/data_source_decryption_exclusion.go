...
// DATA SOURCE for SCM DecryptionExclusion (Package: security_services)
var (
...
// DecryptionExclusionDataSource defines the data source implementation.
...
func (d *DecryptionExclusionDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionExclusionDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DecryptionExclusionsDataSourceSchema
}

func (d *DecryptionExclusionDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionExclusionsDataSource.Configure ---")
...
func (d *DecryptionExclusionDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionExclusionDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DecryptionExclusions data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DecryptionExclusionsAPI.GetDecryptionExclusionsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading DecryptionExclusions", fmt.Sprintf("Could not read DecryptionExclusions with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading DecryptionExclusions data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DecryptionExclusionsAPI.ListDecryptionExclusions(ctx)
		...
		var foundObject *security_services.DecryptionExclusions
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DecryptionExclusions Not Found", fmt.Sprintf("No DecryptionExclusions found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDecryptionExclusionsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DecryptionExclusions data source.")
		return
	}
