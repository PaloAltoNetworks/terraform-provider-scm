...
// DATA SOURCE for SCM DecryptionProfile (Package: security_services)
var (
...
// DecryptionProfileDataSource defines the data source implementation.
...
func (d *DecryptionProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DecryptionProfilesDataSourceSchema
}

func (d *DecryptionProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionProfilesDataSource.Configure ---")
...
func (d *DecryptionProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DecryptionProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DecryptionProfilesAPI.GetDecryptionProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading DecryptionProfiles", fmt.Sprintf("Could not read DecryptionProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading DecryptionProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DecryptionProfilesAPI.ListDecryptionProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing DecryptionProfiless", fmt.Sprintf("Could not list DecryptionProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *security_services.DecryptionProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DecryptionProfiles Not Found", fmt.Sprintf("No DecryptionProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDecryptionProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DecryptionProfiles data source.")
		return
	}
