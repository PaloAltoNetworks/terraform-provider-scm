...
// DATA SOURCE for SCM DecryptionRule (Package: security_services)
var (
...
// DecryptionRuleDataSource defines the data source implementation.
...
func (d *DecryptionRuleDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionRuleDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DecryptionRulesDataSourceSchema
}

func (d *DecryptionRuleDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionRulesDataSource.Configure ---")
...
func (d *DecryptionRuleDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DecryptionRuleDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.DecryptionRulesAPI.GetDecryptionRulesByID(ctx, objectId)
		...
		tflog.Debug(ctx, "Reading DecryptionRules data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DecryptionRulesAPI.ListDecryptionRules(ctx)
		...
		var foundObject *security_services.DecryptionRules
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DecryptionRules Not Found", fmt.Sprintf("No DecryptionRules found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDecryptionRulesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DecryptionRules data source.")
		return
	}
