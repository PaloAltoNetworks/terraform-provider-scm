...
// DATA SOURCE for SCM DosProtectionProfile (Package: security_services)
var (
...
// DosProtectionProfileDataSource defines the data source implementation.
...
func (d *DosProtectionProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: DosProtectionProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.DosProtectionProfilesDataSourceSchema
}

func (d *DosProtectionProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: DosProtectionProfilesDataSource.Configure ---")
...
func (d *DosProtectionProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: DosProtectionProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading DosProtectionProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.DoSProtectionProfilesAPI.GetDoSProtectionProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading DosProtectionProfiles", fmt.Sprintf("Could not read DosProtectionProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading DosProtectionProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.DoSProtectionProfilesAPI.ListDoSProtectionProfiles(ctx)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Listing DosProtectionProfiless", fmt.Sprintf("Could not list DosProtectionProfiless: %s", err.Error()))
			detailedMessage := utils.PrintScmError(err)
			resp.Diagnostics.AddError(
				"Tag Listing Failed: API Request Failed",
				detailedMessage,
		...
		var foundObject *security_services.DosProtectionProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("DosProtectionProfiles Not Found", fmt.Sprintf("No DosProtectionProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packDosProtectionProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the DosProtectionProfiles data source.")
		return
	}
