...
// DATA SOURCE for SCM FileBlockingProfile (Package: security_services)
var (
...
// FileBlockingProfileDataSource defines the data source implementation.
...
func (d *FileBlockingProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: FileBlockingProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.FileBlockingProfilesDataSourceSchema
}

func (d *FileBlockingProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: FileBlockingProfilesDataSource.Configure ---")
...
func (d *FileBlockingProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: FileBlockingProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading FileBlockingProfiles data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.FileBlockingProfilesAPI.GetFileBlockingProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading FileBlockingProfiles", fmt.Sprintf("Could not read FileBlockingProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading FileBlockingProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.FileBlockingProfilesAPI.ListFileBlockingProfiles(ctx)
		...
		var foundObject *security_services.FileBlockingProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("FileBlockingProfiles Not Found", fmt.Sprintf("No FileBlockingProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packFileBlockingProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the FileBlockingProfiles data source.")
		return
	}
