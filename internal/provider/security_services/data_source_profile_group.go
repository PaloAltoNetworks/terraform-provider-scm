...
// DATA SOURCE for SCM ProfileGroup (Package: security_services)
var (
...
// ProfileGroupDataSource defines the data source implementation.
...
func (d *ProfileGroupDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ProfileGroupDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ProfileGroupsDataSourceSchema
}

func (d *ProfileGroupDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ProfileGroupsDataSource.Configure ---")
...
func (d *ProfileGroupDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ProfileGroupDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.ProfileGroupsAPI.GetProfileGroupsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading ProfileGroups", fmt.Sprintf("Could not read ProfileGroups with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading ProfileGroups data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.ProfileGroupsAPI.ListProfileGroups(ctx)
		...
		var foundObject *security_services.ProfileGroups
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("ProfileGroups Not Found", fmt.Sprintf("No ProfileGroups found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packProfileGroupsFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the ProfileGroups data source.")
		return
	}
