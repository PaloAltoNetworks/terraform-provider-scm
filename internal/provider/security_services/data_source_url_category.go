...
// DATA SOURCE for SCM UrlCategory (Package: security_services)
var (
...
// UrlCategoryDataSource defines the data source implementation.
...
func (d *UrlCategoryDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: UrlCategoryDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.UrlCategoriesDataSourceSchema
}

func (d *UrlCategoryDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: UrlCategoriesDataSource.Configure ---")
...
func (d *UrlCategoryDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: UrlCategoryDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		tflog.Debug(ctx, "Reading UrlCategories data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.URLCategoriesAPI.GetURLCategoriesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading UrlCategories", fmt.Sprintf("Could not read UrlCategories with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading UrlCategories data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.URLCategoriesAPI.ListURLCategories(ctx)
		...
		var foundObject *security_services.UrlCategories
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("UrlCategories Not Found", fmt.Sprintf("No UrlCategories found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packUrlCategoriesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the UrlCategories data source.")
		return
	}
