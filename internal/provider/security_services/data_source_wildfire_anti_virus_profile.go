...
// DATA SOURCE for SCM WildfireAntiVirusProfile (Package: security_services)
var (
...
// WildfireAntiVirusProfileDataSource defines the data source implementation.
...
func (d *WildfireAntiVirusProfileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: WildfireAntiVirusProfileDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.WildfireAntiVirusProfilesDataSourceSchema
}

func (d *WildfireAntiVirusProfileDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: WildfireAntiVirusProfilesDataSource.Configure ---")
...
func (d *WildfireAntiVirusProfileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: WildfireAntiVirusProfileDataSource.Read ---")
	...
	if !data.Id.IsNull() {
		...
		getReq := d.client.WildFireAntiVirusProfilesAPI.GetWildFireAntiVirusProfilesByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading WildfireAntiVirusProfiles", fmt.Sprintf("Could not read WildfireAntiVirusProfiles with ID %s: %s", objectId, err.Error()))
		...
		tflog.Debug(ctx, "Reading WildfireAntiVirusProfiles data source by Name", map[string]interface{}{"name": objectName})

		listReq := d.client.WildFireAntiVirusProfilesAPI.ListWildFireAntiVirusProfiles(ctx)
		...
		var foundObject *security_services.WildfireAntiVirusProfiles
		...
		if foundObject == nil {
			resp.Diagnostics.AddError("WildfireAntiVirusProfiles Not Found", fmt.Sprintf("No WildfireAntiVirusProfiles found with name: %s", objectName))
			return
		}

		// Create a packed object from the SCM response.
		packedObject, diags := packWildfireAntiVirusProfilesFromSdk(ctx, *foundObject)
		resp.Diagnostics.Append(diags...)
		if resp.Diagnostics.HasError() {
			return
		}
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the WildfireAntiVirusProfiles data source.")
		return
	}
