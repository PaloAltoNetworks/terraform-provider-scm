...
// RESOURCE for SCM AntiSpywareProfile (Package: security_services)
var (
...
// AntiSpywareProfileResource defines the resource implementation.
type AntiSpywareProfileResource struct {
	client *security_services.APIClient
}

func (r *AntiSpywareProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_anti_spyware_profile"
}

func (r *AntiSpywareProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AntiSpywareProfilesResourceSchema
}

func (r *AntiSpywareProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *AntiSpywareProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for AntiSpywareProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AntiSpywareProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackAntiSpywareProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating anti_spyware_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.AntiSpywareProfilesAPI.CreateAntiSpywareProfiles(ctx).AntiSpywareProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating anti_spyware_profiles", err.Error())
	...
	packedObject, diags := packAntiSpywareProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created anti_spyware_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *AntiSpywareProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.AntiSpywareProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for AntiSpywareProfile")
	var data, savestate models.AntiSpywareProfiles
	...
	tflog.Debug(ctx, "Reading anti_spyware_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.AntiSpywareProfilesAPI.GetAntiSpywareProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no anti_spyware_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading anti_spyware_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *AntiSpywareProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.AntiSpywareProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for AntiSpywareProfile")
	var plan, state models.AntiSpywareProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AntiSpywareProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating anti_spyware_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.AntiSpywareProfilesAPI.UpdateAntiSpywareProfilesByID(ctx, objectId).AntiSpywareProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no anti_spyware_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating anti_spyware_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAntiSpywareProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated anti_spyware_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *AntiSpywareProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting anti_spyware_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AntiSpywareProfilesAPI.DeleteAntiSpywareProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting anti_spyware_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.AntiSpywareProfilesAPI.GetAntiSpywareProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
