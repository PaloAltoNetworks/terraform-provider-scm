...
// RESOURCE for SCM AntiSpywareSignature (Package: security_services)
var (
...
// AntiSpywareSignatureResource defines the resource implementation.
type AntiSpywareSignatureResource struct {
	client *security_services.APIClient
}

func (r *AntiSpywareSignatureResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_anti_spyware_signature"
}

func (r *AntiSpywareSignatureResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.AntiSpywareSignaturesResourceSchema
}

func (r *AntiSpywareSignatureResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *AntiSpywareSignatureResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for AntiSpywareSignature")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AntiSpywareSignatures{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackAntiSpywareSignaturesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating anti_spyware_signatures on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.AntiSpywareSignaturesAPI.CreateAntiSpywareSignatures(ctx).AntiSpywareSignatures(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating anti_spyware_signatures", err.Error())
	...
	packedObject, diags := packAntiSpywareSignaturesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created anti_spyware_signatures", map[string]interface{}{"tfid": data.Tfid.ValueString()})
...
func (r *AntiSpywareSignatureResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.AntiSpywareSignatures - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for AntiSpywareSignature")
	var data, savestate models.AntiSpywareSignatures
	...
	tflog.Debug(ctx, "Reading anti_spyware_signatures from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.AntiSpywareSignaturesAPI.GetAntiSpywareSignaturesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no anti_spyware_signatures on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading anti_spyware_signatures", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *AntiSpywareSignatureResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.AntiSpywareSignatures which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for AntiSpywareSignature")
	var plan, state models.AntiSpywareSignatures
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.AntiSpywareSignatures{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating anti_spyware_signatures on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.AntiSpywareSignaturesAPI.UpdateAntiSpywareSignaturesByID(ctx, objectId).AntiSpywareSignatures(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no anti_spyware_signatures on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating anti_spyware_signatures", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packAntiSpywareSignaturesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated anti_spyware_signatures", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *AntiSpywareSignatureResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting anti_spyware_signatures", map[string]interface{}{"id": objectId})
	deleteReq := r.client.AntiSpywareSignaturesAPI.DeleteAntiSpywareSignaturesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting anti_spyware_signatures", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.AntiSpywareSignaturesAPI.GetAntiSpywareSignaturesByID(ctx, objectId).Execute()
			if getErr != nil {
