...
// RESOURCE for SCM DataFilteringProfile (Package: security_services)
var (
...
// DataFilteringProfileResource defines the resource implementation.
type DataFilteringProfileResource struct {
	client *security_services.APIClient
}

func (r *DataFilteringProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_data_filtering_profile"
}

func (r *DataFilteringProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DataFilteringProfilesResourceSchema
}

func (r *DataFilteringProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DataFilteringProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DataFilteringProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DataFilteringProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDataFilteringProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating data_filtering_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DataFilteringAPI.CreateDataFilteringProfiles(ctx).DataFilteringProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating data_filtering_profiles", err.Error())
	...
	packedObject, diags := packDataFilteringProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created data_filtering_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DataFilteringProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DataFilteringProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DataFilteringProfile")
	var data, savestate models.DataFilteringProfiles
	...
	tflog.Debug(ctx, "Reading data_filtering_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DataFilteringAPI.GetDataFilteringProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no data_filtering_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading data_filtering_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDataFilteringProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.DataFilteringProfiles{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DataFilteringProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.DataFilteringProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DataFilteringProfile")
	var plan, state models.DataFilteringProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DataFilteringProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating data_filtering_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DataFilteringAPI.UpdateDataFilteringProfilesByID(ctx, objectId).DataFilteringProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no data_filtering_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating data_filtering_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDataFilteringProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated data_filtering_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *DataFilteringProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.DataFilteringProfiles
	...
	tflog.Debug(ctx, "Deleting data_filtering_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DataFilteringAPI.DeleteDataFilteringProfilesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting data_filtering_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
...
func (r *DataFilteringProfileResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
