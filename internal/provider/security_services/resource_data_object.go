...
// RESOURCE for SCM DataObject (Package: security_services)
var (
	_ resource.Resource                = &DataObjectResource{}
	_ resource.ResourceWithConfigure   = &DataObjectResource{}
	_ resource.ResourceWithImportState = &DataObjectResource{}
)
...
func (r *DataObjectResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DataObjectsResourceSchema
}
...
func (r *DataObjectResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DataObject")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DataObjects{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDataObjectsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating data_objects on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DataObjectsAPI.CreateDataObjects(ctx).DataObjects(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating data_objects", err.Error())
	...
	packedObject, diags := packDataObjectsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created data_objects", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DataObjectResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DataObjects - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DataObject")
	var data, savestate models.DataObjects
	...
	tflog.Debug(ctx, "Reading data_objects from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DataObjectsAPI.GetDataObjectsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no data_objects on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading data_objects", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDataObjectsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.DataObjects{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DataObjectResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.DataObjects which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DataObject")
	var plan, state models.DataObjects
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DataObjects{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating data_objects on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DataObjectsAPI.UpdateDataObjectsByID(ctx, objectId).DataObjects(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no data_objects on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating data_objects", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDataObjectsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated data_objects", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *DataObjectResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting data_objects", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DataObjectsAPI.DeleteDataObjectsByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting data_objects", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
