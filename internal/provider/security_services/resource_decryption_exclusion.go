...
// RESOURCE for SCM DecryptionExclusion (Package: security_services)
var (
...
// DecryptionExclusionResource defines the resource implementation.
type DecryptionExclusionResource struct {
	client *security_services.APIClient
}

func (r *DecryptionExclusionResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_decryption_exclusion"
}

func (r *DecryptionExclusionResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DecryptionExclusionsResourceSchema
}

func (r *DecryptionExclusionResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DecryptionExclusionResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DecryptionExclusion")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DecryptionExclusions{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDecryptionExclusionsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating decryption_exclusions on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DecryptionExclusionsAPI.CreateDecryptionExclusions(ctx).DecryptionExclusions(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating decryption_exclusions", err.Error())
	...
	packedObject, diags := packDecryptionExclusionsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created decryption_exclusions", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DecryptionExclusionResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DecryptionExclusions - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DecryptionExclusion")
	var data, savestate models.DecryptionExclusions
	...
	tflog.Debug(ctx, "Reading decryption_exclusions from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DecryptionExclusionsAPI.GetDecryptionExclusionsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no decryption_exclusions on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading decryption_exclusions", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDecryptionExclusionsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DecryptionExclusionResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.DecryptionExclusions which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DecryptionExclusion")
	var plan, state models.DecryptionExclusions
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DecryptionExclusions{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating decryption_exclusions on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DecryptionExclusionsAPI.UpdateDecryptionExclusionsByID(ctx, objectId).DecryptionExclusions(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no decryption_exclusions on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating decryption_exclusions", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDecryptionExclusionsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated decryption_exclusions", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *DecryptionExclusionResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting decryption_exclusions", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DecryptionExclusionsAPI.DeleteDecryptionExclusionsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting decryption_exclusions", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.DecryptionExclusionsAPI.GetDecryptionExclusionsByID(ctx, objectId).Execute()
			if getErr != nil {
