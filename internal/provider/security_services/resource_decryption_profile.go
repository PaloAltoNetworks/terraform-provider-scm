...
// RESOURCE for SCM DecryptionProfile (Package: security_services)
var (
...
// DecryptionProfileResource defines the resource implementation.
type DecryptionProfileResource struct {
	client *security_services.APIClient
}

func (r *DecryptionProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_decryption_profile"
}

func (r *DecryptionProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DecryptionProfilesResourceSchema
}

func (r *DecryptionProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DecryptionProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DecryptionProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DecryptionProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDecryptionProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating decryption_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DecryptionProfilesAPI.CreateDecryptionProfiles(ctx).DecryptionProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating decryption_profiles", err.Error())
	...
	packedObject, diags := packDecryptionProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created decryption_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DecryptionProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DecryptionProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DecryptionProfile")
	var data, savestate models.DecryptionProfiles
	...
	tflog.Debug(ctx, "Reading decryption_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DecryptionProfilesAPI.GetDecryptionProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no decryption_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading decryption_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDecryptionProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DecryptionProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.DecryptionProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DecryptionProfile")
	var plan, state models.DecryptionProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DecryptionProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating decryption_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DecryptionProfilesAPI.UpdateDecryptionProfilesByID(ctx, objectId).DecryptionProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no decryption_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating decryption_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDecryptionProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated decryption_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *DecryptionProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting decryption_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DecryptionProfilesAPI.DeleteDecryptionProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting decryption_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.DecryptionProfilesAPI.GetDecryptionProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
