...
// RESOURCE for SCM DecryptionRule (Package: security_services)
var (
...
// DecryptionRuleResource defines the resource implementation.
type DecryptionRuleResource struct {
	client *security_services.APIClient
}

func (r *DecryptionRuleResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_decryption_rule"
}

func (r *DecryptionRuleResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DecryptionRulesResourceSchema
}

func (r *DecryptionRuleResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DecryptionRuleResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DecryptionRule")
	...
	unpackedScmObject, diags := unpackDecryptionRulesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating decryption_rules on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DecryptionRulesAPI.CreateDecryptionRules(ctx).DecryptionRules(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating decryption_rules", err.Error())
	...
	tflog.Debug(ctx, "Created decryption_rules", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DecryptionRuleResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DecryptionRules - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DecryptionRule")
	var data, savestate models.DecryptionRules
	...
	tflog.Debug(ctx, "Reading decryption_rules from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DecryptionRulesAPI.GetDecryptionRulesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no decryption_rules on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading decryption_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDecryptionRulesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.DecryptionRules{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *DecryptionRuleResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.DecryptionRules which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DecryptionRule")
	var plan, state models.DecryptionRules
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DecryptionRules{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating decryption_rules on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DecryptionRulesAPI.UpdateDecryptionRulesByID(ctx, objectId).DecryptionRules(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no decryption_rules on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating decryption_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *DecryptionRuleResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting decryption_rules", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DecryptionRulesAPI.DeleteDecryptionRulesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting decryption_rules", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
