...
// RESOURCE for SCM DnsSecurityProfile (Package: security_services)
var (
...
// DnsSecurityProfileResource defines the resource implementation.
type DnsSecurityProfileResource struct {
	client *security_services.APIClient
}

func (r *DnsSecurityProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_dns_security_profile"
}

func (r *DnsSecurityProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DnsSecurityProfilesResourceSchema
}

func (r *DnsSecurityProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DnsSecurityProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DnsSecurityProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DnsSecurityProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDnsSecurityProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating dns_security_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DNSSecurityProfilesAPI.CreateDNSSecurityProfiles(ctx).DnsSecurityProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating dns_security_profiles", err.Error())
	...
	packedObject, diags := packDnsSecurityProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created dns_security_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DnsSecurityProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DnsSecurityProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DnsSecurityProfile")
	var data, savestate models.DnsSecurityProfiles
	...
	tflog.Debug(ctx, "Reading dns_security_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DNSSecurityProfilesAPI.GetDNSSecurityProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dns_security_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading dns_security_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *DnsSecurityProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.DnsSecurityProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DnsSecurityProfile")
	var plan, state models.DnsSecurityProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DnsSecurityProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating dns_security_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DNSSecurityProfilesAPI.UpdateDNSSecurityProfilesByID(ctx, objectId).DnsSecurityProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dns_security_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating dns_security_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDnsSecurityProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated dns_security_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *DnsSecurityProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting dns_security_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DNSSecurityProfilesAPI.DeleteDNSSecurityProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting dns_security_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.DNSSecurityProfilesAPI.GetDNSSecurityProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
