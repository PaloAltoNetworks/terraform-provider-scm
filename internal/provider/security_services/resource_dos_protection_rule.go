...
// RESOURCE for SCM DosProtectionRule (Package: security_services)
var (
	_ resource.Resource                = &DosProtectionRuleResource{}
	_ resource.ResourceWithConfigure   = &DosProtectionRuleResource{}
	_ resource.ResourceWithImportState = &DosProtectionRuleResource{}
)

func NewDosProtectionRuleResource() resource.Resource {
	return &DosProtectionRuleResource{}
}

// DosProtectionRuleResource defines the resource implementation.
type DosProtectionRuleResource struct {
	client *security_services.APIClient
}

func (r *DosProtectionRuleResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_dos_protection_rule"
}

func (r *DosProtectionRuleResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.DosProtectionRulesResourceSchema
}

func (r *DosProtectionRuleResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *DosProtectionRuleResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for DosProtectionRule")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DosProtectionRules{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackDosProtectionRulesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating dos_protection_rules on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.DoSProtectionRulesAPI.CreateDoSProtectionRules(ctx).DosProtectionRules(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating dos_protection_rules", err.Error())
	...
	packedObject, diags := packDosProtectionRulesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created dos_protection_rules", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *DosProtectionRuleResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.DosProtectionRules - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for DosProtectionRule")
	var data, savestate models.DosProtectionRules
	...
	tflog.Debug(ctx, "Reading dos_protection_rules from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.DoSProtectionRulesAPI.GetDoSProtectionRulesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dos_protection_rules on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading dos_protection_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *DosProtectionRuleResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.DosProtectionRules which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for DosProtectionRule")
	var plan, state models.DosProtectionRules
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.DosProtectionRules{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating dos_protection_rules on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.DoSProtectionRulesAPI.UpdateDoSProtectionRulesByID(ctx, objectId).DosProtectionRules(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no dos_protection_rules on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating dos_protection_rules", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packDosProtectionRulesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated dos_protection_rules", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *DosProtectionRuleResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.DosProtectionRules
	...
	tflog.Debug(ctx, "Deleting dos_protection_rules", map[string]interface{}{"id": objectId})
	deleteReq := r.client.DoSProtectionRulesAPI.DeleteDoSProtectionRulesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting dos_protection_rules", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.DoSProtectionRulesAPI.GetDoSProtectionRulesByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *DosProtectionRuleResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
