...
// RESOURCE for SCM FileBlockingProfile (Package: security_services)
var (
...
// FileBlockingProfileResource defines the resource implementation.
...
func (r *FileBlockingProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.FileBlockingProfilesResourceSchema
}
...
func (r *FileBlockingProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for FileBlockingProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.FileBlockingProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackFileBlockingProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating file_blocking_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.FileBlockingProfilesAPI.CreateFileBlockingProfiles(ctx).FileBlockingProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating file_blocking_profiles", err.Error())
	...
	packedObject, diags := packFileBlockingProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created file_blocking_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *FileBlockingProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.FileBlockingProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for FileBlockingProfile")
	var data, savestate models.FileBlockingProfiles
	...
	tflog.Debug(ctx, "Reading file_blocking_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.FileBlockingProfilesAPI.GetFileBlockingProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no file_blocking_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading file_blocking_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packFileBlockingProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *FileBlockingProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.FileBlockingProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for FileBlockingProfile")
	var plan, state models.FileBlockingProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.FileBlockingProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating file_blocking_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.FileBlockingProfilesAPI.UpdateFileBlockingProfilesByID(ctx, objectId).FileBlockingProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no file_blocking_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating file_blocking_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packFileBlockingProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated file_blocking_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *FileBlockingProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting file_blocking_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.FileBlockingProfilesAPI.DeleteFileBlockingProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting file_blocking_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.FileBlockingProfilesAPI.GetFileBlockingProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
