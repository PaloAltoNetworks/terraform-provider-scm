...
// RESOURCE for SCM HttpHeaderProfile (Package: security_services)
var (
...
// HttpHeaderProfileResource defines the resource implementation.
type HttpHeaderProfileResource struct {
	client *security_services.APIClient
}

func (r *HttpHeaderProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_http_header_profile"
}

func (r *HttpHeaderProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.HttpHeaderProfilesResourceSchema
}

func (r *HttpHeaderProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *HttpHeaderProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for HttpHeaderProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HttpHeaderProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackHttpHeaderProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating http_header_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.HTTPHeaderProfilesAPI.CreateHTTPHeaderProfiles(ctx).HttpHeaderProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating http_header_profiles", err.Error())
	...
	packedObject, diags := packHttpHeaderProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created http_header_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *HttpHeaderProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.HttpHeaderProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for HttpHeaderProfile")
	var data, savestate models.HttpHeaderProfiles
	...
	tflog.Debug(ctx, "Reading http_header_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.HTTPHeaderProfilesAPI.GetHTTPHeaderProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no http_header_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading http_header_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
...
func (r *HttpHeaderProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.HttpHeaderProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for HttpHeaderProfile")
	var plan, state models.HttpHeaderProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.HttpHeaderProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating http_header_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.HTTPHeaderProfilesAPI.UpdateHTTPHeaderProfilesByID(ctx, objectId).HttpHeaderProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no http_header_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating http_header_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packHttpHeaderProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated http_header_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *HttpHeaderProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting http_header_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.HTTPHeaderProfilesAPI.DeleteHTTPHeaderProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting http_header_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.HTTPHeaderProfilesAPI.GetHTTPHeaderProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
