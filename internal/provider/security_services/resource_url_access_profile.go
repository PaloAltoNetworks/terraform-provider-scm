...
// RESOURCE for SCM UrlAccessProfile (Package: security_services)
var (
...
// UrlAccessProfileResource defines the resource implementation.
type UrlAccessProfileResource struct {
	client *security_services.APIClient
}

func (r *UrlAccessProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_url_access_profile"
}

func (r *UrlAccessProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.UrlAccessProfilesResourceSchema
}

func (r *UrlAccessProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *UrlAccessProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for UrlAccessProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UrlAccessProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackUrlAccessProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating url_access_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.URLAccessProfilesAPI.CreateURLAccessProfiles(ctx).UrlAccessProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating url_access_profiles", err.Error())
	...
	packedObject, diags := packUrlAccessProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created url_access_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *UrlAccessProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.UrlAccessProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for UrlAccessProfile")
	var data, savestate models.UrlAccessProfiles
	...
	tflog.Debug(ctx, "Reading url_access_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.URLAccessProfilesAPI.GetURLAccessProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no url_access_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading url_access_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.UrlAccessProfiles{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *UrlAccessProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.UrlAccessProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for UrlAccessProfile")
	var plan, state models.UrlAccessProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UrlAccessProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating url_access_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.URLAccessProfilesAPI.UpdateURLAccessProfilesByID(ctx, objectId).UrlAccessProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no url_access_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating url_access_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packUrlAccessProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated url_access_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *UrlAccessProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting url_access_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.URLAccessProfilesAPI.DeleteURLAccessProfilesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting url_access_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
