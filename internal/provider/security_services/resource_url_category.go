...
// RESOURCE for SCM UrlCategory (Package: security_services)
var (
...
// UrlCategoryResource defines the resource implementation.
type UrlCategoryResource struct {
	client *security_services.APIClient
}

func (r *UrlCategoryResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_url_category"
}

func (r *UrlCategoryResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.UrlCategoriesResourceSchema
}

func (r *UrlCategoryResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *UrlCategoryResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for UrlCategory")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UrlCategories{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackUrlCategoriesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating url_categories on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.URLCategoriesAPI.CreateURLCategories(ctx).UrlCategories(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating url_categories", err.Error())
	...
	packedObject, diags := packUrlCategoriesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created url_categories", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *UrlCategoryResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.UrlCategories - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for UrlCategory")
	var data, savestate models.UrlCategories
	...
	tflog.Debug(ctx, "Reading url_categories from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.URLCategoriesAPI.GetURLCategoriesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no url_categories on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading url_categories", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packUrlCategoriesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.UrlCategories{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *UrlCategoryResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.UrlCategories which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for UrlCategory")
	var plan, state models.UrlCategories
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.UrlCategories{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating url_categories on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.URLCategoriesAPI.UpdateURLCategoriesByID(ctx, objectId).UrlCategories(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no url_categories on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating url_categories", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packUrlCategoriesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated url_categories", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *UrlCategoryResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting url_categories", map[string]interface{}{"id": objectId})
	deleteReq := r.client.URLCategoriesAPI.DeleteURLCategoriesByID(ctx, objectId)
	_, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting url_categories", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
