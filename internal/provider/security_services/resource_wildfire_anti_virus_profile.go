...
// RESOURCE for SCM WildfireAntiVirusProfile (Package: security_services)
var (
...
// WildfireAntiVirusProfileResource defines the resource implementation.
type WildfireAntiVirusProfileResource struct {
	client *security_services.APIClient
}

func (r *WildfireAntiVirusProfileResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "scm_wildfire_anti_virus_profile"
}

func (r *WildfireAntiVirusProfileResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.WildfireAntiVirusProfilesResourceSchema
}

func (r *WildfireAntiVirusProfileResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *WildfireAntiVirusProfileResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for WildfireAntiVirusProfile")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.WildfireAntiVirusProfiles{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackWildfireAntiVirusProfilesToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating wildfire_anti_virus_profiles on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.WildFireAntiVirusProfilesAPI.CreateWildFireAntiVirusProfiles(ctx).WildfireAntiVirusProfiles(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating wildfire_anti_virus_profiles", err.Error())
	...
	packedObject, diags := packWildfireAntiVirusProfilesFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created wildfire_anti_virus_profiles", map[string]interface{}{"tfid": data.Tfid.ValueString()})
...
func (r *WildfireAntiVirusProfileResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.WildfireAntiVirusProfiles - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for WildfireAntiVirusProfile")
	var data, savestate models.WildfireAntiVirusProfiles
	...
	tflog.Debug(ctx, "Reading wildfire_anti_virus_profiles from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.WildFireAntiVirusProfilesAPI.GetWildFireAntiVirusProfilesByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no wildfire_anti_virus_profiles on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading wildfire_anti_virus_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packWildfireAntiVirusProfilesFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *WildfireAntiVirusProfileResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.WildfireAntiVirusProfiles which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for WildfireAntiVirusProfile")
	var plan, state models.WildfireAntiVirusProfiles
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.WildfireAntiVirusProfiles{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating wildfire_anti_virus_profiles on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.WildFireAntiVirusProfilesAPI.UpdateWildFireAntiVirusProfilesByID(ctx, objectId).WildfireAntiVirusProfiles(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no wildfire_anti_virus_profiles on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating wildfire_anti_virus_profiles", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packWildfireAntiVirusProfilesFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated wildfire_anti_virus_profiles", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *WildfireAntiVirusProfileResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting wildfire_anti_virus_profiles", map[string]interface{}{"id": objectId})
	deleteReq := r.client.WildFireAntiVirusProfilesAPI.DeleteWildFireAntiVirusProfilesByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting wildfire_anti_virus_profiles", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.WildFireAntiVirusProfilesAPI.GetWildFireAntiVirusProfilesByID(ctx, objectId).Execute()
			if getErr != nil {
