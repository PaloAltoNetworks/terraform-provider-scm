...
// DATA SOURCE for SCM Connector (Package: ztna_connector_all)
var (
...
func (d *ConnectorDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: ConnectorDataSource.Metadata ---")
...
func (d *ConnectorDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: ConnectorDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ConnectorsDataSourceSchema
}
...
func (d *ConnectorDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: ConnectorsDataSource.Configure ---")
...
func (d *ConnectorDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: ConnectorDataSource.Read ---")
	...
	if !data.Oid.IsNull() {
		...
		tflog.Debug(ctx, "Reading Connectors data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.ConnectorAPI.GetConnectorsByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading Connectors", fmt.Sprintf("Could not read Connectors with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Connectors data source.")
		return
	}
