...
var (
	_ datasource.DataSource              = &ConnectorFiltersDataSource{}
	_ datasource.DataSourceWithConfigure = &ConnectorFiltersDataSource{}
)

func NewConnectorFiltersDataSource() datasource.DataSource {
	return &ConnectorFiltersDataSource{}
}

type ConnectorFiltersDataSource struct {
	client *ztna_connector_all.APIClient
}

type connectorFiltersModel struct {
...
func (d *ConnectorFiltersDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "ztna_connector_filters"
}

func (d *ConnectorFiltersDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "List Connector Filters",
...
func (d *ConnectorFiltersDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ConnectorFiltersDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data connectorFiltersModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}

	apiReq := d.client.ConnectorAPI.ListConnectorFilters(ctx)
		...
		if data.Field.IsNull() {
			if bodyErr, ok := err.(interface{ Body() []byte }); ok {
				...
				if json.Unmarshal(bodyErr.Body(), &filterObj) == nil {
					...
					data.Tfid = types.StringValue("connector_filters")
					resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
					return
				}
			}
		}
		resp.Diagnostics.AddError("Error calling ListConnectorFilters", fmt.Sprintf("API error: %s", err.Error()))
