...
var (
	_ datasource.DataSource              = &ConnectorGroupFiltersDataSource{}
	_ datasource.DataSourceWithConfigure = &ConnectorGroupFiltersDataSource{}
)

func NewConnectorGroupFiltersDataSource() datasource.DataSource {
	return &ConnectorGroupFiltersDataSource{}
}

type ConnectorGroupFiltersDataSource struct {
	client *ztna_connector_all.APIClient
}

type connectorGroupFiltersModel struct {
...
func (d *ConnectorGroupFiltersDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "ztna_connector_group_filters"
}

func (d *ConnectorGroupFiltersDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "List Connector Group Filters",
...
func (d *ConnectorGroupFiltersDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *ConnectorGroupFiltersDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data connectorGroupFiltersModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}

	apiReq := d.client.ConnectorGroupAPI.ListConnectorGroupFilters(ctx)
		...
		if data.Field.IsNull() {
			if bodyErr, ok := err.(interface{ Body() []byte }); ok {
				...
				if json.Unmarshal(bodyErr.Body(), &filterObj) == nil {
					...
					data.Tfid = types.StringValue("connector_group_filters")
					resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
					return
				}
			}
		}
		resp.Diagnostics.AddError("Error calling ListConnectorGroupFilters", fmt.Sprintf("API error: %s", err.Error()))
