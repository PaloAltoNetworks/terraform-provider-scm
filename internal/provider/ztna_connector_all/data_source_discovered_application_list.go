...
var (
	_ datasource.DataSource              = &DiscoveredApplicationListDataSource{}
	_ datasource.DataSourceWithConfigure = &DiscoveredApplicationListDataSource{}
)

func NewDiscoveredApplicationListDataSource() datasource.DataSource {
	return &DiscoveredApplicationListDataSource{}
}

// DiscoveredApplicationListDataSource defines the data source implementation.
type DiscoveredApplicationListDataSource struct {
	client *ztna_connector_all.APIClient
}

func (d *DiscoveredApplicationListDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "ztna_discovered_application_list"
}

func (d *DiscoveredApplicationListDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = models.DiscoveredApplicationsListDataSourceSchema
}

func (d *DiscoveredApplicationListDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *DiscoveredApplicationListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data models.DiscoveredApplicationsListModel
	...
	listReq := d.client.DiscoveredApplicationAPI.ListDiscoveredApplications(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error Listing DiscoveredApplicationss", fmt.Sprintf("Could not list DiscoveredApplicationss: %s", err.Error()))
	...
	packedList, diags := packDiscoveredApplicationsListFromSdk(ctx, listResponse.GetData())
