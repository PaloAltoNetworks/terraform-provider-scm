...
// DATA SOURCE for SCM FqdnApplication (Package: ztna_connector_all)
var (
	_ datasource.DataSource              = &FqdnApplicationDataSource{}
	_ datasource.DataSourceWithConfigure = &FqdnApplicationDataSource{}
)

func NewFqdnApplicationDataSource() datasource.DataSource {
	return &FqdnApplicationDataSource{}
}

// FqdnApplicationDataSource defines the data source implementation.
type FqdnApplicationDataSource struct {
	client *ztna_connector_all.APIClient
}

func (d *FqdnApplicationDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: FqdnApplicationDataSource.Metadata ---")
	resp.TypeName = "ztna_fqdn_application"
}

func (d *FqdnApplicationDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: FqdnApplicationDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.ApplicationsDataSourceSchema
}

func (d *FqdnApplicationDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *FqdnApplicationDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: FqdnApplicationDataSource.Read ---")
	...
	if !data.Oid.IsNull() {
		...
		getReq := d.client.FQDNAPI.GetApplicationsByID(ctx, objectId)
