...
// DATA SOURCE for SCM License (Package: ztna_connector_all)
var (
...
func (d *LicenseDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: LicenseDataSource.Read ---")
	...
	tflog.Debug(ctx, "Reading Singleton License")

	// 1. Perform the API call (no ID argument)
	readReq := d.client.TenantAPI.GetLicense(ctx)
	...
	var scmObject *ztna_connector_all.License
	...
	if val.Kind() == reflect.Struct {
		...
		if dataField.IsValid() && dataField.Kind() == reflect.Slice {
			if dataField.Len() > 0 {
				...
				var targetStruct ztna_connector_all.License
	...
	packedObject, diags := packLicenseFromSdk(ctx, *scmObject)
	...
	// Force synthetic ID
	data.Tfid = types.StringValue("singleton_license")
