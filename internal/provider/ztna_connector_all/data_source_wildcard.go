...
// DATA SOURCE for SCM Wildcard (Package: ztna_connector_all)
var (
	_ datasource.DataSource              = &WildcardDataSource{}
	_ datasource.DataSourceWithConfigure = &WildcardDataSource{}
)

func NewWildcardDataSource() datasource.DataSource {
	return &WildcardDataSource{}
}

// WildcardDataSource defines the data source implementation.
type WildcardDataSource struct {
	client *ztna_connector_all.APIClient
}

func (d *WildcardDataSource) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	tflog.Debug(ctx, "--- ENTER: WildcardDataSource.Metadata ---")
	resp.TypeName = "ztna_wildcard"
}

func (d *WildcardDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	tflog.Debug(ctx, "--- ENTER: WildcardDataSource.Schema ---")
	// Use the pre-generated schema from the model file.
	resp.Schema = models.WildcardsDataSourceSchema
}

func (d *WildcardDataSource) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	tflog.Debug(ctx, "--- ENTER: WildcardsDataSource.Configure ---")
...
func (d *WildcardDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	tflog.Debug(ctx, "--- ENTER: WildcardDataSource.Read ---")
	...
	if !data.Oid.IsNull() {
		...
		tflog.Debug(ctx, "Reading Wildcards data source by ID", map[string]interface{}{"id": objectId})

		getReq := d.client.WildcardAPI.GetWildcardByID(ctx, objectId)
		...
		if err != nil {
			resp.Diagnostics.AddError("Error Reading Wildcards", fmt.Sprintf("Could not read Wildcards with ID %s: %s", objectId, err.Error()))
		...
		resp.Diagnostics.AddError("Missing Identifier", "Either 'id' or 'name' must be provided for the Wildcards data source.")
		return
	}
