...
var (
	_ datasource.DataSource              = &WildcardFiltersDataSource{}
	_ datasource.DataSourceWithConfigure = &WildcardFiltersDataSource{}
)

func NewWildcardFiltersDataSource() datasource.DataSource {
	return &WildcardFiltersDataSource{}
}

type WildcardFiltersDataSource struct {
	client *ztna_connector_all.APIClient
}

type wildcardFiltersModel struct {
...
func (d *WildcardFiltersDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = "ztna_wildcard_filters"
}

func (d *WildcardFiltersDataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "List Wildcard Filters",
...
func (d *WildcardFiltersDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *WildcardFiltersDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data wildcardFiltersModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}

	apiReq := d.client.WildcardAPI.ListWildcardFilters(ctx)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error calling ListWildcardFilters", fmt.Sprintf("API error: %s", err.Error()))
	...
	data.Tfid = types.StringValue("wildcard_filters")
