...
// RESOURCE for SCM Connector (Package: ztna_connector_all)
var (
...
func (r *ConnectorResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Connector")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Connectors{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackConnectorsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating connectors on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ConnectorAPI.CreateConnectors(ctx).Connectors(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating connectors", err.Error())
	...
	createdObject, err := r.client.ConnectorAPI.FetchConnector(ctx, data.Name.ValueString(), nil, nil, nil)
	if err != nil {
		resp.Diagnostics.AddError("Error fetching created connectors after creation", err.Error())
		return
	}
	if createdObject == nil {
		resp.Diagnostics.AddError("Error fetching created connectors", fmt.Sprintf("Could not find connectors with name %s after creation", data.Name.ValueString()))
		return
	}
	// 6. Pack the API response back into a Terraform model data.
	packedObject, diags := packConnectorsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created connectors", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ConnectorResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Connectors - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Connector")
	var data, savestate models.Connectors
	...
	tflog.Debug(ctx, "Reading connectors from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ConnectorAPI.GetConnectorsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no connectors on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
	...
	packedObject, diags := packConnectorsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Connectors{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ConnectorResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Connectors which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Connector")
	var plan, state models.Connectors
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Connectors{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating connectors on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ConnectorAPI.UpdateConnectorsByID(ctx, objectId).Connectors(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no connectors on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating connectors", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	updatedObject, _, err := r.client.ConnectorAPI.GetConnectorsByID(ctx, objectId).Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error fetching updated connectors after update", err.Error())
		return
	}

	// Step 9: Pack the SCM updatedObject into a TF object
	packedObject, diags := packConnectorsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated connectors", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *ConnectorResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting connectors", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ConnectorAPI.DeleteConnectorsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting connectors", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.ConnectorAPI.GetConnectorsByID(ctx, objectId).Execute()
			if getErr != nil {
