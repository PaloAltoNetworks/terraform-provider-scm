...
// RESOURCE for SCM ConnectorGroup (Package: ztna_connector_all)
var (
	_ resource.Resource                = &ConnectorGroupResource{}
	_ resource.ResourceWithConfigure   = &ConnectorGroupResource{}
	_ resource.ResourceWithImportState = &ConnectorGroupResource{}
)
...
// ConnectorGroupResource defines the resource implementation.
...
func (r *ConnectorGroupResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for ConnectorGroup")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ConnectorGroups{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackConnectorGroupsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating connector_groups on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.ConnectorGroupAPI.CreateConnectorGroup(ctx).ConnectorGroups(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating connector_groups", err.Error())
	...
	createdObject, err := r.client.ConnectorGroupAPI.FetchConnectorGroup(ctx, data.Name.ValueString(), nil, nil, nil)
	if err != nil {
		resp.Diagnostics.AddError("Error fetching created connector_groups after creation", err.Error())
		return
	}
	if createdObject == nil {
		resp.Diagnostics.AddError("Error fetching created connector_groups", fmt.Sprintf("Could not find connector_groups with name %s after creation", data.Name.ValueString()))
		return
	}
	// 6. Pack the API response back into a Terraform model data.
	packedObject, diags := packConnectorGroupsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created connector_groups", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *ConnectorGroupResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.ConnectorGroups - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for ConnectorGroup")
	var data, savestate models.ConnectorGroups
	...
	tflog.Debug(ctx, "Reading connector_groups from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.ConnectorGroupAPI.GetConnectorGroupByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no connector_groups on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading connector_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	packedObject, diags := packConnectorGroupsFromSdk(ctx, *scmObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.ConnectorGroups{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *ConnectorGroupResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.ConnectorGroups which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for ConnectorGroup")
	var plan, state models.ConnectorGroups
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.ConnectorGroups{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating connector_groups on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.ConnectorGroupAPI.UpdateConnectorGroupByID(ctx, objectId).ConnectorGroups(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no connector_groups on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating connector_groups", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	updatedObject, _, err := r.client.ConnectorGroupAPI.GetConnectorGroupByID(ctx, objectId).Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error fetching updated connector_groups after update", err.Error())
		return
	}

	// Step 9: Pack the SCM updatedObject into a TF object
	packedObject, diags := packConnectorGroupsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated connector_groups", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *ConnectorGroupResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting connector_groups", map[string]interface{}{"id": objectId})
	deleteReq := r.client.ConnectorGroupAPI.DeleteConnectorGroupByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting connector_groups", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.ConnectorGroupAPI.GetConnectorGroupByID(ctx, objectId).Execute()
			if getErr != nil {
