...
// RESOURCE for SCM FqdnApplication (Package: ztna_connector_all)
var (
	_ resource.Resource                = &FqdnApplicationResource{}
	_ resource.ResourceWithConfigure   = &FqdnApplicationResource{}
	_ resource.ResourceWithImportState = &FqdnApplicationResource{}
)

func NewFqdnApplicationResource() resource.Resource {
	return &FqdnApplicationResource{}
}

// FqdnApplicationResource defines the resource implementation.
type FqdnApplicationResource struct {
	client *ztna_connector_all.APIClient
}

func (r *FqdnApplicationResource) Metadata(ctx context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = "ztna_fqdn_application"
}

func (r *FqdnApplicationResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.ApplicationsResourceSchema
}

func (r *FqdnApplicationResource) Configure(ctx context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
...
func (r *FqdnApplicationResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for FqdnApplication")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Applications{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackApplicationsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating applications on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.FQDNAPI.CreateApplications(ctx).Applications(*unpackedScmObject)
	...
	createdObject, err := r.client.FQDNAPI.FetchFQDN(ctx, data.Name.ValueString(), nil, nil, nil)
	if err != nil {
		resp.Diagnostics.AddError("Error fetching created applications after creation", err.Error())
		return
	}
	if createdObject == nil {
		resp.Diagnostics.AddError("Error fetching created applications", fmt.Sprintf("Could not find applications with name %s after creation", data.Name.ValueString()))
		return
	}
	// 6. Pack the API response back into a Terraform model data.
	packedObject, diags := packApplicationsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created applications", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *FqdnApplicationResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Applications - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for FqdnApplication")
	var data, savestate models.Applications
	...
	tflog.Debug(ctx, "Reading applications from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.FQDNAPI.GetApplicationsByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no applications on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Applications{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *FqdnApplicationResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {

	// Step 1: Initialize a plan and state of type models.Applications which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for FqdnApplication")
	var plan, state models.Applications
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Applications{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating applications on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.FQDNAPI.UpdateApplicationsByID(ctx, objectId).Applications(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no applications on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating applications", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	updatedObject, _, err := r.client.FQDNAPI.GetApplicationsByID(ctx, objectId).Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error fetching updated applications after update", err.Error())
		return
	}

	// Step 9: Pack the SCM updatedObject into a TF object
	packedObject, diags := packApplicationsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated applications", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &plan)...)
}

func (r *FqdnApplicationResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	var data models.Applications
	...
	tflog.Debug(ctx, "Deleting applications", map[string]interface{}{"id": objectId})
	deleteReq := r.client.FQDNAPI.DeleteApplicationsByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting applications", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.FQDNAPI.GetApplicationsByID(ctx, objectId).Execute()
			if getErr != nil {
...
func (r *FqdnApplicationResource) ImportState(ctx context.Context, req resource.ImportStateRequest, resp *resource.ImportStateResponse) {
