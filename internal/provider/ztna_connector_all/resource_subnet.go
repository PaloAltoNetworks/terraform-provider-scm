...
// RESOURCE for SCM Subnet (Package: ztna_connector_all)
var (
...
func (r *SubnetResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = models.SubnetsResourceSchema
}
...
func (r *SubnetResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "Starting Create function for Subnet")
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Subnets{}.AttrTypes(), &data)
	...
	unpackedScmObject, diags := unpackSubnetsToSdk(ctx, planObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}

	tflog.Debug(ctx, "Creating subnets on SCM API")

	// 3. Initiate the API request with the body.
	createReq := r.client.SubnetAPI.CreateSubnet(ctx).Subnets(*unpackedScmObject)
	...
	if err != nil {
		resp.Diagnostics.AddError("Error creating subnets", err.Error())
	...
	createdObject, err := r.client.SubnetAPI.FetchSubnet(ctx, data.Name.ValueString(), nil, nil, nil)
	if err != nil {
		resp.Diagnostics.AddError("Error fetching created subnets after creation", err.Error())
		return
	}
	if createdObject == nil {
		resp.Diagnostics.AddError("Error fetching created subnets", fmt.Sprintf("Could not find subnets with name %s after creation", data.Name.ValueString()))
		return
	}
	// 6. Pack the API response back into a Terraform model data.
	packedObject, diags := packSubnetsFromSdk(ctx, *createdObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Created subnets", map[string]interface{}{"tfid": data.Tfid.ValueString()})
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
}

func (r *SubnetResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	// Step 1 - Initialize a data and savestate of type models.Subnets - which is the TF schema struct
	tflog.Debug(ctx, "Starting Read function for Subnet")
	var data, savestate models.Subnets
	...
	tflog.Debug(ctx, "Reading subnets from SCM API", map[string]interface{}{"id": objectId})
	getReq := r.client.SubnetAPI.GetSubnetByID(ctx, objectId)
	scmObject, httpErr, err := getReq.Execute()
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no subnets on read SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on read SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error reading subnets", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	savestateObject, diags := types.ObjectValueFrom(ctx, models.Subnets{}.AttrTypes(), &savestate)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
...
func (r *SubnetResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Step 1: Initialize a plan and state of type models.Subnets which is the terraform schema struct
	tflog.Debug(ctx, "Starting Update function for Subnet")
	var plan, state models.Subnets
	...
	planObject, diags := types.ObjectValueFrom(ctx, models.Subnets{}.AttrTypes(), &plan)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updating subnets on SCM API", map[string]interface{}{"id": objectId})
	updateReq := r.client.SubnetAPI.UpdateSubnetByID(ctx, objectId).Subnets(*unpackedScmObject)
	...
	if err != nil {
		if httpErr != nil && httpErr.StatusCode == http.StatusNotFound {
			tflog.Debug(ctx, "Got no subnets on update SCM API. Remove from state to let terraform create", map[string]interface{}{"id": objectId})
			resp.State.RemoveResource(ctx)
		} else {
			tflog.Debug(ctx, "Got an exception on update SCM API. ", map[string]interface{}{"id": objectId})
			resp.Diagnostics.AddError("Error updating subnets", err.Error())
			detailedMessage := utils.PrintScmError(err)
	...
	updatedObject, _, err := r.client.SubnetAPI.GetSubnetByID(ctx, objectId).Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error fetching updated subnets after update", err.Error())
		return
	}

	// Step 9: Pack the SCM updatedObject into a TF object
	packedObject, diags := packSubnetsFromSdk(ctx, *updatedObject)
	resp.Diagnostics.Append(diags...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	tflog.Debug(ctx, "Updated subnets", map[string]interface{}{"tfid": plan.Tfid.ValueString()})
...
func (r *SubnetResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	tflog.Debug(ctx, "Deleting subnets", map[string]interface{}{"id": objectId})
	deleteReq := r.client.SubnetAPI.DeleteSubnetByID(ctx, objectId)
	httpResp, err := deleteReq.Execute()
	if err != nil {
		resp.Diagnostics.AddError("Error deleting subnets", err.Error())
		detailedMessage := utils.PrintScmError(err)
		resp.Diagnostics.AddError(
	...
	if httpResp != nil && httpResp.StatusCode == http.StatusAccepted {
		...
		for time.Now().Before(deadline) {
			...
			_, getResp, getErr := r.client.SubnetAPI.GetSubnetByID(ctx, objectId).Execute()
			if getErr != nil {
